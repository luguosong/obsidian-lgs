...
**前置知识**：如果你还不了解 [[Log4j]] 1.x 的[[核心概念]]（`Logger` / `Appender` / `Layout` 三层架构），请先阅读[[Log4j]]。Logback 沿用了这套架构，理解 [[Log4j]] 后学习 Logback 会非常顺畅。
...
答案要从 Logback 的作者说起——**Ceki Gülcü**，正是他当年创建了 [[Log4j]] 1.x。随着 [[Log4j]] 1.x 的架构局限性日益明显，Ceki 决定从头设计一个更好的[[日志框架]]，这就是 Logback。
