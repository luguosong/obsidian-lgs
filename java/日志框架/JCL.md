...
`org.apache.commons.logging.Log` 是日志门面接口，定义了 6 个日志级别方法。它的实现类充当**适配器**，将 JCL 的方法调用翻译成底层[[日志框架]]的 API 调用：
...
!!! info "不需要额外依赖"
    ...
    与 [[SLF4J]] 不同，JCL 自带了 `SimpleLog` 作为兜底实现。即使 classpath 中没有任何[[日志框架]]，JCL 也能正常工作——只是输出功能很弱。
