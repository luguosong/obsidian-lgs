...
| D8 | Stage C 形态 | **视觉定位 + ODA 单点选择 + 技经表数值验证** | 复用 LandBoundary[[Vision]]Test 已验证的代码;数值验证消除"看着像"的误识别 |
...
- Stage A + [[Stage B]] 完成后,UI 提示"[[图纸记忆]]已建立 (基础信息),可开始审图"
- Stage C 在以下时机触发:
...
- `src/services/memory/ObjectExtractionPipeline.js` — Stage C 通用管线 (抽象自 LandBoundary[[Vision]]Test)
...
| 失败点 | 行为 | UI 表现 |
|---|---|---|
| Stage A 视觉超时 (>30s) | 只填 metadata 进 summary,跳过 [[Stage B]]/C | "[[图纸记忆]]建立失败,基础元数据已保存" |
...
| P6 — 化石清理 | 删 LandBoundary[[Vision]]Test / LandBoundaryAnalyzer / `RUN_LAND_BOUNDARY_TEST` / `AUTO_DRAWING_ANALYSIS_ENABLED` | P2-P5 落地 |
