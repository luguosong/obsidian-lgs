...
- **方案**：新增 `DwgReadAdapter` 6 个预构建查询方法（`extractStructure` / `extractEntities` / `extractTextContent` / `extractBlockReferences` / `extractDimensions` / `spatialQuery`），内建[[内存管理]]和安全值提取
...
- "UASVGSeal[[Entity]] 自定义实体"在图层规范、绘图标准、结构完整性 3 个维度重复
...
**仅存的 2 条跨维度重复**：
- 结构完整性维度报告了"AcDb2dPolyline 共存"和"UASVGSeal[[Entity]] 兼容风险"，这两个话题的 owner 是绘图标准维度
