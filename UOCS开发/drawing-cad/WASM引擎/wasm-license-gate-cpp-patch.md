...
## 文件 2：`LicenseGate.cpp`（新增于仓根 · 参考实现）
...
## 文件 3：`root_pubkey.inc`（自动生成，位于仓根）

**生成方法**（建议在 drawing-ai-server 侧执行，避免厂商在 drawingweb 仓库误存私钥）：

```bash
# 厂商一次性生成根密钥对（ai-server CLI，结果已在本次 commit d1a287e 验证可用）
...
## 文件 4：`CadCore.cpp` 插桩（仓根）

> **只改 2 处，每处插入约 10 行**；**不修改原有任何语句**。

### `OpenFile`（设计文档引用行号 571，按实际代码行号就近插入即可）
...
## 文件 5：`CadCoreWebBridge.cpp`（embind 追加，仓根）
...
## 文件 6：`CMakeLists.txt`（仓根 · `buildTeighaClient` macro 内）
...
## 协议对齐点（已由后端实现，无须 drawingweb 配合改动）
