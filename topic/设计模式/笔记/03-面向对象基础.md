...
[[设计模式]]是在面向对象范式上生长出来的。要真正理解模式，首先得搞清楚 OOP 的基石——对象、类、四大特性，以及类之间各种关系的精确含义。
...
- [[设计模式]]如何依赖这些 OOP 特性——即[[设计模式]]的"OOP 基因"
...
理解这六种关系，是读懂 UML 类图（[[设计模式]]图示的通用语言）的关键前提。

## 🔍 UML 类图速读

[[设计模式]]的结构通常用 **UML 类图（Class Diagram）** 表示。类图里每种关系都有固定的箭头符号：
...
GoF 的 23 种[[设计模式]]每一个都建立在 OOP 特性之上。理解模式背后的"OOP 基因"，能帮助你更快掌握模式的本质：
...
| OOP 特性 | 对应[[设计原则]] | 最能体现该特性的模式 |
|---------|-----------|-----------------|
| **封装** | 封装变化 | 工厂方法（封装对象创建）、外观（封装子系统）、命令（封装请求） |
| **继承** | ——（被克制使用） | 模板方法（唯一以继承为核心的模式） |
| **多态** | 针对接口编程 | 策略、状态、命令、观察者、迭代器——几乎所有[[行为模式]] |
| **抽象** | 依赖倒置（DIP） | 抽象工厂（多系列产品抽象）、桥接（抽象与实现解耦） |

> [!tip] 继承的
>
> 注意表格中「继承」一行：23 种模式里只有**模板方法**以继承为核心机制，其余模式几乎都通过接口/组合来实现可扩展性。这印证了「组合优于继承」原则在[[设计模式]]实践中的主流地位。
