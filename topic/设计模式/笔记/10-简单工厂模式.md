...
> 简单工厂（Simple Factory）不是 GoF 23 种[[设计模式]]之一，而是一种常见编程惯用法，也是理解[[工厂方法模式]]的起点。
...
简单工厂不是为了"高大上的[[设计模式]]"，而是一个实用的**封装变化**手段。当对象的创建逻辑（如根据文件后缀选择 Parser）散落在调用方时，每次新增类型都要修改调用方代码。把创建逻辑收拢到工厂类，调用方只需传入类型标识，其余与它无关。
