...
- 这些原语和已经学过的「非[[对称加密]]」[[数字签名]]之间的桥梁

---

## 🤔 为什么需要"下一代"密码学？

过去几十年里，密码学的产出主要解决三件事：**保密性**（[[对称加密]] / 非[[对称加密]]）、**完整性**（`MAC` / 哈希）、**真实性**（[[数字签名]] / `PKI`）。它们的共同前提是：**计算发生在明文上**。一旦数据被加密，云服务、协作方就什么也做不了，只能解密 → 计算 → 再加密。
...
> 通用 `ZKP` 与[[数字签名]]原本同源：[[数字签名]]本质上是「我知道私钥」的非交互式 `ZKP`，详见「[数字签名](../digital-signatures/)」。
