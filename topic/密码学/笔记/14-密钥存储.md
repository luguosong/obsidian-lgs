...
前面的章节中，你学会了生成密钥对、创建[[数字签名]]、加解密数据。但你有没有想过一个问题——**密钥用完之后存哪？**
...
[[2014]]年4月，安全研究员披露了 OpenSSL 的 `heartbeat` 扩展存在内存越界读取漏洞（CVE-[[2014]]-0160）。攻击者通过构造恶意心跳包，让服务器在响应中泄露最多 64KB 的内存内容——而这段内存恰好可能包含服务器的 **TLS 私钥**。
...
**通过 ProcessBuilder 调用 keytool 并验证结果**
...
macOS 的 Keychain 是系统级[[密钥管理]]服务，应用程序通过 Keychain Services API 请求密钥，系统弹出授权对话框确认访问。Java 通过 `KeychainStore` Provider 访问：

**通过 Java 访问 macOS Keychain**
...
- 🎯 [[对称加密]]密钥：每 90 天轮换一次（或每加密 `2^32` 个块后强制轮换）
...
> "[[密钥管理]]领域最大的挑战是：如果密钥存储在单一地点，这个地点就成为单点故障。"
>
...
**Shamir 秘密分享：概念性实现示例（需要 Bouncy Castle）**
