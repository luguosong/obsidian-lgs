...
所有确定性响应消息 SHALL（应当）进行数字签名。用于签名响应的密钥 MUST 属于以下之一：
...
支持 OCSP 服务（无论是本地托管还是由 Authorized Responder 提供）的 CA MUST 提供在 AccessDescription SEQUENCE 中包含 URI [RFC3986] accessLocation 值和 id-ad-ocsp accessMethod OID 值的能力。
...
请求 OCSP 服务的客户端 SHALL 能够处理使用 SHA-256 的 RSA 签名的响应（由 [RFC4055] 中指定的 sha256WithRSAEncryption OID 标识）。客户端 SHOULD 也能够处理使用 SHA-1 的 RSA 签名的响应（由 [RFC3279] 中指定的 sha1WithRSAEncryption OID 标识）以及使用 SHA-1 的数字签名算法（DSA）签名的响应（由 [RFC3279] 中指定的 id-dsa-with-sha1 OID 标识）。客户端 MAY 支持其他算法。
...
启用 OCSP 的应用程序将使用 OCSP 归档截止日期来为数字签名在产生日期时是否可靠提供证据，即使验证签名所需的证书早已过期。
...
AlgorithmIdentifier 的语法定义在 RFC 5280 [RFC5280] 第 4.1.1.2 节中。SMIMECapability 的语法定义在 RFC 5751 [RFC5751] 中。
