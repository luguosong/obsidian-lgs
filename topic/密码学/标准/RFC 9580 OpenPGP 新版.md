...
Information about the current status of this document, any errata, and how to provide feedback on it may be obtained at [https://www.rfc-editor.org/info/rfc9580](https://www.rfc-editor.org/info/rfc9580).[¶](#section-boilerplate.1-3)
...
Both digital signature and confidentiality services may be applied to the same message. First, a signature is generated for the message and attached to the message. Then, the message plus signature is encrypted using a symmetric message key derived from the session key. Finally, the session key is encrypted using public key encryption and prefixed to the encrypted block.[¶](#section-2.1-4)

### 2.2.

The digital signature uses a cryptographic hash function and a public key algorithm capable of signing. The sequence is as follows:[¶](#section-2.2-1)

1. The sender creates a message.[¶](#section-2.2-2.1)
2. The sending implementation generates a hash digest of the message.[¶](#section-2.2-2.2)
3. The sending implementation generates a signature from the hash digest using the sender's private key.[¶](#section-2.2-2.3)
4. The signature is attached to or transmitted alongside the message.[¶](#section-2.2-2.4)
5. The receiving implementation obtains a copy of the message and the message signature.[¶](#section-2.2-2.5)
6. The receiving implementation generates a new hash digest for the received message and verifies it using the message's signature. If the verification is successful, the message is accepted as authentic.[¶](#section-2.2-2.6)
...
Scalar numbers are unsigned and always stored in big-endian format. Using n\[k\] to refer to the kth octet being interpreted, the value of a 2-octet scalar is ((n\[0\] << 8) + n\[1\]). The value of a 4-octet scalar is ((n\[0\] << 24) + (n\[1\] << 16) + (n\[2\] << 8) + n\[3\]).[¶](#section-3.1-1)

### 3.2.

Multiprecision Integers (MPIs) are unsigned integers used to hold large integers such as the ones used in cryptographic calculations.[¶](#section-3.2-1)

An MPI consists of two pieces: a 2-octet scalar that is the length of the MPI in bits, followed by a string of octets that contain the actual integer.[¶](#section-3.2-2)

These octets form a big-endian number; a big-endian number can be made into an MPI by prefixing it with the appropriate length.[¶](#section-3.2-3)

Examples:[¶](#section-3.2-4)

(Note that all numbers in the octet strings identified by square brackets are in hexadecimal.) [¶](#section-3.2-5)

- The string of octets \[00 00\] forms an MPI with the value 0.[¶](#section-3.2-6.1)
- The string of octets \[00 01 01\] forms an MPI with the value 1.[¶](#section-3.2-6.2)
- The string \[00 09 01 FF\] forms an MPI with the value 511.[¶](#section-3.2-6.3)

Additional rules:[¶](#section-3.2-7)

- The size of an MPI is ((MPI.length + 7) / 8) + 2 octets.[¶](#section-3.2-8.1)
- The length field of an MPI describes the length starting from its most significant non-zero bit. Thus, the MPI \[00 02 01\] is not formed correctly. It should be \[00 01 01\]. When parsing an MPI in a version 6 Key, Signature, or Public Key Encrypted Session Key (PKESK) packet, the implementation MUST check that the encoded length matches the length starting from the most significant non-zero bit; if it doesn't match, reject the packet as malformed.[¶](#section-3.2-8.2)
- Unused bits of an MPI MUST be zero.[¶](#section-3.2-8.3)

#### 3.2.1.

Note that in some places, MPIs are used to encode non-integer data, such as an elliptic curve (EC) point (see ) or an octet string of known, fixed length (see ). The wire representation is the same: 2 octets of length in bits counted from the first non-zero bit, followed by the smallest series of octets that can represent the value while stripping off any leading zero octets.[¶](#section-3.2.1-1)

### 3.3.

A Key ID is an 8-octet scalar that identifies a key. Implementations SHOULD NOT assume that Key IDs are unique. A fingerprint is more likely to be unique than a Key ID. The fingerprint and Key ID of a key are calculated differently according to the version of the key.[¶](#section-3.3-1)

describes how Key IDs and Fingerprints are formed.[¶](#section-3.3-2)

### 3.4.

Unless otherwise specified, the character set for text is the UTF-8 \[\] encoding of Unicode \[\].[¶](#section-3.4-1)

### 3.5.

A time field is an unsigned 4-octet number containing the number of seconds elapsed since midnight, 1 January 1970 UTC.[¶](#section-3.5-1)

### 3.6.

A keyring is a collection of one or more keys in a file or database. Typically, a keyring is simply a sequential list of keys, but it may be any suitable database. It is beyond the scope of this specification to discuss the details of keyrings or other databases.[¶](#section-3.6-1)

### 3.7.

A string-to-key (S2K) Specifier is used to convert a passphrase string into a symmetric key encryption/decryption key. Passphrases requiring use of S2K conversion are currently used in two places: to encrypt the secret part of private keys and for symmetrically encrypted messages.[¶](#section-3.7-1)
...
Simple S2K directly hashes the string to produce the key data. This hashing is done as shown below.[¶](#section-3.7.1.1-1)
...
Salted S2K is exactly like Simple S2K, except that the input to the hash function(s) consists of the 8 octets of salt from the S2K Specifier, followed by the passphrase.[¶](#section-3.7.1.2-3)

##### 3.7.1.3.

Iterated and Salted S2K includes both a salt and an octet count. The salt is combined with the passphrase, and the resulting value is repeated and then hashed. This further increases the amount of work an attacker must do to try dictionary attacks.[¶](#section-3.7.1.3-1)
...
The count is coded into a 1-octet number using the following formula:[¶](#section-3.7.1.3-3)
...
The above formula is described in \[\], where "Int32" is a type for a 32-bit integer, and the variable "c" is the coded count, octet 10.[¶](#section-3.7.1.3-5)

Iterated and Salted S2K hashes the passphrase and salt data multiple times. The total number of octets to be hashed is specified in the encoded count in the S2K Specifier. Note that the resulting count value is an octet count of how many octets will be hashed, not an iteration count.[¶](#section-3.7.1.3-6)

Initially, one or more hash contexts are set up the same as the other S2K algorithms, depending on how many octets of key data are needed. Then the salt, followed by the passphrase data, is repeatedly processed as input to each hash context until the number of octets specified by the octet count has been hashed. The input is truncated to the octet count, except if the octet count is less than the initial size of the salt plus passphrase. That is, at least one copy of the full salt plus passphrase will be provided as input to each hash context regardless of the octet count. After the hashing is done, the key data is produced from the hash digest(s), which is the same way it is produced for the other S2K algorithms.[¶](#section-3.7.1.3-7)
...
The salt SHOULD be unique for each passphrase.[¶](#section-3.7.1.4-3)

The number of passes t and the degree of parallelism p MUST be non-zero.[¶](#section-3.7.1.4-4)
...
As an example, with the first recommended option (t=1, p=4, m=2 <sup>21</sup>), the full S2K Specifier would be:[¶](#section-3.7.1.4-8)
...
where XX represents a random octet of salt.[¶](#section-3.7.1.4-10)
...
The first octet following the public key material in a Secret Key packet () indicates whether and how the secret key material is passphrase protected. This first octet is known as the "S2K usage octet".[¶](#section-3.7.2.1-1)

If the S2K usage octet is zero, the secret key data is unprotected. If it is non-zero, it describes how to use a passphrase to unlock the secret key.[¶](#section-3.7.2.1-2)

Implementations predating \[\] indicated a protected key by storing a Symmetric Cipher Algorithm ID (see ) in the S2K usage octet. In this case, the MD5 hash function was always used to convert the passphrase to a key for the specified cipher algorithm.[¶](#section-3.7.2.1-3)
...
<table><caption>:</caption><thead><tr><th rowspan="1" colspan="1">S2K Usage Octet</th><th rowspan="1" colspan="1">Shorthand</th><th rowspan="1" colspan="1">Encryption Parameter Fields</th><th rowspan="1" colspan="1">Encryption</th><th rowspan="1" colspan="1">Gen?</th></tr></thead><tbody><tr><td rowspan="1" colspan="1">0</td><td rowspan="1" colspan="1">Unprotected</td><td rowspan="1" colspan="1">-</td><td rowspan="1" colspan="1"><strong>v3 or v4 keys:</strong> [cleartext secrets || check(secrets)]<br><strong>v6 keys:</strong> [cleartext secrets]</td><td rowspan="1" colspan="1">Yes</td></tr><tr><td rowspan="1" colspan="1">Known symmetric cipher algo ID (see )</td><td rowspan="1" colspan="1">LegacyCFB</td><td rowspan="1" colspan="1">IV</td><td rowspan="1" colspan="1">CFB(MD5(passphrase), secrets || check(secrets))</td><td rowspan="1" colspan="1">No</td></tr><tr><td rowspan="1" colspan="1">253</td><td rowspan="1" colspan="1">AEAD</td><td rowspan="1" colspan="1">params-length (<strong>v6-only</strong>), cipher-algo, AEAD-mode, S2K-specifier-length (<strong>v6-only</strong>), S2K-specifier, nonce</td><td rowspan="1" colspan="1">AEAD(HKDF(S2K(passphrase), info), secrets, packetprefix)</td><td rowspan="1" colspan="1">Yes</td></tr><tr><td rowspan="1" colspan="1">254</td><td rowspan="1" colspan="1">CFB</td><td rowspan="1" colspan="1">params-length (<strong>v6-only</strong>), cipher-algo, S2K-specifier-length (<strong>v6-only</strong>), S2K-specifier, IV</td><td rowspan="1" colspan="1">CFB(S2K(passphrase), secrets || SHA1(secrets))</td><td rowspan="1" colspan="1">Yes</td></tr><tr><td rowspan="1" colspan="1">255</td><td rowspan="1" colspan="1">MalleableCFB</td><td rowspan="1" colspan="1">cipher-algo, S2K-specifier, IV</td><td rowspan="1" colspan="1">CFB(S2K(passphrase), secrets || check(secrets))</td><td rowspan="1" colspan="1">No</td></tr></tbody></table>

When emitting a secret key (with or without passphrase protection), an implementation MUST only produce data from a row with "Generate?" marked as "Yes". Each row with "Generate?" marked as "No" is described for backward compatibility (for reading version 4 and earlier keys only) and MUST NOT be used to generate new output. Version 6 secret keys using these formats MUST be rejected.[¶](#section-3.7.2.1-8)
...
When handling a stream of packets, the length information in each packet header is the canonical source of packet boundaries. An implementation handling a packet stream that wants to find the next packet MUST look for it at the precise offset indicated in the previous packet header.[¶](#section-4.1-3)

Additionally, some packets contain internal length indicators (for example, a subfield within the packet). In the event that a subfield length indicator within a packet implies inclusion of octets outside the range indicated in the packet header, a parser MUST abort without writing outside the indicated range and MUST treat the packet as malformed and unusable.[¶](#section-4.1-4)

An implementation MUST NOT interpret octets outside the range indicated in the packet header as part of the contents of the packet.[¶](#section-4.1-5)
...
3. A 5-octet Body Length header encodes packet lengths of up to 4,294,967,295 (0xFFFFFFFF) octets in length. (This actually encodes a 4-octet scalar number.) [¶](#section-4.2.1-2.3.1)
4. When the length of the packet body is not known in advance by the issuer, Partial Body Length headers encode a packet of indeterminate length, effectively making it a stream.[¶](#section-4.2.1-2.4.1)

##### 4.2.1.1.

A 1-octet Body Length header encodes a length of 0 to 191 octets. This type of length header is recognized because the 1-octet value is less than 192. The body length is equal to:[¶](#section-4.2.1.1-1)
...
A 2-octet Body Length header encodes a length of 192 to 8383 octets. It is recognized because its first octet is in the range 192 to 223. The body length is equal to:[¶](#section-4.2.1.2-1)
...
A 5-octet Body Length header consists of a single octet holding the value 255, followed by a 4-octet scalar. The body length is equal to:[¶](#section-4.2.1.3-1)
...
This basic set of 1-octet, 2-octet, and 5-octet lengths is also used internally to some packets.[¶](#section-4.2.1.3-3)

##### 4.2.1.4.

A Partial Body Length header is 1 octet long and encodes the length of only part of the data packet. This length is a power of 2, from 1 to 1,073,741,824 (2 to the 30th power). It is recognized by its 1-octet value that is greater than or equal to 224, and less than 255. The Partial Body Length is equal to:[¶](#section-4.2.1.4-1)
...
Each Partial Body Length header is followed by a portion of the packet body data; the Partial Body Length header specifies this portion's length. Another length header (1-octet, 2-octet, 5-octet, or partial) follows that portion. The last length header in the packet MUST NOT be a Partial Body Length header. Partial Body Length headers may only be used for the non-final parts of the packet.[¶](#section-4.2.1.4-3)

Note also that the last Body Length header can be a zero-length header.[¶](#section-4.2.1.4-4)

An implementation MAY use Partial Body Lengths for data packets, whether they are literal, compressed, or encrypted. The first partial length MUST be at least 512 octets long. Partial Body Lengths MUST NOT be used for any other packet types.[¶](#section-4.2.1.4-5)

#### 4.2.2.

A zero in bit 6 of the first octet of the packet indicates a Legacy packet format. Bits 1 and 0 of the first octet of a Legacy packet are the "length-type" field. The meaning of length-type in Legacy format packets is as follows:[¶](#section-4.2.2-1)

0

The packet has a 1-octet length. The header is 2 octets long.[¶](#section-4.2.2-2.2)

1

The packet has a 2-octet length. The header is 3 octets long.[¶](#section-4.2.2-2.4)

2

The packet has a 4-octet length. The header is 5 octets long.[¶](#section-4.2.2-2.6)
...
- A packet body with length 100000 may have its length encoded in five octets: 0xFF, 0x00, 0x01, 0x86, 0xA0.[¶](#section-4.2.3-2.3)

It might also be encoded in the following octet stream:[¶](#section-4.2.3-3)

- 0xEF, first 32768 octets of data;[¶](#section-4.2.3-4.1)
- 0xE1, next 2 octets of data;[¶](#section-4.2.3-4.2)
- 0xE0, next 1 octet of data;[¶](#section-4.2.3-4.3)
- 0xF0, next 65536 octets of data; and [¶](#section-4.2.3-4.4)
- 0xC5, 0xDD, last 1693 octets of data.[¶](#section-4.2.3-4.5)

This is just one possible encoding, and many variations are possible on the size of the Partial Body Length headers, as long as a regular Body Length header encodes the last portion of the data.[¶](#section-4.2.3-5)

Please note that in all of these explanations, the total length of the packet is the length of the header(s) plus the length of the body.[¶](#section-4.2.3-6)

### 4.3.

The Packet Type ID space is partitioned into critical packets and non-critical packets. If an implementation encounters a critical packet where the packet type is unknown in a packet sequence, it MUST reject the whole packet sequence (see ). On the other hand, an unknown non-critical packet MUST be ignored.[¶](#section-4.3-1)

Packets with Type IDs from 0 to 39 are critical. Packets with Type IDs from 40 to 63 are non-critical.[¶](#section-4.3-2)

## 5.

The defined packet types are as follows:[¶](#section-5-1)
...
The versions differ in how they identify the recipient key and in what they encode. The version of the PKESK packet must align with the version of the SEIPD packet (see ). Any new version of the PKESK packet should be registered in the registry established in.[¶](#section-5.1-3)

#### 5.1.1.

A version 3 PKESK packet precedes a v1 SEIPD packet (see ). In historic data, it is sometimes found preceding a deprecated SED packet; see. A v3 PKESK packet MUST NOT precede a v2 SEIPD packet (see ).[¶](#section-5.1.1-1)

The v3 PKESK packet consists of:[¶](#section-5.1.1-2)

- A 1-octet version number with value 3.[¶](#section-5.1.1-3.1)
- An 8-octet number that gives the Key ID of the public key to which the session key is encrypted. If the session key is encrypted to a subkey, then the Key ID of this subkey is used here instead of the Key ID of the primary key. The Key ID may also be all zeros, for an "anonymous recipient" (see ).[¶](#section-5.1.1-3.2)
- A 1-octet number giving the public key algorithm used.[¶](#section-5.1.1-3.3)
- A series of values comprising the encrypted session key. This is algorithm specific and described below.[¶](#section-5.1.1-3.4)

The public key encryption algorithm (described in subsequent sections) is passed two values:[¶](#section-5.1.1-4)

- The session key.[¶](#section-5.1.1-5.1)
- The 1-octet algorithm identifier that specifies the symmetric key encryption algorithm used to encrypt the v1 SEIPD packet described in the following section.[¶](#section-5.1.1-5.2.1)

#### 5.1.2.

A v6 PKESK packet precedes a v2 SEIPD packet (see ). A v6 PKESK packet MUST NOT precede a v1 SEIPD packet or a deprecated SED packet (see ).[¶](#section-5.1.2-1)

The v6 PKESK packet consists of the following fields:[¶](#section-5.1.2-2)

- A 1-octet version number with value 6.[¶](#section-5.1.2-3.1)
- A 1-octet size of the following two fields. This size may be zero, if the key version number field and the fingerprint field are omitted for an "anonymous recipient" (see ).[¶](#section-5.1.2-3.2)
- A 1-octet key version number.[¶](#section-5.1.2-3.3)
- The fingerprint of the public key or subkey to which the session key is encrypted. Note that the length N of the fingerprint for a version 4 key is 20 octets; for a version 6 key, N is 32.[¶](#section-5.1.2-3.4)
- A 1-octet number giving the public key algorithm used.[¶](#section-5.1.2-3.5)
- A series of values comprising the encrypted session key. This is algorithm specific and described below.[¶](#section-5.1.2-3.6)

The session key is encrypted according to the public key algorithm used, as described below. No symmetric key encryption algorithm identifier is passed to the public key algorithm for a v6 PKESK packet, as it is included in the v2 SEIPD packet.[¶](#section-5.1.2-4)

#### 5.1.3.

- MPI of RSA-encrypted value m <sup>e</sup> mod n.[¶](#section-5.1.3-1.1)

To produce the value "m" in the above formula, first concatenate the following values:[¶](#section-5.1.3-2)

- The 1-octet algorithm identifier, if it was passed (in the case of a v3 PKESK packet).[¶](#section-5.1.3-3.1)
- The session key.[¶](#section-5.1.3-3.2)
- A 2-octet checksum of the session key, equal to the sum of the session key octets, modulo 65536.[¶](#section-5.1.3-3.3)

Then, the above values are encoded using the PKCS#1 block encoding EME-PKCS1-v1\_5, as described in Step 2 in [Section 7.2.1](https://rfc-editor.org/rfc/rfc8017#section-7.2.1) of \[\] (see also ). When decoding "m" during decryption, an implementation should follow Step 3 in [Section 7.2.2](https://rfc-editor.org/rfc/rfc8017#section-7.2.2) of \[\] (see also ).[¶](#section-5.1.3-4)

Note that when an implementation forms several PKESK packets with one session key, forming a message that can be decrypted by several keys, the implementation MUST make a new PKCS#1 encoding for each key. This defends against attacks such as those discussed in \[\].[¶](#section-5.1.3-5)

#### 5.1.4.

- MPI of Elgamal (Diffie-Hellman) value g <sup>k</sup> mod p.[¶](#section-5.1.4-1.1.1)
- MPI of Elgamal (Diffie-Hellman) value m \* y <sup>k</sup> mod p.[¶](#section-5.1.4-1.2.1)

To produce the value "m" in the above formula, first concatenate the following values:[¶](#section-5.1.4-2)

- The 1-octet algorithm identifier, if it was passed (in the case of a v3 PKESK packet).[¶](#section-5.1.4-3.1.1)
- The session key.[¶](#section-5.1.4-3.2.1)
- A 2-octet checksum of the session key, equal to the sum of the session key octets, modulo 65536.[¶](#section-5.1.4-3.3.1)

Then, the above values are encoded using the PKCS#1 block encoding EME-PKCS1-v1\_5, as described in Step 2 in [Section 7.2.1](https://rfc-editor.org/rfc/rfc8017#section-7.2.1) of \[\] (see also ). When decoding "m" during decryption, an implementation should follow Step 3 in [Section 7.2.2](https://rfc-editor.org/rfc/rfc8017#section-7.2.2) of \[\] (see also ).[¶](#section-5.1.4-4)

Note that when an implementation forms several PKESK packets with one session key, forming a message that can be decrypted by several keys, the implementation MUST make a new PKCS#1 encoding for each key. This defends against attacks such as those discussed in \[\].[¶](#section-5.1.4-5)

An implementation MUST NOT generate ElGamal v6 PKESK packets.[¶](#section-5.1.4-6)

#### 5.1.5.

- MPI of an EC point representing an ephemeral public key in the point format associated with the curve as specified in.[¶](#section-5.1.5-1.1)
- A 1-octet size, followed by a symmetric key encoded using the method described in.[¶](#section-5.1.5-1.2)

#### 5.1.6.

- 32 octets representing an ephemeral X25519 public key.[¶](#section-5.1.6-1.1)
- A 1-octet size of the following fields.[¶](#section-5.1.6-1.2)
...
- 32 octets of the shared secret.[¶](#section-5.1.6-3.3.1)

The key produced from HKDF is used to encrypt the session key with AES-128 key wrap, as defined in \[\].[¶](#section-5.1.6-4)
...
- 56 octets representing an ephemeral X448 public key.[¶](#section-5.1.7-1.1.1)
- A 1-octet size of the following fields.[¶](#section-5.1.7-1.2.1)
...
- 56 octets of the shared secret.[¶](#section-5.1.7-3.3.1)

The key produced from HKDF is used to encrypt the session key with AES-256 key wrap, as defined in \[\].[¶](#section-5.1.7-4)

Note that unlike ECDH, no checksum or padding are appended to the session key before key wrapping. Finally, note that unlike the other public key algorithms, in the case of a v3 PKESK packet, the symmetric algorithm ID is not encrypted. Instead, it is prepended to the encrypted session key in plaintext. In this case, the symmetric algorithm used MUST be AES-128, AES-192, or AES-256 (algorithm ID 7, 8, or 9).[¶](#section-5.1.7-5)

#### 5.1.8.

An implementation MAY accept or use a Key ID of all zeros, or an omitted key fingerprint, to hide the intended decryption key. In this case, the receiving implementation would try all available private keys, checking for a valid decrypted session key. This format helps reduce traffic analysis of messages.[¶](#section-5.1.8-1)

### 5.2.

A Signature packet describes a binding between some public key and some data. The most common signatures are a signature of a file or a block of text and a signature that is a certification of a User ID.[¶](#section-5.2-1)

Three versions of Signature packets are defined. Version 3 provides basic signature information, while versions 4 and 6 provide an expandable format with subpackets that can specify more information about the signature.[¶](#section-5.2-2)

For historical reasons, versions 1, 2, and 5 of the Signature packet are unspecified. Any new Signature packet version should be registered in the registry established in.[¶](#section-5.2-3)

An implementation MUST generate a version 6 signature when signing with a version 6 key. An implementation MUST generate a version 4 signature when signing with a version 4 key. Implementations MUST NOT create version 3 signatures; they MAY accept version 3 signatures. See for more details about packet version correspondence between keys and signatures.[¶](#section-5.2-4)
...
The meanings of each signature type are described in the subsections below.[¶](#section-5.2.1-3)

##### 5.2.1.1.

This means the signer owns it, created it, or certifies that it has not been modified.[¶](#section-5.2.1.1-1)

##### 5.2.1.2.

This means the signer owns it, created it, or certifies that it has not been modified. The signature is calculated over the text data with its line endings converted to <CR><LF>.[¶](#section-5.2.1.2-1)

##### 5.2.1.3.

This signature is a signature of only its own subpacket contents. It is calculated identically to a signature over a zero-length binary document. Version 3 Standalone signatures MUST NOT be generated and MUST be ignored.[¶](#section-5.2.1.3-1)

##### 5.2.1.4.

The issuer of this certification does not make any particular assertion as to how well the certifier has checked that the owner of the key is in fact the person described by the User ID.[¶](#section-5.2.1.4-1)

##### 5.2.1.5.

The issuer of this certification has not done any verification of the claim that the owner of this key is the User ID specified.[¶](#section-5.2.1.5-1)

##### 5.2.1.6.

The issuer of this certification has done some casual verification of the claim of identity.[¶](#section-5.2.1.6-1)
...
This signature is a statement by the top-level signing key, indicating that it owns the subkey. This signature is calculated directly on the primary key and subkey, and not on any User ID or other packets. A signature that binds a signing subkey MUST have an Embedded Signature subpacket in this binding signature that contains a 0x19 signature made by the signing subkey on the primary key and subkey.[¶](#section-5.2.1.8-1)

##### 5.2.1.9.

This signature is a statement by a signing subkey, indicating that it is owned by the primary key. This signature is calculated the same way as a Subkey Binding signature (Type ID 0x18): directly on the primary key and subkey, and not on any User ID or other packets.[¶](#section-5.2.1.9-1)

##### 5.2.1.10.

This signature is calculated directly on a key. It binds the information in the Signature subpackets to the key and is appropriate to be used for subpackets that provide information about the key, such as the Key Flags subpacket or the (deprecated) Revocation Key subpacket. It is also appropriate for statements that non-self certifiers want to make about the key itself rather than the binding between a key and a name.[¶](#section-5.2.1.10-1)

##### 5.2.1.11.

This signature is calculated directly on the key being revoked. A revoked key is not to be used. Only Revocation Signatures by the key being revoked, or by a (deprecated) Revocation Key, should be considered valid Revocation Signatures.[¶](#section-5.2.1.11-1)

##### 5.2.1.12.

This signature is calculated directly on the primary key and the subkey being revoked. A revoked subkey is not to be used. Only Revocation Signatures by the top-level signature key that is bound to this subkey, or by a (deprecated) Revocation Key, should be considered valid Revocation Signatures.[¶](#section-5.2.1.12-1)

##### 5.2.1.13.

This signature revokes an earlier User ID certification signature (Type IDs 0x10 through 0x13) or Direct Key signature (Type ID 0x1F). It should be issued by the same key that issued the revoked signature or by a (deprecated) Revocation Key. The signature is computed over the same data as the certification that it revokes, and it should have a later creation date than that certification.[¶](#section-5.2.1.13-1)
...
An implementation MUST NOT create any signature with this type and MUST NOT validate any signature made with this type. See for more details.[¶](#section-5.2.1.16-1)

#### 5.2.2.

The body of a version 3 Signature packet contains:[¶](#section-5.2.2-1)

- A 1-octet version number with value 3.[¶](#section-5.2.2-2.1.1)
- A 1-octet length of the following hashed material; it MUST be 5:[¶](#section-5.2.2-2.2.1)
	- A 1-octet Signature Type ID.[¶](#section-5.2.2-2.2.2.1.1)
		- A 4-octet creation time.[¶](#section-5.2.2-2.2.2.2.1)
- An 8-octet Key ID of the signer.[¶](#section-5.2.2-2.3.1)
- A 1-octet public key algorithm.[¶](#section-5.2.2-2.4.1)
- A 1-octet hash algorithm.[¶](#section-5.2.2-2.5.1)
- A 2-octet field holding left 16 bits of the signed hash value.[¶](#section-5.2.2-2.6.1)
- One or more MPIs comprising the signature. This portion is algorithm specific, as described below.[¶](#section-5.2.2-2.7.1)

The concatenation of the data to be signed, the signature type, and the creation time from the Signature packet (5 additional octets) is hashed. The resulting hash value is used in the signature algorithm. The high 16 bits (first two octets) of the hash are included in the Signature packet to provide a way to reject some invalid signatures without performing a signature verification.[¶](#section-5.2.2-3)

Algorithm-specific fields for RSA signatures:[¶](#section-5.2.2-4)

- MPI of RSA signature value m <sup>d</sup> mod n.[¶](#section-5.2.2-5.1.1)

Algorithm-specific fields for DSA signatures:[¶](#section-5.2.2-6)

- MPI of DSA value r.[¶](#section-5.2.2-7.1.1)
- MPI of DSA value s.[¶](#section-5.2.2-7.2.1)

The signature calculation is based on a hash of the signed data, as described above. The details of the calculation are different for DSA signatures than for RSA signatures; see Sections and.[¶](#section-5.2.2-8)

#### 5.2.3.

The body of a version 4 or version 6 Signature packet contains:[¶](#section-5.2.3-1)

- A 1-octet version number. This is 4 for version 4 signatures and 6 for version 6 signatures.[¶](#section-5.2.3-2.1.1)
- A 1-octet Signature Type ID.[¶](#section-5.2.3-2.2.1)
- A 1-octet public key algorithm.[¶](#section-5.2.3-2.3.1)
- A 1-octet hash algorithm.[¶](#section-5.2.3-2.4.1)
- A scalar octet count for the hashed subpacket data that follows this field. For a version 4 signature, this is a 2-octet field. For a version 6 signature, this is a 4-octet field. Note that this is the length in octets of all of the hashed subpackets; an implementation's pointer incremented by this number will skip over the hashed subpackets.[¶](#section-5.2.3-2.5.1)
- A hashed subpacket data set (zero or more subpackets).[¶](#section-5.2.3-2.6.1)
- A scalar octet count for the unhashed subpacket data that follows this field. For a version 4 signature, this is a 2-octet field. For a version 6 signature, this is a 4-octet field. Note that this is the length in octets of all of the unhashed subpackets; an implementation's pointer incremented by this number will skip over the unhashed subpackets.[¶](#section-5.2.3-2.7.1)
- An unhashed subpacket data set (zero or more subpackets).[¶](#section-5.2.3-2.8.1)
- A 2-octet field holding the left 16 bits of the signed hash value.[¶](#section-5.2.3-2.9.1)
- Only for version 6 signatures, a variable-length field containing:[¶](#section-5.2.3-2.10.1)
	- A 1-octet salt size. The value MUST match the value defined for the hash algorithm as specified in.[¶](#section-5.2.3-2.10.2.1.1)
		- The salt, which is a random value of the specified size.[¶](#section-5.2.3-2.10.2.2.1)
- One or more MPIs comprising the signature. This portion is algorithm specific.[¶](#section-5.2.3-2.11.1)
...
- MPI of DSA or ECDSA value r.[¶](#section-5.2.3.2-1.1.1)
- MPI of DSA or ECDSA value s.[¶](#section-5.2.3.2-1.2.1)

A version 3 signature MUST NOT be created and MUST NOT be used with the Elliptic Curve Digital Signature Algorithm (ECDSA).[¶](#section-5.2.3.2-2)

A DSA signature MUST use a hash algorithm with a digest size of at least the number of bits of q, the group generated by the DSA key's generator value.[¶](#section-5.2.3.2-3)

If the output size of the chosen hash is larger than the number of bits of q, the hash result is truncated to fit by taking the number of leftmost bits equal to the number of bits of q. This (possibly truncated) hash function result is treated as a number and used directly in the DSA signature algorithm.[¶](#section-5.2.3.2-4)

An ECDSA signature MUST use a hash algorithm with a digest size of at least the curve's "fsize" value (see ), except in the case of NIST P-521, for which at least a 512-bit hash algorithm MUST be used.[¶](#section-5.2.3.2-5)

##### 5.2.3.3.

- Two MPI-encoded values, whose contents and formatting depend on the choice of curve used (see ).[¶](#section-5.2.3.3-1.1.1)

A version 3 signature MUST NOT be created and MUST NOT be used with EdDSALegacy.[¶](#section-5.2.3.3-2)

An EdDSALegacy signature MUST use a hash algorithm with a digest size of at least the curve's "fsize" value (see ). A verifying implementation MUST reject any EdDSALegacy signature that uses a hash algorithm with a smaller digest size.[¶](#section-5.2.3.3-3)

###### 5.2.3.3.1.

The two MPIs for Ed25519Legacy represent the octet strings R and S of the Edwards-curve Digital Signature Algorithm (EdDSA) described in \[\].[¶](#section-5.2.3.3.1-1)

- MPI of an EC point R, represented as a (non-prefixed) native (little-endian) octet string up to 32 octets.[¶](#section-5.2.3.3.1-2.1.1)
- MPI of EdDSA value S, also in (non-prefixed) native (little-endian) format with a length up to 32 octets.[¶](#section-5.2.3.3.1-2.2.1)

Ed25519Legacy MUST NOT be used in Signature packets version 6 or above.[¶](#section-5.2.3.3.1-3)

##### 5.2.3.4.

- 64 octets of the native signature.[¶](#section-5.2.3.4-1.1.1)

For more details, see.[¶](#section-5.2.3.4-2)

A version 3 signature MUST NOT be created and MUST NOT be used with Ed25519.[¶](#section-5.2.3.4-3)

An Ed25519 signature MUST use a hash algorithm with a digest size of at least 256 bits. A verifying implementation MUST reject any Ed25519 signature that uses a hash algorithm with a smaller digest size.[¶](#section-5.2.3.4-4)

##### 5.2.3.5.

- 114 octets of the native signature.[¶](#section-5.2.3.5-1.1.1)

For more details, see.[¶](#section-5.2.3.5-2)

A version 3 signature MUST NOT be created and MUST NOT be used with Ed448.[¶](#section-5.2.3.5-3)

An Ed448 signature MUST use a hash algorithm with a digest size of at least 512 bits. A verifying implementation MUST reject any Ed448 signature that uses a hash algorithm with a smaller digest size.[¶](#section-5.2.3.5-4)

##### 5.2.3.6.

The concatenation of the data being signed, the signature data from the version number through the hashed subpacket data (inclusive), and (for signature versions later than 3) a 6-octet trailer (see ) is hashed. The resulting hash value is what is signed. The high 16 bits (first two octets) of the hash are included in the Signature packet to provide a way to reject some invalid signatures without performing a signature verification. When verifying a version 6 signature, an implementation MUST reject the signature if these octets do not match the first two octets of the computed hash.[¶](#section-5.2.3.6-1)

There are two fields consisting of Signature subpackets. The first field is hashed with the rest of the signature data, while the second is not hashed into the signature. The second set of subpackets (the "unhashed section") is not cryptographically protected by the signature and should include only advisory information. See for more information.[¶](#section-5.2.3.6-2)

The differences between a version 4 and version 6 signature are two-fold: first, a version 6 signature increases the width of the fields that indicate the size of the hashed and unhashed subpackets, making it possible to include significantly more data in subpackets. Second, the hash is salted with random data (see ).[¶](#section-5.2.3.6-3)

The algorithms for converting the hash function result to a signature are described in.[¶](#section-5.2.3.6-4)

##### 5.2.3.7.

A subpacket data set consists of zero or more Signature subpackets. In Signature packets, the subpacket data set is preceded by a 2-octet (for version 4 signatures) or 4-octet (for version 6 signatures) scalar count of the length in octets of all the subpackets. A pointer incremented by this number will skip over the subpacket data set.[¶](#section-5.2.3.7-1)

Each subpacket consists of a subpacket header and a body. The header consists of:[¶](#section-5.2.3.7-2)

- The encoded subpacket length (1, 2, or 5 octets).[¶](#section-5.2.3.7-3.1)
...
An implementation SHOULD ignore any non-critical subpacket of a type that it does not recognize.[¶](#section-5.2.3.7-7)

An evaluator may "recognize" a subpacket but not implement it. The purpose of the critical bit is to allow the signer to tell an evaluator that it would prefer a new, unknown feature to generate an error rather than being ignored.[¶](#section-5.2.3.7-8)

The other bits of the encoded Subpacket Type ID (i.e., bits 6-0) contain the Subpacket Type ID.[¶](#section-5.2.3.7-9)

The following signature subpackets are defined:[¶](#section-5.2.3.7-10)

<table><caption>:</caption><thead><tr><th rowspan="1" colspan="1">ID</th><th rowspan="1" colspan="1">Description</th><th rowspan="1" colspan="1">Reference</th></tr></thead><tbody><tr><td rowspan="1" colspan="1">0</td><td rowspan="1" colspan="1">Reserved</td><td rowspan="1" colspan="1"></td></tr><tr><td rowspan="1" colspan="1">1</td><td rowspan="1" colspan="1">Reserved</td><td rowspan="1" colspan="1"></td></tr><tr><td rowspan="1" colspan="1">2</td><td rowspan="1" colspan="1">Signature Creation Time</td><td rowspan="1" colspan="1"></td></tr><tr><td rowspan="1" colspan="1">3</td><td rowspan="1" colspan="1">Signature Expiration Time</td><td rowspan="1" colspan="1"></td></tr><tr><td rowspan="1" colspan="1">4</td><td rowspan="1" colspan="1">Exportable Certification</td><td rowspan="1" colspan="1"></td></tr><tr><td rowspan="1" colspan="1">5</td><td rowspan="1" colspan="1">Trust Signature</td><td rowspan="1" colspan="1"></td></tr><tr><td rowspan="1" colspan="1">6</td><td rowspan="1" colspan="1">Regular Expression</td><td rowspan="1" colspan="1"></td></tr><tr><td rowspan="1" colspan="1">7</td><td rowspan="1" colspan="1">Revocable</td><td rowspan="1" colspan="1"></td></tr><tr><td rowspan="1" colspan="1">8</td><td rowspan="1" colspan="1">Reserved</td><td rowspan="1" colspan="1"></td></tr><tr><td rowspan="1" colspan="1">9</td><td rowspan="1" colspan="1">Key Expiration Time</td><td rowspan="1" colspan="1"></td></tr><tr><td rowspan="1" colspan="1">10</td><td rowspan="1" colspan="1">Placeholder for backward compatibility</td><td rowspan="1" colspan="1"></td></tr><tr><td rowspan="1" colspan="1">11</td><td rowspan="1" colspan="1">Preferred Symmetric Ciphers for v1 SEIPD</td><td rowspan="1" colspan="1"></td></tr><tr><td rowspan="1" colspan="1">12</td><td rowspan="1" colspan="1">Revocation Key (deprecated)</td><td rowspan="1" colspan="1"></td></tr><tr><td rowspan="1" colspan="1">13-15</td><td rowspan="1" colspan="1">Reserved</td><td rowspan="1" colspan="1"></td></tr><tr><td rowspan="1" colspan="1">16</td><td rowspan="1" colspan="1">Issuer Key ID</td><td rowspan="1" colspan="1"></td></tr><tr><td rowspan="1" colspan="1">17-19</td><td rowspan="1" colspan="1">Reserved</td><td rowspan="1" colspan="1"></td></tr><tr><td rowspan="1" colspan="1">20</td><td rowspan="1" colspan="1">Notation Data</td><td rowspan="1" colspan="1"></td></tr><tr><td rowspan="1" colspan="1">21</td><td rowspan="1" colspan="1">Preferred Hash Algorithms</td><td rowspan="1" colspan="1"></td></tr><tr><td rowspan="1" colspan="1">22</td><td rowspan="1" colspan="1">Preferred Compression Algorithms</td><td rowspan="1" colspan="1"></td></tr><tr><td rowspan="1" colspan="1">23</td><td rowspan="1" colspan="1">Key Server Preferences</td><td rowspan="1" colspan="1"></td></tr><tr><td rowspan="1" colspan="1">24</td><td rowspan="1" colspan="1">Preferred Key Server</td><td rowspan="1" colspan="1"></td></tr><tr><td rowspan="1" colspan="1">25</td><td rowspan="1" colspan="1">Primary User ID</td><td rowspan="1" colspan="1"></td></tr><tr><td rowspan="1" colspan="1">26</td><td rowspan="1" colspan="1">Policy URI</td><td rowspan="1" colspan="1"></td></tr><tr><td rowspan="1" colspan="1">27</td><td rowspan="1" colspan="1">Key Flags</td><td rowspan="1" colspan="1"></td></tr><tr><td rowspan="1" colspan="1">28</td><td rowspan="1" colspan="1">Signer's User ID</td><td rowspan="1" colspan="1"></td></tr><tr><td rowspan="1" colspan="1">29</td><td rowspan="1" colspan="1">Reason for Revocation</td><td rowspan="1" colspan="1"></td></tr><tr><td rowspan="1" colspan="1">30</td><td rowspan="1" colspan="1">Features</td><td rowspan="1" colspan="1"></td></tr><tr><td rowspan="1" colspan="1">31</td><td rowspan="1" colspan="1">Signature Target</td><td rowspan="1" colspan="1"></td></tr><tr><td rowspan="1" colspan="1">32</td><td rowspan="1" colspan="1">Embedded Signature</td><td rowspan="1" colspan="1"></td></tr><tr><td rowspan="1" colspan="1">33</td><td rowspan="1" colspan="1">Issuer Fingerprint</td><td rowspan="1" colspan="1"></td></tr><tr><td rowspan="1" colspan="1">34</td><td rowspan="1" colspan="1">Reserved</td><td rowspan="1" colspan="1"></td></tr><tr><td rowspan="1" colspan="1">35</td><td rowspan="1" colspan="1">Intended Recipient Fingerprint</td><td rowspan="1" colspan="1"></td></tr><tr><td rowspan="1" colspan="1">37</td><td rowspan="1" colspan="1">Reserved (Attested Certifications)</td><td rowspan="1" colspan="1"></td></tr><tr><td rowspan="1" colspan="1">38</td><td rowspan="1" colspan="1">Reserved (Key Block)</td><td rowspan="1" colspan="1"></td></tr><tr><td rowspan="1" colspan="1">39</td><td rowspan="1" colspan="1">Preferred AEAD Ciphersuites</td><td rowspan="1" colspan="1"></td></tr><tr><td rowspan="1" colspan="1">100-110</td><td rowspan="1" colspan="1">Private or Experimental Use</td><td rowspan="1" colspan="1"></td></tr></tbody></table>

Implementations SHOULD implement the four preferred algorithm subpackets (11, 21, 22, and 39), as well as the "Features" (30) and "Reason for Revocation" (29) subpackets. To avoid surreptitious forwarding (see ), implementations SHOULD also implement the "Intended Recipients Fingerprint" (35) subpacket. Note that if an implementation chooses not to implement some of the preferences subpackets, it MUST default to the mandatory-to-implement algorithms to ensure interoperability. An encrypting implementation that does not implement the "Features" (30) subpacket SHOULD select the type of encrypted data format based on the versions of the recipient keys or external inference (see for more details).[¶](#section-5.2.3.7-12)
...
(4-octet time field) [¶](#section-5.2.3.13-1)

The validity period of the key. This is the number of seconds after the key creation time that the key expires. For a direct or certification self-signature, the key creation time is that of the primary key. For a Subkey Binding signature, the key creation time is that of the subkey. If this is not present or has a value of zero, the key never expires. This is found only on a self-signature.[¶](#section-5.2.3.13-2)

When an implementation generates this subpacket, it SHOULD be marked as critical.[¶](#section-5.2.3.13-3)

##### 5.2.3.14.

(array of 1-octet values) [¶](#section-5.2.3.14-1)

A series of Symmetric Cipher Algorithm IDs indicating how the keyholder prefers to receive the version 1 Symmetrically Encrypted and Integrity Protected Data packet (). The subpacket body is an ordered list of octets with the most preferred listed first. It is assumed that only the algorithms listed are supported by the recipient's implementation. Algorithm IDs are defined in. This is only found on a self-signature.[¶](#section-5.2.3.14-2)

When generating a v2 SEIPD packet, this preference list is not relevant. See instead.[¶](#section-5.2.3.14-3)

##### 5.2.3.15.

(array of pairs of octets indicating Symmetric Cipher and AEAD algorithms) [¶](#section-5.2.3.15-1)

A series of paired algorithm IDs indicating how the keyholder prefers to receive the version 2 Symmetrically Encrypted and Integrity Protected Data packet (). Each pair of octets indicates a combination of a symmetric cipher and an AEAD mode that the keyholder prefers to use. The Symmetric Cipher Algorithm ID precedes the AEAD algorithm ID in each pair. The subpacket body is an ordered list of pairs of octets with the most preferred algorithm combination listed first.[¶](#section-5.2.3.15-2)

It is assumed that only the combinations of algorithms listed are supported by the recipient's implementation, with the exception of the mandatory-to-implement combination of AES-128 and OCB. If AES-128 and OCB are not found in the subpacket, it is implicitly listed at the end.[¶](#section-5.2.3.15-3)

AEAD algorithm IDs are listed in. Symmetric Cipher Algorithm IDs are listed in.[¶](#section-5.2.3.15-4)

For example, a subpacket containing the six octets [¶](#section-5.2.3.15-5)
...
indicates that the keyholder prefers to receive v2 SEIPD using AES-256 with OCB, then AES-256 with GCM, then Camellia-256 with OCB, and finally the implicit AES-128 with OCB.[¶](#section-5.2.3.15-7)

Note that support for the version 2 Symmetrically Encrypted and Integrity Protected Data packet () in general is indicated by a Features Flag ().[¶](#section-5.2.3.15-8)

This subpacket is only found on a self-signature.[¶](#section-5.2.3.15-9)

When generating a v1 SEIPD packet, this preference list is not relevant. See instead.[¶](#section-5.2.3.15-10)

##### 5.2.3.16.

(array of 1-octet values) [¶](#section-5.2.3.16-1)

Message digest algorithm IDs that indicate which algorithms the keyholder prefers to receive. Like the Preferred AEAD Ciphersuites, the list is ordered. Algorithm IDs are defined in. This is only found on a self-signature.[¶](#section-5.2.3.16-2)

##### 5.2.3.17.

(array of 1-octet values) [¶](#section-5.2.3.17-1)

Compression algorithm IDs that indicate which algorithms the keyholder prefers to use. Like the Preferred AEAD Ciphersuites, the list is ordered. Algorithm IDs are defined in. A zero, or the absence of this subpacket, denotes that uncompressed data is preferred; the keyholder's implementation might have no compression support available. This is only found on a self-signature.[¶](#section-5.2.3.17-2)

##### 5.2.3.18.

(4-octet time field) [¶](#section-5.2.3.18-1)

The validity period of the signature. This is the number of seconds after the Signature Creation Time that the signature expires. If this is not present or has a value of zero, it never expires.[¶](#section-5.2.3.18-2)

When an implementation generates this subpacket, it SHOULD be marked as critical.[¶](#section-5.2.3.18-3)

##### 5.2.3.19.

(1 octet of exportability, 0 for not, 1 for exportable) [¶](#section-5.2.3.19-1)

This subpacket denotes whether a certification signature is "exportable"; it is intended for use by users other than the signature's issuer. The packet body contains a Boolean flag indicating whether the signature is exportable. If this packet is not present, the certification is exportable; it is equivalent to a flag containing a 1.[¶](#section-5.2.3.19-2)

Non-exportable, or "local", certifications are signatures made by a user to mark a key as valid within that user's implementation only.[¶](#section-5.2.3.19-3)

Thus, when an implementation prepares a user's copy of a key for transport to another user (this is the process of "exporting" the key), any local certification signatures are deleted from the key.[¶](#section-5.2.3.19-4)
...
Some implementations do not represent the interest of a single user (for example, a key server). Such implementations always trim local certifications from any key they handle.[¶](#section-5.2.3.19-6)

When an implementation generates this subpacket and denotes the signature as non-exportable, the subpacket MUST be marked as critical.[¶](#section-5.2.3.19-7)

##### 5.2.3.20.

(1 octet of revocability, 0 for not, 1 for revocable) [¶](#section-5.2.3.20-1)

A Signature's revocability status. The packet body contains a Boolean flag indicating whether the signature is revocable. Signatures that are not revocable ignore any later Revocation Signatures. They represent the signer's commitment that its signature cannot be revoked for the life of its key. If this packet is not present, the signature is revocable.[¶](#section-5.2.3.20-2)

##### 5.2.3.21.

(1 octet "level" (depth), 1 octet of trust amount) [¶](#section-5.2.3.21-1)

The signer asserts that the key is not only valid but also trustworthy at the specified level. Level 0 has the same meaning as an ordinary validity signature. Level 1 means that the signed key is asserted to be a valid trusted introducer, with the 2nd octet of the body specifying the degree of trust. Level 2 means that the signed key is asserted to be trusted to issue level 1 Trust Signatures; that is, the signed key is a "meta introducer". Generally, a level n Trust Signature asserts that a key is trusted to issue level n-1 Trust Signatures. The trust amount is in a range from 0-255, interpreted such that values less than 120 indicate partial trust and values of 120 or greater indicate complete trust. Implementations SHOULD emit values of 60 for partial trust and 120 for complete trust.[¶](#section-5.2.3.21-2)

##### 5.2.3.22.

(null-terminated UTF-8 encoded Regular Expression) [¶](#section-5.2.3.22-1)

Used in conjunction with Trust Signature packets (of level > 0) to limit the scope of trust that is extended. Only signatures by the target key on User IDs that match the Regular Expression in the body of this packet have trust extended by the Trust Signature subpacket. The Regular Expression uses the same syntax as Henry Spencer's "almost public domain" Regular Expression \[\] package. A description of the syntax is found in. The Regular Expression matches (or does not match) a sequence of UTF-8-encoded Unicode characters from User IDs. The expression itself is also written with UTF-8 characters.[¶](#section-5.2.3.22-2)

For historical reasons, this subpacket includes a null character (an octet with value zero) after the Regular Expression. When an implementation parses a Regular Expression subpacket, it MUST remove this octet; if it is not present, it MUST reject the subpacket (i.e., ignore the subpacket if it's non-critical and reject the signature if it's critical). When an implementation generates a Regular Expression subpacket, it MUST include the null terminator.[¶](#section-5.2.3.22-3)

When generating this subpacket, it SHOULD be marked as critical.[¶](#section-5.2.3.22-4)

##### 5.2.3.23.

(1 octet of class, 1 octet of public key algorithm ID, 20 octets of version 4 fingerprint) [¶](#section-5.2.3.23-1)

This mechanism is deprecated. Applications MUST NOT generate such a subpacket.[¶](#section-5.2.3.23-2)

An application that wants the functionality of delegating revocation can use an escrowed Revocation Signature. See for more details.[¶](#section-5.2.3.23-3)

The remainder of this section describes how some implementations attempt to interpret this deprecated subpacket.[¶](#section-5.2.3.23-4)

This packet was intended to authorize the specified key to issue Revocation Signatures for this key. The class octet must have bit 0x80 set. If bit 0x40 is set, it means the revocation information is sensitive. Other bits are for future expansion to other kinds of authorizations. This is only found on a Direct Key self-signature (Type ID 0x1F). The use on other types of self-signatures is unspecified.[¶](#section-5.2.3.23-5)

If the "sensitive" flag is set, the keyholder feels this subpacket contains private trust information that describes a real-world sensitive relationship. If this flag is set, implementations SHOULD NOT export this signature to other users except in cases where the data needs to be available, i.e., when the signature is being sent to the designated revoker or when it is accompanied by a Revocation Signature from that revoker. Note that it may be appropriate to isolate this subpacket within a separate signature so that it is not combined with other subpackets that need to be exported.[¶](#section-5.2.3.23-6)

##### 5.2.3.24.

(4 octets of flags, 2 octets of name length (M), 2 octets of value length (N), M octets of name data, N octets of value data) [¶](#section-5.2.3.24-1)

This subpacket describes a "notation" on the signature that the issuer wishes to make. The notation has a name and a value, each of which are strings of octets. There may be more than one notation in a signature. Notations can be used for any extension the issuer of the signature cares to make. The "flags" field holds 4 octets of flags.[¶](#section-5.2.3.24-2)

All undefined flags MUST be zero. Defined flags are as follows:[¶](#section-5.2.3.24-3)

<table><caption>:</caption><thead><tr><th rowspan="1" colspan="1">Flag Position</th><th rowspan="1" colspan="1">Shorthand</th><th rowspan="1" colspan="1">Description</th></tr></thead><tbody><tr><td rowspan="1" colspan="1">0x80000000 (first bit of the first octet)</td><td rowspan="1" colspan="1">human-readable</td><td rowspan="1" colspan="1">Notation value is UTF-8 text</td></tr></tbody></table>

Notation names are arbitrary strings encoded in UTF-8. They reside in two namespaces: the IETF namespace and the user namespace.[¶](#section-5.2.3.24-5)

The IETF namespace is registered with IANA. These names MUST NOT contain the "@" character (0x40). This is a tag for the user namespace.[¶](#section-5.2.3.24-6)

<table><caption>:</caption><thead><tr><th rowspan="1" colspan="1">Notation Name</th><th rowspan="1" colspan="1">Data Type</th><th rowspan="1" colspan="1">Allowed Values</th></tr></thead><tbody><tr><th rowspan="1" colspan="3">No registrations at this time.</th></tr></tbody></table>

This registry is initially empty.[¶](#section-5.2.3.24-8)

Names in the user namespace consist of a UTF-8 string tag followed by "@", followed by a DNS domain name. Note that the tag MUST NOT contain an "@" character. For example, the "sample" tag used by Example Corporation could be "sample@example.com".[¶](#section-5.2.3.24-9)

Names in a user space are owned and controlled by the owners of that domain. Obviously, it's bad form to create a new name in a DNS space that you don't own.[¶](#section-5.2.3.24-10)

Since the user namespace is in the form of an email address, implementers MAY wish to arrange for that address to reach a person who can be consulted about the use of the named tag. Note that due to UTF-8 encoding, not all valid user space name tags are valid email addresses.[¶](#section-5.2.3.24-11)

If there is a critical notation, the criticality applies to that specific notation and not to notations in general.[¶](#section-5.2.3.24-12)

##### 5.2.3.25.

(N octets of flags) [¶](#section-5.2.3.25-1)

This is a list of 1-bit flags that indicates preferences that the keyholder has about how the key is handled on a key server. All undefined flags MUST be zero.[¶](#section-5.2.3.25-2)

<table><caption>:</caption><thead><tr><th rowspan="1" colspan="1">Flag</th><th rowspan="1" colspan="1">Shorthand</th><th rowspan="1" colspan="1">Definition</th></tr></thead><tbody><tr><td rowspan="1" colspan="1">0x80...</td><td rowspan="1" colspan="1">No-modify</td><td rowspan="1" colspan="1">The keyholder requests that this key only be modified or updated by the keyholder or an administrator of the key server.</td></tr></tbody></table>

This is found only on a self-signature.[¶](#section-5.2.3.25-4)

##### 5.2.3.26.

(String) [¶](#section-5.2.3.26-1)

This is a URI of a key server that the keyholder prefers be used for updates. Note that keys with multiple User IDs can have a Preferred Key Server for each User ID. Note also that since this is a URI, the key server can actually be a copy of the key retrieved by https, ftp, http, etc.[¶](#section-5.2.3.26-2)

##### 5.2.3.27.

(1 octet, Boolean) [¶](#section-5.2.3.27-1)

This is a flag in a User ID's self-signature that states whether this User ID is the main User ID for this key. It is reasonable for an implementation to resolve ambiguities in preferences, for example, by referring to the Primary User ID. If this flag is absent, its value is zero. If more than one User ID in a key is marked as primary, the implementation may resolve the ambiguity in any way it sees fit, but it is RECOMMENDED that priority be given to the User ID with the most recent self-signature.[¶](#section-5.2.3.27-2)

When appearing on a self-signature on a User ID packet, this subpacket applies only to User ID packets. When appearing on a self-signature on a User Attribute packet, this subpacket applies only to User Attribute packets. That is, there are two different and independent "primaries" -- one for User IDs and one for User Attributes.[¶](#section-5.2.3.27-3)

##### 5.2.3.28.

(String) [¶](#section-5.2.3.28-1)

This subpacket contains a URI of a document that describes the policy under which the signature was issued.[¶](#section-5.2.3.28-2)

##### 5.2.3.29.

(N octets of flags) [¶](#section-5.2.3.29-1)

This subpacket contains a list of binary flags that hold information about a key. It is a string of octets, and an implementation MUST NOT assume a fixed size, so that it can grow over time. If a list is shorter than an implementation expects, the unstated flags are considered to be zero. The defined flags are as follows:[¶](#section-5.2.3.29-2)

<table><caption>:</caption><thead><tr><th rowspan="1" colspan="1">Flag</th><th rowspan="1" colspan="1">Definition</th></tr></thead><tbody><tr><td rowspan="1" colspan="1">0x01...</td><td rowspan="1" colspan="1">This key may be used to make User ID certifications (Signature Type IDs 0x10-0x13) or Direct Key signatures (Signature Type ID 0x1F) over other keys.</td></tr><tr><td rowspan="1" colspan="1">0x02...</td><td rowspan="1" colspan="1">This key may be used to sign data.</td></tr><tr><td rowspan="1" colspan="1">0x04...</td><td rowspan="1" colspan="1">This key may be used to encrypt communications.</td></tr><tr><td rowspan="1" colspan="1">0x08...</td><td rowspan="1" colspan="1">This key may be used to encrypt storage.</td></tr><tr><td rowspan="1" colspan="1">0x10...</td><td rowspan="1" colspan="1">The private component of this key may have been split by a secret-sharing mechanism.</td></tr><tr><td rowspan="1" colspan="1">0x20...</td><td rowspan="1" colspan="1">This key may be used for authentication.</td></tr><tr><td rowspan="1" colspan="1">0x80...</td><td rowspan="1" colspan="1">The private component of this key may be in the possession of more than one person.</td></tr><tr><td rowspan="1" colspan="1">0x0004...</td><td rowspan="1" colspan="1">Reserved (ADSK)</td></tr><tr><td rowspan="1" colspan="1">0x0008...</td><td rowspan="1" colspan="1">Reserved (timestamping)</td></tr></tbody></table>

Usage notes:[¶](#section-5.2.3.29-4)

The flags in this packet may appear in self-signatures or in certification signatures. They mean different things depending on who is making the statement. For example, a certification signature that has the "sign data" flag is stating that the certification is for that use. On the other hand, the "communications encryption" flag in a self-signature is stating a preference that a given key be used for communications. However, note that determining what is "communications" and what is "storage" is a thorny issue. This decision is left wholly up to the implementation; the authors of this document do not claim any special wisdom on the issue and realize that accepted opinion may change.[¶](#section-5.2.3.29-5)

The "split key" (0x10) and "group key" (0x80) flags are placed on a self-signature only; they are meaningless on a certification signature. They SHOULD be placed only on a Direct Key signature (Type ID 0x1F) or a Subkey Binding signature (Type ID 0x18), one that refers to the key the flag applies to.[¶](#section-5.2.3.29-6)

When an implementation generates this subpacket, it SHOULD be marked as critical.[¶](#section-5.2.3.29-7)

##### 5.2.3.30.

(String) [¶](#section-5.2.3.30-1)

This subpacket allows a keyholder to state which User ID is responsible for the signing. Many keyholders use a single key for different purposes, such as business communications as well as personal communications. This subpacket allows such a keyholder to state which of their roles is making a signature.[¶](#section-5.2.3.30-2)

This subpacket is not appropriate to use to refer to a User Attribute packet.[¶](#section-5.2.3.30-3)

##### 5.2.3.31.

(1 octet of revocation code, N octets of reason string) [¶](#section-5.2.3.31-1)

This subpacket is used only in Key Revocation and Certification Revocation signatures. It describes the reason why the key or certification was revoked.[¶](#section-5.2.3.31-2)

The first octet contains a machine-readable code that denotes the reason for the revocation:[¶](#section-5.2.3.31-3)

<table><caption>:</caption><thead><tr><th rowspan="1" colspan="1">Code</th><th rowspan="1" colspan="1">Reason</th></tr></thead><tbody><tr><td rowspan="1" colspan="1">0</td><td rowspan="1" colspan="1">No reason specified (Key Revocation or Certification Revocation signatures)</td></tr><tr><td rowspan="1" colspan="1">1</td><td rowspan="1" colspan="1">Key is superseded (Key Revocation signatures)</td></tr><tr><td rowspan="1" colspan="1">2</td><td rowspan="1" colspan="1">Key material has been compromised (Key Revocation signatures)</td></tr><tr><td rowspan="1" colspan="1">3</td><td rowspan="1" colspan="1">Key is retired and no longer used (Key Revocation signatures)</td></tr><tr><td rowspan="1" colspan="1">32</td><td rowspan="1" colspan="1">User ID information is no longer valid (Certification Revocation signatures)</td></tr><tr><td rowspan="1" colspan="1">100-110</td><td rowspan="1" colspan="1">Private Use</td></tr></tbody></table>

Following the revocation code is a string of octets that gives information about the Reason for Revocation in human-readable form (UTF-8). The string may be null (of zero length). The length of the subpacket is the length of the reason string plus one. An implementation SHOULD implement this subpacket, include it in all Revocation Signatures, and interpret revocations appropriately. There are important semantic differences between the reasons, and there are thus important reasons for revoking signatures.[¶](#section-5.2.3.31-5)

If a key has been revoked because of a compromise, all signatures created by that key are suspect. However, if it was merely superseded or retired, old signatures are still valid. If the revoked signature is the self-signature for certifying a User ID, a revocation denotes that that user name is no longer in use. Such a signature revocation SHOULD include a Reason for Revocation subpacket containing code 32.[¶](#section-5.2.3.31-6)

Note that any certification may be revoked, including a certification on some other person's key. There are many good reasons for revoking a certification signature, such as the case where the keyholder leaves the employ of a business with an email address. A revoked certification is no longer a part of validity calculations.[¶](#section-5.2.3.31-7)
...
An implementation SHOULD NOT use a feature listed when sending to a user who does not state that they can use it, unless the implementation can infer support for the feature from another implementation-dependent mechanism.[¶](#section-5.2.3.32-4)

Defined features are as follows:[¶](#section-5.2.3.32-5)

First octet:[¶](#section-5.2.3.32-6)

<table><caption>:</caption><thead><tr><th rowspan="1" colspan="1">Feature</th><th rowspan="1" colspan="1">Definition</th><th rowspan="1" colspan="1">Reference</th></tr></thead><tbody><tr><td rowspan="1" colspan="1">0x01...</td><td rowspan="1" colspan="1">Version 1 Symmetrically Encrypted and Integrity Protected Data packet</td><td rowspan="1" colspan="1"></td></tr><tr><td rowspan="1" colspan="1">0x02...</td><td rowspan="1" colspan="1">Reserved</td><td rowspan="1" colspan="1"></td></tr><tr><td rowspan="1" colspan="1">0x04...</td><td rowspan="1" colspan="1">Reserved</td><td rowspan="1" colspan="1"></td></tr><tr><td rowspan="1" colspan="1">0x08...</td><td rowspan="1" colspan="1">Version 2 Symmetrically Encrypted and Integrity Protected Data packet</td><td rowspan="1" colspan="1"></td></tr></tbody></table>

If an implementation implements any of the defined features, it SHOULD implement the Features subpacket, too.[¶](#section-5.2.3.32-8)

See for details about how to use the Features subpacket when generating encryption data.[¶](#section-5.2.3.32-9)

##### 5.2.3.33.

(1 octet public key algorithm, 1 octet hash algorithm, N octets hash) [¶](#section-5.2.3.33-1)

This subpacket identifies a specific target signature to which a signature refers. For Revocation Signatures, this subpacket provides explicit designation of which signature is being revoked. For a Third-Party Confirmation or Timestamp signature, this designates what signature is signed. All arguments are an identifier of that target signature.[¶](#section-5.2.3.33-2)

The N octets of hash data MUST be the size of the signature's hash. For example, a target signature with a SHA-1 hash MUST have 20 octets of hash data.[¶](#section-5.2.3.33-3)

##### 5.2.3.34.

(1 Signature packet body) [¶](#section-5.2.3.34-1)

This subpacket contains a complete Signature packet body as specified in. It is useful when one signature needs to refer to, or be incorporated in, another signature.[¶](#section-5.2.3.34-2)
...
An implementation SHOULD generate this subpacket when creating a signed and encrypted message.[¶](#section-5.2.3.36-4)

When generating this subpacket in a version 6 signature, it SHOULD be marked as critical.[¶](#section-5.2.3.36-5)

#### 5.2.4.

All signatures are formed by producing a hash over the signature data and then using the resulting hash in the signature algorithm.[¶](#section-5.2.4-1)

When creating or verifying a version 6 signature, the salt is fed into the hash context before any other data.[¶](#section-5.2.4-2)

For binary document signatures (Type ID 0x00), the document data is hashed directly. For text document signatures (Type ID 0x01), the implementation MUST first canonicalize the document by converting line endings to <CR><LF> and encoding it in UTF-8 (see \[\]). The resulting UTF-8 byte stream is hashed.[¶](#section-5.2.4-3)

When a version 4 signature is made over a key, the hash data starts with the octet 0x99, followed by a 2-octet length of the key, followed by the body of the key packet. When a version 6 signature is made over a key, the hash data starts with the salt and then octet 0x9B, followed by a 4-octet length of the key, followed by the body of the key packet.[¶](#section-5.2.4-4)

A Subkey Binding signature (Type ID 0x18) or Primary Key Binding signature (Type ID 0x19) then hashes the subkey using the same format as the main key (also using 0x99 or 0x9B as the first octet). Primary Key Revocation signatures (Type ID 0x20) hash only the key being revoked. A Subkey Revocation signature (Type ID 0x28) first hashes the primary key and then the subkey being revoked.[¶](#section-5.2.4-5)

A Certification signature (Type IDs 0x10 through 0x13) hashes the User ID that is bound to the key into the hash context after the above data. A version 3 certification hashes the contents of the User ID or User Attribute packet without the packet header. A version 4 or version 6 certification hashes the constant 0xB4 for User ID certifications or the constant 0xD1 for User Attribute certifications, followed by a 4-octet number giving the length of the User ID or User Attribute data, followed by the User ID or User Attribute data.[¶](#section-5.2.4-6)
...
Once the data body is hashed, then a trailer is hashed. This trailer depends on the version of the signature.[¶](#section-5.2.4-8)

- A version 3 signature hashes five octets of the packet body, starting from the signature type field. This data is the signature type, followed by the 4-octet Signature Creation Time.[¶](#section-5.2.4-9.1.1)
- A version 4 or version 6 signature hashes the packet body starting from its first field, the version number, through the end of the hashed subpacket data and a final extra trailer. Thus, the hashed fields are:[¶](#section-5.2.4-9.2.1)
	- an octet indicating the signature version (0x04 for version 4, and 0x06 for version 6),[¶](#section-5.2.4-9.2.2.1.1)
		- the signature type,[¶](#section-5.2.4-9.2.2.2.1)
		- the public key algorithm,[¶](#section-5.2.4-9.2.2.3.1)
		- the hash algorithm,[¶](#section-5.2.4-9.2.2.4.1)
		- the hashed subpacket length,[¶](#section-5.2.4-9.2.2.5.1)
		- the hashed subpacket body,[¶](#section-5.2.4-9.2.2.6.1)
		- a second version octet (0x04 for version 4, and 0x06 for version 6),[¶](#section-5.2.4-9.2.2.7.1)
		- a single octet 0xFF, and [¶](#section-5.2.4-9.2.2.8.1)
		- a number representing the length (in octets) of the hashed data from the Signature packet through the hashed subpacket body. This a 4-octet big-endian unsigned integer of the length modulo 2 <sup>32</sup>.[¶](#section-5.2.4-9.2.2.9.1)

After all this has been hashed in a single hash context, the resulting hash field is used in the signature algorithm, and its first two octets are placed in the Signature packet, as described in.[¶](#section-5.2.4-10)

For worked examples of the data hashed during a signature, see.[¶](#section-5.2.4-11)
...
In some cases, a Signature packet (or its corresponding One-Pass Signature packet; see ) may be malformed or unknown. For example, it might encounter any of the following problems (this is not an exhaustive list):[¶](#section-5.2.5-1)

- An unknown signature type [¶](#section-5.2.5-2.1.1)
- An unknown signature version [¶](#section-5.2.5-2.2.1)
- An unsupported signature version [¶](#section-5.2.5-2.3.1)
- An unknown "critical" subpacket (see ) in the hashed area [¶](#section-5.2.5-2.4.1)
- A subpacket with a length that diverges from the expected length [¶](#section-5.2.5-2.5.1)
- A hashed subpacket area with length that exceeds the length of the Signature packet itself [¶](#section-5.2.5-2.6.1)
- A hash algorithm known to be weak (e.g., MD5) [¶](#section-5.2.5-2.7.1)
- A mismatch between the expected salt length of the hash algorithm and the actual salt length [¶](#section-5.2.5-2.8.1)
- A mismatch between the One-Pass Signature version and the Signature version (see ) [¶](#section-5.2.5-2.9.1)
- A signature with a version other than 6, made by a version 6 key [¶](#section-5.2.5-2.10.1)

When an implementation encounters such a malformed or unknown signature, it MUST ignore the signature for validation purposes. It MUST NOT indicate a successful signature validation for such a signature. At the same time, it MUST NOT halt processing on the packet stream or reject other signatures in the same packet stream just because an unknown or invalid signature exists.[¶](#section-5.2.5-3)

This requirement is necessary for forward compatibility. Producing an output that indicates that no successful signatures were found is preferable to aborting processing entirely.[¶](#section-5.2.5-4)

### 5.3.

The Symmetric Key Encrypted Session Key (SKESK) packet holds the symmetric key encryption of a session key used to encrypt a message. Zero or more Public Key Encrypted Session Key packets () and/or Symmetric Key Encrypted Session Key packets precede an encryption container (that is, a Symmetrically Encrypted and Integrity Protected Data packet or -- for historic data -- a Symmetrically Encrypted Data packet) that holds an Encrypted Message. The message is encrypted with a session key, and the session key is itself encrypted and stored in the Encrypted Session Key packet(s).[¶](#section-5.3-1)

If the encryption container is preceded by one or more Symmetric Key Encrypted Session Key packets, each specifies a passphrase that may be used to decrypt the message. This allows a message to be encrypted to a number of public keys, and also to one or more passphrases.[¶](#section-5.3-2)

The body of this packet starts with a 1-octet number giving the version number of the packet type. The currently defined versions are 4 and 6. The remainder of the packet depends on the version.[¶](#section-5.3-3)

The versions differ in how they encrypt the session key with the passphrase and in what they encode. The version of the SKESK packet must align with the version of the SEIPD packet (see ). Any new version of the SKESK packet should be registered in the registry established in.[¶](#section-5.3-4)

#### 5.3.1.

A v4 SKESK packet precedes a v1 SEIPD (see ). In historic data, it is sometimes found preceding a deprecated SED packet (see ). A v4 SKESK packet MUST NOT precede a v2 SEIPD packet (see ).[¶](#section-5.3.1-1)

A version 4 Symmetric Key Encrypted Session Key packet consists of:[¶](#section-5.3.1-2)

- A 1-octet version number with value 4.[¶](#section-5.3.1-3.1.1)
- A 1-octet number describing the symmetric algorithm used.[¶](#section-5.3.1-3.2.1)
- An S2K Specifier. The length of the S2K Specifier depends on its type (see ).[¶](#section-5.3.1-3.3.1)
- Optionally, the encrypted session key itself, which is decrypted with the S2K object.[¶](#section-5.3.1-3.4.1)

If the encrypted session key is not present (which can be detected on the basis of packet length and S2K Specifier size), then the S2K algorithm applied to the passphrase produces the session key for decrypting the message, using the Symmetric Cipher Algorithm ID from the Symmetric Key Encrypted Session Key packet.[¶](#section-5.3.1-4)
...
A v6 SKESK packet precedes a v2 SEIPD packet (see ). A v6 SKESK packet MUST NOT precede a v1 SEIPD packet or a deprecated Symmetrically Encrypted Data packet (see ).[¶](#section-5.3.2-1)

A version 6 Symmetric Key Encrypted Session Key packet consists of:[¶](#section-5.3.2-2)

- A 1-octet version number with value 6.[¶](#section-5.3.2-3.1.1)
- A 1-octet scalar octet count for the 5 fields following this octet.[¶](#section-5.3.2-3.2.1)
- A 1-octet Symmetric Cipher Algorithm ID (from ).[¶](#section-5.3.2-3.3.1)
- A 1-octet AEAD algorithm identifier (from ).[¶](#section-5.3.2-3.4.1)
- A 1-octet scalar octet count of the following field.[¶](#section-5.3.2-3.5.1)
- An S2K Specifier. The length of the S2K Specifier depends on its type (see ).[¶](#section-5.3.2-3.6.1)
- A starting IV of the size specified by the AEAD algorithm.[¶](#section-5.3.2-3.7.1)
...
The One-Pass Signature packet precedes the signed data and contains enough information to allow the receiver to begin calculating any hashes needed to verify the signature. It allows the Signature packet to be placed at the end of the message so that the signer can compute the entire signed message in one pass.[¶](#section-5.4-1)

The body of this packet consists of:[¶](#section-5.4-2)

- A 1-octet version number. The currently defined versions are 3 and 6. Any new One-Pass Signature packet version should be registered in the registry established in.[¶](#section-5.4-3.1.1)
- A 1-octet Signature Type ID. Signature types are described in.[¶](#section-5.4-3.2.1)
- A 1-octet number describing the hash algorithm used.[¶](#section-5.4-3.3.1)
- A 1-octet number describing the public key algorithm used.[¶](#section-5.4-3.4.1)
- Only for version 6 packets, a variable-length field containing:[¶](#section-5.4-3.5.1)
	- A 1-octet salt size. The value MUST match the value defined for the hash algorithm as specified in.[¶](#section-5.4-3.5.2.1.1)
		- The salt; a random value of the specified size. The value MUST match the salt field of the corresponding Signature packet.[¶](#section-5.4-3.5.2.2.1)
- Only for v3 packets, an 8-octet number holding the Key ID of the signing key.[¶](#section-5.4-3.6.1)
- Only for version 6 packets, 32 octets of the fingerprint of the signing key. Since a version 6 signature can only be made by a version 6 key, the length of the fingerprint is fixed.[¶](#section-5.4-3.7.1)
- A 1-octet number holding a flag showing whether the signature is nested. A zero value indicates that the next packet is another One-Pass Signature packet that describes another signature to be applied to the same message data.[¶](#section-5.4-3.8.1)

When generating a one-pass signature, the OPS packet version MUST correspond to the version of the associated Signature packet, except for the historical accident that version 4 keys use a version 3 One-Pass Signature packet (there is no version 4 OPS). See for the full correspondence of versions between Keys, Signatures, and One-Pass Signatures.[¶](#section-5.4-4)

Note that if a message contains more than one one-pass signature, then the Signature packets bracket the message; that is, the first Signature packet after the message corresponds to the last One-Pass Signature packet and the final Signature packet corresponds to the first One-Pass Signature packet.[¶](#section-5.4-5)

### 5.5.

A key material packet contains all the information about a public or private key. There are four variants of this packet type: two major versions (versions 4 and 6) and two strongly deprecated versions (versions 2 and 3). Consequently, this section is complex.[¶](#section-5.5-1)

For historical reasons, versions 1 and 5 of the key packets are unspecified.[¶](#section-5.5-2)
...
A Public Subkey packet (Type ID 14) has exactly the same format as a Public Key packet, but it denotes a subkey. One or more subkeys may be associated with a top-level key. By convention, the top-level key offers certification capability, but it does not provide encryption services, while a dedicated subkey provides encryption (see ).[¶](#section-5.5.1.2-1)

##### 5.5.1.3.

A Secret Key packet contains all the information that is found in a Public Key packet, including the public key material, but it also includes the secret key material after all the public key fields.[¶](#section-5.5.1.3-1)

##### 5.5.1.4.

A Secret Subkey packet (Type ID 7) is the subkey analog of the Secret Key packet and has exactly the same format.[¶](#section-5.5.1.4-1)
...
Version 2 keys are identical to version 3 keys except for the version number. A version 3 Public Key or Public Subkey packet contains:[¶](#section-5.5.2.1-1)

- A 1-octet version number (3).[¶](#section-5.5.2.1-2.1.1)
- A 4-octet number denoting the time that the key was created.[¶](#section-5.5.2.1-2.2.1)
- A 2-octet number denoting the time in days that the key is valid. If this number is zero, then it does not expire.[¶](#section-5.5.2.1-2.3.1)
- A 1-octet number denoting the public key algorithm of the key.[¶](#section-5.5.2.1-2.4.1)
- A series of multiprecision integers comprising the key material:[¶](#section-5.5.2.1-2.5.1)
	- MPI of RSA public modulus n.[¶](#section-5.5.2.1-2.5.2.1.1)
		- MPI of RSA public encryption exponent e.[¶](#section-5.5.2.1-2.5.2.2.1)

Version 3 keys are deprecated. They contain three weaknesses. First, it is relatively easy to construct a version 3 key that has the same Key ID as any other key because the Key ID is simply the low 64 bits of the public modulus. Second, because the fingerprint of a version 3 key hashes the key material, but not its length, there is an increased opportunity for fingerprint collisions. Third, there are weaknesses in the MD5 hash algorithm that cause developers to prefer other algorithms. See for a fuller discussion of Key IDs and fingerprints.[¶](#section-5.5.2.1-3)

##### 5.5.2.2.

The version 4 format is similar to the version 3 format except for the absence of a validity period. This has been moved to the Signature packet. In addition, fingerprints of version 4 keys are calculated differently from version 3 keys, as described in.[¶](#section-5.5.2.2-1)

A version 4 packet contains:[¶](#section-5.5.2.2-2)

- A 1-octet version number (4).[¶](#section-5.5.2.2-3.1.1)
- A 4-octet number denoting the time that the key was created.[¶](#section-5.5.2.2-3.2.1)
- A 1-octet number denoting the public key algorithm of the key.[¶](#section-5.5.2.2-3.3.1)
- A series of values comprising the key material. This is algorithm specific and described in.[¶](#section-5.5.2.2-3.4.1)

##### 5.5.2.3.

The version 6 format is similar to the version 4 format except for the addition of a count for the key material. This count helps parsing Secret Key packets (which are an extension of the Public Key packet format) in the case of an unknown algorithm. In addition, fingerprints of version 6 keys are calculated differently from version 4 keys, as described in.[¶](#section-5.5.2.3-1)

A version 6 packet contains:[¶](#section-5.5.2.3-2)

- A 1-octet version number (6).[¶](#section-5.5.2.3-3.1.1)
- A 4-octet number denoting the time that the key was created.[¶](#section-5.5.2.3-3.2.1)
- A 1-octet number denoting the public key algorithm of the key.[¶](#section-5.5.2.3-3.3.1)
- A 4-octet scalar octet count for the public key material specified in the next field.[¶](#section-5.5.2.3-3.4.1)
- A series of values comprising the public key material. This is algorithm specific and described in.[¶](#section-5.5.2.3-3.5.1)

#### 5.5.3.

The Secret Key and Secret Subkey packets contain all the data of the Public Key and Public Subkey packets, with additional algorithm-specific secret key data appended, usually in encrypted form.[¶](#section-5.5.3-1)

The packet contains:[¶](#section-5.5.3-2)

- The fields of a Public Key or Public Subkey packet, as described above.[¶](#section-5.5.3-3.1.1)
...
- Only for a version 6 packet where the secret key material is encrypted (that is, where the previous octet is not zero), a 1-octet scalar octet count of the cumulative length of all the following conditionally included S2K parameter fields.[¶](#section-5.5.3-3.3.1)
- Conditionally included S2K parameter fields:[¶](#section-5.5.3-3.4.1)
	- If the S2K usage octet was 253, 254, or 255, a 1-octet symmetric key encryption algorithm.[¶](#section-5.5.3-3.4.2.1.1)
		- If the S2K usage octet was 253 (AEAD), a 1-octet AEAD algorithm.[¶](#section-5.5.3-3.4.2.2.1)
		- Only for a version 6 packet, and if the S2K usage octet was 253 or 254, a 1-octet count of the size of the one field following this octet.[¶](#section-5.5.3-3.4.2.3.1)
		- If the S2K usage octet was 253, 254, or 255, an S2K Specifier. The length of the S2K Specifier depends on its type (see ).[¶](#section-5.5.3-3.4.2.4.1)
		- If the S2K usage octet was 253 (AEAD), an IV of a size specified by the AEAD algorithm (see ), which is used as the nonce for the AEAD algorithm.[¶](#section-5.5.3-3.4.2.5.1)
		- If the S2K usage octet was 254, 255, or a cipher algorithm ID (that is, the secret data uses some form of CFB encryption), an IV of the same length as the cipher's block size.[¶](#section-5.5.3-3.4.2.6.1)
- Plain or encrypted multiprecision integers comprising the secret key data. This is algorithm specific and described in. If the S2K usage octet is 253 (AEAD), then an AEAD authentication tag is at the end of that data. If the S2K usage octet is 254 (CFB), a 20-octet SHA-1 hash of the plaintext of the algorithm-specific portion is appended to plaintext and encrypted with it. If the S2K usage octet is 255 (MalleableCFB) or another non-zero value (that is, a symmetric key encryption algorithm identifier), a 2-octet checksum of the plaintext of the algorithm-specific portion (sum of all octets, mod 65536) is appended to plaintext and encrypted with it. (This is deprecated and SHOULD NOT be used; see below.) [¶](#section-5.5.3-3.5.1)
- Only for a version 3 or 4 packet where the S2K usage octet is zero, a 2-octet checksum of the algorithm-specific portion (sum of all octets, mod 65536).[¶](#section-5.5.3-3.6.1)

The details about storing algorithm-specific secrets above are summarized in.[¶](#section-5.5.3-4)

Note that the version 6 packet format adds two count values to help parsing packets with unknown S2K or public key algorithms.[¶](#section-5.5.3-5)

Secret MPI values can be encrypted using a passphrase. If an S2K Specifier is given, it describes the algorithm for converting the passphrase to a key; otherwise, a simple MD5 hash of the passphrase is used. An implementation producing a passphrase-protected Secret Key packet MUST use an S2K Specifier; the simple hash is for read-only backward compatibility, though implementations MAY continue to use existing private keys in the old format. The cipher for encrypting the MPIs is specified in the Secret Key packet.[¶](#section-5.5.3-6)

Encryption/decryption of the secret data is done using the key created from the passphrase and the IV from the packet. If the S2K usage octet is not 253, CFB mode is used. A different mode is used with version 3 keys (which are only RSA) than with other key formats. With version 3 keys, the MPI bit count prefix (that is, the first two octets) is not encrypted. Only the MPI non-prefix data is encrypted. Furthermore, the CFB state is resynchronized at the beginning of each new MPI value so that the CFB block boundary is aligned with the start of the MPI data.[¶](#section-5.5.3-7)
...
For a version 3 key, the 8-octet Key ID consists of the low 64 bits of the public modulus of the RSA key.[¶](#section-5.5.4.1-1)

The fingerprint of a version 3 key is formed by hashing the body (but not the 2-octet length) of the MPIs that form the key material (public modulus n, followed by exponent e) with MD5. Note that both version 3 keys and MD5 are deprecated.[¶](#section-5.5.4.1-2)

##### 5.5.4.2.

A version 4 fingerprint is the 160-bit SHA-1 hash of the octet 0x99, followed by the 2-octet packet length, followed by the entire Public Key packet starting with the version field. The Key ID is the low-order 64 bits of the fingerprint. Here are the fields of the hash material, including an example of an Ed25519 key:[¶](#section-5.5.4.2-1)

a.1)

0x99 (1 octet) [¶](#section-5.5.4.2-2.1)

a.2)

2-octet, big-endian scalar octet count of (b)-(e) [¶](#section-5.5.4.2-2.2)

b)

version number = 4 (1 octet) [¶](#section-5.5.4.2-3.1)

c)

timestamp of key creation (4 octets) [¶](#section-5.5.4.2-3.2)

d)

algorithm (1 octet): 27 = Ed25519 (example) [¶](#section-5.5.4.2-3.3)

e)

algorithm-specific fields [¶](#section-5.5.4.2-3.4)

Algorithm-specific fields for Ed25519 keys (example):[¶](#section-5.5.4.2-4)

e.1)

32 octets representing the public key [¶](#section-5.5.4.2-5.1)

##### 5.5.4.3.

A version 6 fingerprint is the 256-bit SHA2-256 hash of the octet 0x9B, followed by the 4-octet packet length, followed by the entire Public Key packet starting with the version field. The Key ID is the high-order 64 bits of the fingerprint. Here are the fields of the hash material, including an example of an Ed25519 key:[¶](#section-5.5.4.3-1)

a.1)

0x9B (1 octet) [¶](#section-5.5.4.3-2.1)

a.2)

4-octet scalar octet count of (b)-(f) [¶](#section-5.5.4.3-2.2)

b)

version number = 6 (1 octet) [¶](#section-5.5.4.3-3.1)

c)

timestamp of key creation (4 octets) [¶](#section-5.5.4.3-3.2)

d)

algorithm (1 octet): 27 = Ed25519 (example) [¶](#section-5.5.4.3-3.3)

e)

4-octet scalar octet count for the key material specified in the next field [¶](#section-5.5.4.3-3.4)

f)

algorithm-specific public key material [¶](#section-5.5.4.3-3.5)

Algorithm-specific fields for Ed25519 keys (example):[¶](#section-5.5.4.3-4)

f.1)

32 octets representing the public key [¶](#section-5.5.4.3-5.1)

Note that it is possible for there to be collisions of Key IDs -- that is, two different keys with the same Key ID. Note that there is a much smaller, but still non-zero, probability that two different keys have the same fingerprint.[¶](#section-5.5.4.3-6)

Also note that if version 3, version 4, and version 6 format keys share the same RSA key material, they will have different Key IDs as well as different fingerprints.[¶](#section-5.5.4.3-7)

Finally, the Key ID and fingerprint of a subkey are calculated in the same way as for a primary key, including the 0x99 (version 4 key) or 0x9B (version 6 key) as the first octet (even though this is not a valid Packet Type ID for a public subkey).[¶](#section-5.5.4.3-8)

#### 5.5.5.

The public and secret key formats specify algorithm-specific parts of a key. The following sections describe them in detail.[¶](#section-5.5.5-1)

##### 5.5.5.1.

For RSA keys, the public key consists of this series of multiprecision integers:[¶](#section-5.5.5.1-1)

- MPI of RSA public modulus n,[¶](#section-5.5.5.1-2.1.1)
- MPI of RSA public encryption exponent e.[¶](#section-5.5.5.1-2.2.1)

The secret key consists of this series of multiprecision integers:[¶](#section-5.5.5.1-3)

- MPI of RSA secret exponent d;[¶](#section-5.5.5.1-4.1.1)
- MPI of RSA secret prime value p;[¶](#section-5.5.5.1-4.2.1)
- MPI of RSA secret prime value q (p < q); and [¶](#section-5.5.5.1-4.3.1)
- MPI of u, the multiplicative inverse of p, mod q.[¶](#section-5.5.5.1-4.4.1)

##### 5.5.5.2.

For DSA keys, the public key consists of this series of multiprecision integers:[¶](#section-5.5.5.2-1)

- MPI of DSA prime p;[¶](#section-5.5.5.2-2.1.1)
- MPI of DSA group order q (q is a prime divisor of p-1);[¶](#section-5.5.5.2-2.2.1)
- MPI of DSA group generator g; and [¶](#section-5.5.5.2-2.3.1)
- MPI of DSA public key value y (= g <sup>x</sup> mod p where x is secret).[¶](#section-5.5.5.2-2.4.1)

The secret key consists of this single multiprecision integer:[¶](#section-5.5.5.2-3)

- MPI of DSA secret exponent x.[¶](#section-5.5.5.2-4.1.1)

##### 5.5.5.3.

For Elgamal keys, the public key consists of this series of multiprecision integers:[¶](#section-5.5.5.3-1)

- MPI of Elgamal prime p;[¶](#section-5.5.5.3-2.1.1)
- MPI of Elgamal group generator g; and [¶](#section-5.5.5.3-2.2.1)
- MPI of Elgamal public key value y (= g <sup>x</sup> mod p where x is secret).[¶](#section-5.5.5.3-2.3.1)

The secret key consists of this single multiprecision integer:[¶](#section-5.5.5.3-3)

- MPI of Elgamal secret exponent x.[¶](#section-5.5.5.3-4.1.1)

##### 5.5.5.4.

For ECDSA keys, the public key consists of this series of values:[¶](#section-5.5.5.4-1)

- A variable-length field containing a curve OID, which is formatted as follows:[¶](#section-5.5.5.4-2.1.1)
	- A 1-octet size of the following field; values 0 and 0xFF are reserved for future extensions.[¶](#section-5.5.5.4-2.1.2.1.1)
		- The octets representing a curve OID, as defined in.[¶](#section-5.5.5.4-2.1.2.2.1)
- An MPI of an EC point representing a public key.[¶](#section-5.5.5.4-2.2.1)

The secret key consists of this single multiprecision integer:[¶](#section-5.5.5.4-3)

- An MPI of an integer representing the secret key, which is a scalar of the public EC point.[¶](#section-5.5.5.4-4.1.1)

##### 5.5.5.5.

For EdDSALegacy keys (deprecated), the public key consists of this series of values:[¶](#section-5.5.5.5-1)

- A variable-length field containing a curve OID, formatted as follows:[¶](#section-5.5.5.5-2.1.1)
	- A 1-octet size of the following field; values 0 and 0xFF are reserved for future extensions.[¶](#section-5.5.5.5-2.1.2.1.1)
		- The octets representing a curve OID, as defined in.[¶](#section-5.5.5.5-2.1.2.2.1)
- An MPI of an EC point representing a public key Q in prefixed native form (see ).[¶](#section-5.5.5.5-2.2.1)

The secret key consists of this single multiprecision integer:[¶](#section-5.5.5.5-3)
...
For ECDH keys, the public key consists of this series of values:[¶](#section-5.5.5.6-1)

- A variable-length field containing a curve OID, which is formatted as follows:[¶](#section-5.5.5.6-2.1.1)
	- A 1-octet size of the following field; values 0 and 0xFF are reserved for future extensions.[¶](#section-5.5.5.6-2.1.2.1.1)
		- The octets representing a curve OID, as defined in.[¶](#section-5.5.5.6-2.1.2.2.1)
- An MPI of an EC point representing a public key, in the point format associated with the curve, as specified in.[¶](#section-5.5.5.6-2.2.1)
- A variable-length field containing key derivation function (KDF) parameters, which is formatted as follows:[¶](#section-5.5.5.6-2.3.1)
	- A 1-octet size of the following fields; values 0 and 0xFF are reserved for future extensions.[¶](#section-5.5.5.6-2.3.2.1.1)
		- A 1-octet value 1, reserved for future extensions.[¶](#section-5.5.5.6-2.3.2.2.1)
		- A 1-octet hash function ID used with a KDF.[¶](#section-5.5.5.6-2.3.2.3.1)
		- A 1-octet algorithm ID for the symmetric algorithm that is used to wrap the symmetric key for message encryption; see for details.[¶](#section-5.5.5.6-2.3.2.4.1)

The secret key consists of this single multiprecision integer:[¶](#section-5.5.5.6-3)

- An MPI representing the secret key, in the curve-specific format described in.[¶](#section-5.5.5.6-4.1.1)

###### 5.5.5.6.1.

When curve NIST P-256, NIST P-384, NIST P-521, brainpoolP256r1, brainpoolP384r1, or brainpoolP512r1 are used in ECDH, their secret keys are represented as a simple integer in standard MPI form. Other curves are presented on the wire differently (though still as a single MPI), as described below and in.[¶](#section-5.5.5.6.1-1)
...
```
[¶](#section-5.5.5.6.1.1-4)

##### 5.5.5.7.

For X25519 keys, the public key consists of this single value:[¶](#section-5.5.5.7-1)

- 32 octets of the native public key.[¶](#section-5.5.5.7-2.1.1)

The secret key consists of this single value:[¶](#section-5.5.5.7-3)
...
For X448 keys, the public key consists of this single value:[¶](#section-5.5.5.8-1)

- 56 octets of the native public key.[¶](#section-5.5.5.8-2.1.1)

The secret key consists of this single value:[¶](#section-5.5.5.8-3)
...
For Ed25519 keys, the public key consists of this single value:[¶](#section-5.5.5.9-1)

- 32 octets of the native public key.[¶](#section-5.5.5.9-2.1.1)

The secret key consists of this single value:[¶](#section-5.5.5.9-3)
...
For Ed448 keys, the public key consists of this single value:[¶](#section-5.5.5.10-1)

- 57 octets of the native public key.[¶](#section-5.5.5.10-2.1.1)

The secret key consists of this single value:[¶](#section-5.5.5.10-3)
...
The Compressed Data packet contains compressed data. Typically, this packet is found as the contents of an encrypted packet, or following a Signature or One-Pass Signature packet, and contains a Literal Data packet.[¶](#section-5.6-1)

The body of this packet consists of:[¶](#section-5.6-2)
...
A ZLIB-compressed series of packets is compressed with raw ZLIB-style blocks \[\].[¶](#section-5.6-6)
...
- A random prefix, containing block-size random octets (for example, 16 octets for a 128-bit block length) followed by a copy of the last two octets, encrypted together using Cipher Feedback (CFB) mode, with an IV of all zeros.[¶](#section-5.7-5.1.1)
- Data encrypted using CFB mode, with the last block-size octets of the first ciphertext as the IV.[¶](#section-5.7-5.2.1)

The symmetric cipher used may be specified in a Public Key or Symmetric Key Encrypted Session Key packet that precedes the Symmetrically Encrypted Data packet. In that case, the cipher algorithm ID is prefixed to the session key before it is encrypted. If no packets of these types precede the encrypted data, the IDEA algorithm is used with the session key calculated as the MD5 hash of the passphrase, though this use is deprecated.[¶](#section-5.7-6)

The data is encrypted in CFB mode (see ). For the random prefix, the IV is specified as all zeros. Instead of achieving randomized encryption through an IV, a string of length equal to the block size of the cipher plus two is encrypted for this purpose. The first block-size octets (for example, 16 octets for a 128-bit block length) are random, and the following two octets are copies of the last two octets of the first block-size random octets. For example, for a 16-octet block length, octet 17 is a copy of octet 15, and octet 18 is a copy of octet 16. For a cipher of block length 8, octet 9 is a copy of octet 7, and octet 10 is a copy of octet 8. (In both of these examples, we consider the first octet to be numbered 1.) [¶](#section-5.7-7)
...
The repetition of two octets in the random prefix allows the receiver to immediately check whether the session key is incorrect. See for hints on the proper use of this "quick check".[¶](#section-5.7-9)

### 5.8.

The body of the Marker packet consists of:[¶](#section-5.8-1)

- The three octets 0x50, 0x47, 0x50 (which spell "PGP" in UTF-8).[¶](#section-5.8-2.1.1)

Such a packet MUST be ignored when received.[¶](#section-5.8-3)

### 5.9.

A Literal Data packet contains the body of a message; that is, data that is not to be further interpreted.[¶](#section-5.9-1)

The body of this packet consists of:[¶](#section-5.9-2)
...
The Trust packet is used only within keyrings and is not normally exported. Trust packets contain data that record the user's specifications of which keyholders are trustworthy introducers, along with other information that implementation uses for trust information. The format of Trust packets is defined by a given implementation.[¶](#section-5.10-1)

Trust packets SHOULD NOT be emitted to output streams that are transferred to other users, and they SHOULD be ignored on any input other than local keyring files.[¶](#section-5.10-2)

### 5.11.

A User ID packet consists of UTF-8 text that is intended to represent the name and email address of the keyholder. By convention, it includes a mail name-addr as described in \[\], but there are no restrictions on its content. The packet length in the header specifies the length of the User ID.[¶](#section-5.11-1)
...
- the subpacket length (1, 2, or 5 octets) [¶](#section-5.12-4.1.1)
- the Subpacket Type ID (1 octet) [¶](#section-5.12-4.2.1)

and is followed by the subpacket specific data.[¶](#section-5.12-5)

The following table lists the currently known subpackets:[¶](#section-5.12-6)

<table><caption>:</caption><thead><tr><th rowspan="1" colspan="1">ID</th><th rowspan="1" colspan="1">Attribute Subpacket</th><th rowspan="1" colspan="1">Reference</th></tr></thead><tbody><tr><td rowspan="1" colspan="1">0</td><td rowspan="1" colspan="1">Reserved</td><td rowspan="1" colspan="1"></td></tr><tr><td rowspan="1" colspan="1">1</td><td rowspan="1" colspan="1">Image Attribute Subpacket</td><td rowspan="1" colspan="1"></td></tr><tr><td rowspan="1" colspan="1">100-110</td><td rowspan="1" colspan="1">Private or Experimental Use</td><td rowspan="1" colspan="1"></td></tr></tbody></table>

An implementation SHOULD ignore any subpacket of a type that it does not recognize.[¶](#section-5.12-8)

#### 5.12.1.

The Image Attribute subpacket is used to encode an image, presumably (but not required to be) that of the key owner.[¶](#section-5.12.1-1)

The Image Attribute subpacket begins with an image header. The first two octets of the image header contain the length of the image header. Note that unlike other multi-octet numerical values in this document, due to a historical accident, this value is encoded as a little-endian number. The image header length is followed by a single octet for the image header version. The only currently defined version of the image header is 1, which is a 16-octet image header. The first three octets of a version 1 image header are thus 0x10, 0x00, 0x01.[¶](#section-5.12.1-2)

<table><caption>:</caption><thead><tr><th rowspan="1" colspan="1">Version</th><th rowspan="1" colspan="1">Reference</th></tr></thead><tbody><tr><td rowspan="1" colspan="1">1</td><td rowspan="1" colspan="1"></td></tr></tbody></table>

The fourth octet of a version 1 image header designates the encoding format of the image. The only currently defined encoding format is the value 1 to indicate JPEG. Image format IDs 100 through 110 are reserved for Private or Experimental Use. The rest of the version 1 image header is made up of 12 reserved octets, all of which MUST be set to 0.[¶](#section-5.12.1-4)

<table><caption>:</caption><thead><tr><th rowspan="1" colspan="1">ID</th><th rowspan="1" colspan="1">Encoding</th></tr></thead><tbody><tr><td rowspan="1" colspan="1">0</td><td rowspan="1" colspan="1">Reserved</td></tr><tr><td rowspan="1" colspan="1">1</td><td rowspan="1" colspan="1">JPEG []</td></tr><tr><td rowspan="1" colspan="1">100-110</td><td rowspan="1" colspan="1">Private or Experimental Use</td></tr></tbody></table>

The rest of the image subpacket contains the image itself. As the only currently defined image type is JPEG, the image is encoded in the JPEG File Interchange Format (JFIF), a standard file format for JPEG images \[\].[¶](#section-5.12.1-6)

An implementation MAY try to determine the type of an image by examination of the image data if it is unable to handle a particular version of the image header or if a specified encoding format value is not recognized.[¶](#section-5.12.1-7)
...
Version 2 of this packet contains data encrypted with an AEAD construction. This offers a more cryptographically rigorous defense against ciphertext malleability. See for more details on choosing between these formats.[¶](#section-5.13-3)

Any new version of the SEIPD packet should be registered in the registry established in.[¶](#section-5.13-4)

#### 5.13.1.

A version 1 Symmetrically Encrypted and Integrity Protected Data packet consists of:[¶](#section-5.13.1-1)

- A 1-octet version number with value 1.[¶](#section-5.13.1-2.1.1)
- Encrypted data -- the output of the selected symmetric key cipher operating in CFB mode.[¶](#section-5.13.1-2.2.1)
...
Two constant octets with the values 0xD3 and 0x14 are appended to the plaintext. Then, the plaintext of the data to be encrypted is passed through the SHA-1 hash function. The input to the hash function is comprised of the prefix data described above and all of the plaintext, including the trailing constant octets 0xD3, 0x14. The 20 octets of the SHA-1 hash are then appended to the plaintext (after the constant octets 0xD3, 0x14) and encrypted along with the plaintext using the same CFB context. This trailing checksum is known as the Modification Detection Code (MDC).[¶](#section-5.13.1-6)

During decryption, the plaintext data should be hashed with SHA-1, including the prefix data as well as the trailing constant octets 0xD3, 0x14, but excluding the last 20 octets containing the SHA-1 hash. The computed SHA-1 hash is then compared with the last 20 octets of plaintext. A mismatch of the hash indicates that the message has been modified and MUST be treated as a security problem. Any failure SHOULD be reported to the user.[¶](#section-5.13.1-7)

NON-NORMATIVE EXPLANATION [¶](#section-5.13.1-8)

The MDC system, as the integrity protection mechanism of the version 1 Symmetrically Encrypted and Integrity Protected Data packet is called, was created to provide an integrity mechanism that is less strong than a signature, yet stronger than bare CFB encryption.[¶](#section-5.13.1-9)

CFB encryption has a limitation as damage to the ciphertext will corrupt the affected cipher blocks and the block following. Additionally, if data is removed from the end of a CFB-encrypted block, that removal is undetectable. (Note also that CBC mode has a similar limitation, but data removed from the front of the block is undetectable.) [¶](#section-5.13.1-10)
...
A version 2 Symmetrically Encrypted and Integrity Protected Data packet consists of:[¶](#section-5.13.2-1)

- A 1-octet version number with value 2.[¶](#section-5.13.2-2.1.1)
- A 1-octet cipher algorithm ID.[¶](#section-5.13.2-2.2.1)
- A 1-octet AEAD algorithm identifier.[¶](#section-5.13.2-2.3.1)
- A 1-octet chunk size.[¶](#section-5.13.2-2.4.1)
- 32 octets of salt. The salt is used to derive the message key and MUST be securely generated (see ).[¶](#section-5.13.2-2.5.1)
...
A v2 SEIPD packet consists of one or more chunks of data. The plaintext of each chunk is of a size specified by the chunk size octet using the method specified below.[¶](#section-5.13.2-5)
...
The chunk size octet specifies the size of chunks using the following formula (in C \[\]), where c is the chunk size octet:[¶](#section-5.13.2-9)
...
An implementation MUST accept chunk size octets with values from 0 to 16. An implementation MUST NOT create data with a chunk size octet value larger than 16 (4 MiB chunks).[¶](#section-5.13.2-11)

The nonce for AEAD mode consists of two parts. Let N be the size of the nonce. The leftmost N - 64 bits are the IV derived using HKDF. The rightmost 64 bits are the chunk index as a big-endian value. The index of the first chunk is zero.[¶](#section-5.13.2-12)

#### 5.13.3.

The EAX AEAD algorithm used in this document is defined in \[\].[¶](#section-5.13.3-1)

The EAX algorithm can only use block ciphers with 16-octet blocks. The nonce is 16 octets long. EAX authentication tags are 16 octets long.[¶](#section-5.13.3-2)

#### 5.13.4.

The OCB AEAD algorithm used in this document is defined in \[\].[¶](#section-5.13.4-1)

The OCB algorithm can only use block ciphers with 16-octet blocks. The nonce is 15 octets long. OCB authentication tags are 16 octets long.[¶](#section-5.13.4-2)

#### 5.13.5.

The GCM AEAD algorithm used in this document is defined in \[\].[¶](#section-5.13.5-1)

The GCM algorithm can only use block ciphers with 16-octet blocks. The nonce is 12 octets long. GCM authentication tags are 16 octets long.[¶](#section-5.13.5-2)

### 5.14.

The Padding packet contains random data and can be used to defend against traffic analysis (see ) on v2 SEIPD messages (see ) and Transferable Public Keys (see ).[¶](#section-5.14-1)

Such a packet MUST be ignored when received.[¶](#section-5.14-2)
...
- An ASCII-armored Transferable Public Key packet sequence of a version 6 key MUST NOT contain a CRC24 footer.[¶](#section-6.1-5.3.1)
- An ASCII-armored keyring consisting of only version 6 keys MUST NOT contain a CRC24 footer.[¶](#section-6.1-5.4.1)

Rationale: Previous draft versions of this document stated that the CRC24 footer is optional, but the text was ambiguous. In practice, very few implementations require the CRC24 footer to be present. Computing the CRC24 incurs a significant cost, while providing no meaningful integrity protection. Therefore, generating it is now discouraged.[¶](#section-6.1-6)

#### 6.1.1.

The following code is written in \[\].[¶](#section-6.1.1-1)
...
- An Armor Header Line, appropriate for the type of data [¶](#section-6.2-3.1.1)
- Armor Headers [¶](#section-6.2-3.2.1)
- A blank (zero length or containing only whitespace) line [¶](#section-6.2-3.3.1)
- The ASCII-Armored data [¶](#section-6.2-3.4.1)
- An optional Armor Checksum (discouraged; see ) [¶](#section-6.2-3.5.1)
- The Armor Tail, which depends on the Armor Header Line [¶](#section-6.2-3.6.1)
...
While there is a limit of 76 characters for the base64 data (), there is no limit for the length of Armor Headers. Care should be taken to ensure that the Armor Headers are short enough to survive transport. One way to do this is to repeat an Armor Header Key multiple times with different values for each so that no one line is overly long.[¶](#section-6.2.2-4)

Currently defined Armor Header Keys are as follows:[¶](#section-6.2.2-5)

<table><caption>:</caption><thead><tr><th rowspan="1" colspan="1">Key</th><th rowspan="1" colspan="1">Summary</th><th rowspan="1" colspan="1">Reference</th></tr></thead><tbody><tr><td rowspan="1" colspan="1"><code>Version</code></td><td rowspan="1" colspan="1">Implementation information</td><td rowspan="1" colspan="1"></td></tr><tr><td rowspan="1" colspan="1"><code>Comment</code></td><td rowspan="1" colspan="1">Arbitrary text</td><td rowspan="1" colspan="1"></td></tr><tr><td rowspan="1" colspan="1"><code>Hash</code></td><td rowspan="1" colspan="1">Hash algorithms used in some v4 cleartext signed messages</td><td rowspan="1" colspan="1"></td></tr><tr><td rowspan="1" colspan="1"><code>Charset</code></td><td rowspan="1" colspan="1">Character set</td><td rowspan="1" colspan="1"></td></tr></tbody></table>

## 7.

It is desirable to be able to sign a textual octet stream without ASCII armoring the stream itself, so the signed text is still readable with any tool capable of rendering text. In order to bind a signature to such a cleartext, the Cleartext Signature Framework is used, which follows the same basic format and restrictions as the ASCII armoring described in. (Note that this framework is not intended to be reversible. \[\] defines another way to sign cleartext messages for environments that support MIME.) [¶](#section-7-1)
...
- An empty line (not included in the message digest).[¶](#section-7.1-2.3.1)
- The dash-escaped cleartext.[¶](#section-7.1-2.4.1)
- A line ending separating the cleartext and following armored signature (not included in the message digest).[¶](#section-7.1-2.5.1)
- The ASCII-armored signature(s), including the `-----BEGIN PGP SIGNATURE-----` Armor Header and Armor Tail Lines.[¶](#section-7.1-2.6.1)

As with any other Text signature (), a cleartext signature is calculated on the text using canonical <CR><LF> line endings. As described above, the line ending before the `-----BEGIN PGP SIGNATURE-----` Armor Header Line of the armored signature is not considered part of the signed text.[¶](#section-7.1-3)

Also, any trailing whitespace -- spaces (0x20) and tabs (0x09) -- at the end of any line is removed before signing or verifying a cleartext signed message.[¶](#section-7.1-4)

Between the `-----BEGIN PGP SIGNED MESSAGE-----` line and the first empty line, the only Armor Header permitted is a well-formed `Hash` Armor Header (see ). To reduce the risk of confusion about what has been signed, a verifying implementation MUST decline to validate any signature in a cleartext message if that message has any other Armor Header present in this location.[¶](#section-7.1-5)

### 7.2.

The cleartext content of the message must also be dash-escaped.[¶](#section-7.2-1)

Dash-escaped cleartext is the ordinary cleartext where every line starting with a "-" (HYPHEN-MINUS, U+002D) is prefixed by the sequence "-" (HYPHEN-MINUS, U+002D) and " " (SPACE, U+0020). This prevents the parser from recognizing Armor Headers of the cleartext itself. An implementation MAY dash-escape any line, SHOULD dash-escape lines commencing in "From" followed by a space, and MUST dash-escape any line commencing in a dash. The message digest is computed using the cleartext itself, not the dash-escaped form.[¶](#section-7.2-2)

When reversing dash-escaping, an implementation MUST strip the string `-` if it occurs at the beginning of a line, and it SHOULD provide a warning for `-` and any character other than a space at the beginning of a line.[¶](#section-7.2-3)

### 7.3.

Since creating a cleartext signed message involves trimming trailing whitespace on every line, the Cleartext Signature Framework will fail to safely round-trip any textual stream that may include semantically meaningful whitespace.[¶](#section-7.3-1)

For example, the Unified Diff format \[\] contains semantically meaningful whitespace: an empty line of context will consist of a line with a single " " (SPACE, U+0020) character, and any line that has trailing whitespace added or removed will represent such a change with semantically meaningful whitespace.[¶](#section-7.3-2)
...
Either of these alternatives may be ASCII-armored (see ) if they need to be transmitted across a text-only (or 7-bit clean) channel.[¶](#section-7.3-6)

Finally, when a Cleartext Signature Framework message is presented to the user as is, an attacker can include additional text in the `Hash` header, which may mislead the user into thinking it is part of the signed text. The signature validation constraints described in Sections and help to mitigate the risk of arbitrary or misleading text in the Armor Headers.[¶](#section-7.3-7)

## 8.

This section describes Regular Expressions.[¶](#section-8-1)

Regular Expression:

Zero or more branches, separated by `|`. It matches anything that matches one of the branches.[¶](#section-8-2.2)

Branch:

Zero or more pieces, concatenated. It matches a match for the first, followed by a match for the second, etc.[¶](#section-8-2.4)

Piece:

An atom possibly followed by `*`, `+`, or `?`. An atom followed by `*` matches a sequence of 0 or more matches of the atom. An atom followed by `+` matches a sequence of 1 or more matches of the atom. An atom followed by `?` matches a match of the atom or the null string.[¶](#section-8-2.6)

Atom:

A Regular Expression in parentheses (matching a match for the Regular Expression), a range (see below), a `.` (matching any single Unicode character), a `^` (matching the null string at the beginning of the input string), a `$` (matching the null string at the end of the input string), a `\` followed by a single Unicode character (matching that character), or a single Unicode character with no other significance (matching that character).[¶](#section-8-2.8)

Range:

A sequence of characters enclosed in `[]`. It normally matches any single character from the sequence. If the sequence begins with `^`, it matches any single Unicode character not from the rest of the sequence. If two characters in the sequence are separated by `-`, this is shorthand for the full list of Unicode characters between them in codepoint order (for example, `[0-9]` matches any decimal digit). To include a literal `]` in the sequence, make it the first character (following a possible `^`). To include a literal `-`, make it the first or last character.[¶](#section-8-2.10)
...
See for more discussion of the algorithms.[¶](#section-9-3)
...
RSA (1) keys are deprecated and SHOULD NOT be generated but may be interpreted. RSA Encrypt-Only (2) and RSA Sign-Only (3) are deprecated and MUST NOT be generated (see ). Elgamal (16) keys are deprecated and MUST NOT be generated (see ). DSA (17) keys are deprecated and MUST NOT be generated (see ). For notes on Elgamal Encrypt or Sign (20) and X9.42 (21), see. Implementations MAY implement any other algorithm.[¶](#section-9.1-3)

Note that an implementation conforming to the previous version of this specification \[\] has only DSA (17) and Elgamal (16) as the algorithms that MUST be implemented.[¶](#section-9.1-4)

A compatible specification of ECDSA is given in \[\] (as "KT-I Signatures") and in \[\]; ECDH is defined in of this document.[¶](#section-9.1-5)

### 9.2.

The parameter curve OID is an array of octets that defines a named curve.[¶](#section-9.2-1)
...
Some elliptic curve public key algorithms use different conventions for specific fields depending on the curve in use. Each field is always formatted as an MPI, but with a curve-specific framing. This table summarizes those distinctions.[¶](#section-9.2.1-1)

<table><caption>:</caption><thead><tr><th rowspan="1" colspan="1">Curve</th><th rowspan="1" colspan="1">ECDH Point Format</th><th rowspan="1" colspan="1">ECDH Secret Key MPI</th><th rowspan="1" colspan="1">EdDSA Secret Key MPI</th><th rowspan="1" colspan="1">EdDSA Signature first MPI</th><th rowspan="1" colspan="1">EdDSA Signature second MPI</th></tr></thead><tbody><tr><td rowspan="1" colspan="1">NIST P-256</td><td rowspan="1" colspan="1">SEC1</td><td rowspan="1" colspan="1">integer</td><td rowspan="1" colspan="1">N/A</td><td rowspan="1" colspan="1">N/A</td><td rowspan="1" colspan="1">N/A</td></tr><tr><td rowspan="1" colspan="1">NIST P-384</td><td rowspan="1" colspan="1">SEC1</td><td rowspan="1" colspan="1">integer</td><td rowspan="1" colspan="1">N/A</td><td rowspan="1" colspan="1">N/A</td><td rowspan="1" colspan="1">N/A</td></tr><tr><td rowspan="1" colspan="1">NIST P-521</td><td rowspan="1" colspan="1">SEC1</td><td rowspan="1" colspan="1">integer</td><td rowspan="1" colspan="1">N/A</td><td rowspan="1" colspan="1">N/A</td><td rowspan="1" colspan="1">N/A</td></tr><tr><td rowspan="1" colspan="1">brainpoolP256r1</td><td rowspan="1" colspan="1">SEC1</td><td rowspan="1" colspan="1">integer</td><td rowspan="1" colspan="1">N/A</td><td rowspan="1" colspan="1">N/A</td><td rowspan="1" colspan="1">N/A</td></tr><tr><td rowspan="1" colspan="1">brainpoolP384r1</td><td rowspan="1" colspan="1">SEC1</td><td rowspan="1" colspan="1">integer</td><td rowspan="1" colspan="1">N/A</td><td rowspan="1" colspan="1">N/A</td><td rowspan="1" colspan="1">N/A</td></tr><tr><td rowspan="1" colspan="1">brainpoolP512r1</td><td rowspan="1" colspan="1">SEC1</td><td rowspan="1" colspan="1">integer</td><td rowspan="1" colspan="1">N/A</td><td rowspan="1" colspan="1">N/A</td><td rowspan="1" colspan="1">N/A</td></tr><tr><td rowspan="1" colspan="1">Ed25519Legacy</td><td rowspan="1" colspan="1">N/A</td><td rowspan="1" colspan="1">N/A</td><td rowspan="1" colspan="1">32 octets of secret</td><td rowspan="1" colspan="1">32 octets of R</td><td rowspan="1" colspan="1">32 octets of S</td></tr><tr><td rowspan="1" colspan="1">Curve25519Legacy</td><td rowspan="1" colspan="1">prefixed native</td><td rowspan="1" colspan="1">integer ()</td><td rowspan="1" colspan="1">N/A</td><td rowspan="1" colspan="1">N/A</td><td rowspan="1" colspan="1">N/A</td></tr></tbody></table>

For the native octet-string forms of Ed25519Legacy values, see \[\]. For the native octet-string forms of Curve25519Legacy secret scalars and points, see \[\].[¶](#section-9.2.1-3)

### 9.3.

<table><caption>:</caption><thead><tr><th rowspan="1" colspan="1">ID</th><th rowspan="1" colspan="1">Algorithm</th></tr></thead><tbody><tr><td rowspan="1" colspan="1">0</td><td rowspan="1" colspan="1">Plaintext or unencrypted data</td></tr><tr><td rowspan="1" colspan="1">1</td><td rowspan="1" colspan="1">IDEA []</td></tr><tr><td rowspan="1" colspan="1">2</td><td rowspan="1" colspan="1">TripleDES (or DES-EDE) [] with 168-bit key derived from 192</td></tr><tr><td rowspan="1" colspan="1">3</td><td rowspan="1" colspan="1">CAST5 with 128-bit key []</td></tr><tr><td rowspan="1" colspan="1">4</td><td rowspan="1" colspan="1">Blowfish with 128-bit key, 16 rounds []</td></tr><tr><td rowspan="1" colspan="1">5</td><td rowspan="1" colspan="1">Reserved</td></tr><tr><td rowspan="1" colspan="1">6</td><td rowspan="1" colspan="1">Reserved</td></tr><tr><td rowspan="1" colspan="1">7</td><td rowspan="1" colspan="1">AES with 128-bit key []</td></tr><tr><td rowspan="1" colspan="1">8</td><td rowspan="1" colspan="1">AES with 192-bit key</td></tr><tr><td rowspan="1" colspan="1">9</td><td rowspan="1" colspan="1">AES with 256-bit key</td></tr><tr><td rowspan="1" colspan="1">10</td><td rowspan="1" colspan="1">Twofish with 256-bit key []</td></tr><tr><td rowspan="1" colspan="1">11</td><td rowspan="1" colspan="1">Camellia with 128-bit key []</td></tr><tr><td rowspan="1" colspan="1">12</td><td rowspan="1" colspan="1">Camellia with 192-bit key</td></tr><tr><td rowspan="1" colspan="1">13</td><td rowspan="1" colspan="1">Camellia with 256-bit key</td></tr><tr><td rowspan="1" colspan="1">100-110</td><td rowspan="1" colspan="1">Private or Experimental Use</td></tr><tr><td rowspan="1" colspan="1">253-255</td><td rowspan="1" colspan="1">Reserved to avoid collision with Secret Key Encryption ( and )</td></tr></tbody></table>

Implementations MUST implement AES-128. Implementations SHOULD implement AES-256. Implementations MUST NOT encrypt data with IDEA, TripleDES, or CAST5. Implementations MAY decrypt data that uses IDEA, TripleDES, or CAST5 for the sake of reading older messages or new messages from implementations predating support for \[\]. An Implementation that decrypts data using IDEA, TripleDES, or CAST5 SHOULD generate a deprecation warning about the symmetric algorithm, indicating that message confidentiality is suspect. Implementations MAY implement any other algorithm.[¶](#section-9.3-2)

### 9.4.

<table><caption>:</caption><thead><tr><th rowspan="1" colspan="1">ID</th><th rowspan="1" colspan="1">Algorithm</th></tr></thead><tbody><tr><td rowspan="1" colspan="1">0</td><td rowspan="1" colspan="1">Uncompressed</td></tr><tr><td rowspan="1" colspan="1">1</td><td rowspan="1" colspan="1">ZIP []</td></tr><tr><td rowspan="1" colspan="1">2</td><td rowspan="1" colspan="1">ZLIB []</td></tr><tr><td rowspan="1" colspan="1">3</td><td rowspan="1" colspan="1">BZip2 []</td></tr><tr><td rowspan="1" colspan="1">100-110</td><td rowspan="1" colspan="1">Private or Experimental Use</td></tr></tbody></table>

Implementations MUST implement uncompressed data. Implementations SHOULD implement ZLIB. For interoperability reasons, implementations SHOULD be able to decompress using ZIP. Implementations MAY implement any other algorithm.[¶](#section-9.4-2)

### 9.5.

<table><caption>:</caption><thead><tr><th rowspan="1" colspan="1">ID</th><th rowspan="1" colspan="1">Algorithm</th><th rowspan="1" colspan="1">Text Name</th><th rowspan="1" colspan="1">V6 Signature Salt Size</th></tr></thead><tbody><tr><td rowspan="1" colspan="1">0</td><td rowspan="1" colspan="1">Reserved</td><td rowspan="1" colspan="1"></td><td rowspan="1" colspan="1"></td></tr><tr><td rowspan="1" colspan="1">1</td><td rowspan="1" colspan="1">MD5 []</td><td rowspan="1" colspan="1">"MD5"</td><td rowspan="1" colspan="1">N/A</td></tr><tr><td rowspan="1" colspan="1">2</td><td rowspan="1" colspan="1">SHA-1 []</td><td rowspan="1" colspan="1">"SHA1"</td><td rowspan="1" colspan="1">N/A</td></tr><tr><td rowspan="1" colspan="1">3</td><td rowspan="1" colspan="1">RIPEMD-160 []</td><td rowspan="1" colspan="1">"RIPEMD160"</td><td rowspan="1" colspan="1">N/A</td></tr><tr><td rowspan="1" colspan="1">4</td><td rowspan="1" colspan="1">Reserved</td><td rowspan="1" colspan="1"></td><td rowspan="1" colspan="1"></td></tr><tr><td rowspan="1" colspan="1">5</td><td rowspan="1" colspan="1">Reserved</td><td rowspan="1" colspan="1"></td><td rowspan="1" colspan="1"></td></tr><tr><td rowspan="1" colspan="1">6</td><td rowspan="1" colspan="1">Reserved</td><td rowspan="1" colspan="1"></td><td rowspan="1" colspan="1"></td></tr><tr><td rowspan="1" colspan="1">7</td><td rowspan="1" colspan="1">Reserved</td><td rowspan="1" colspan="1"></td><td rowspan="1" colspan="1"></td></tr><tr><td rowspan="1" colspan="1">8</td><td rowspan="1" colspan="1">SHA2-256 []</td><td rowspan="1" colspan="1">"SHA256"</td><td rowspan="1" colspan="1">16</td></tr><tr><td rowspan="1" colspan="1">9</td><td rowspan="1" colspan="1">SHA2-384 []</td><td rowspan="1" colspan="1">"SHA384"</td><td rowspan="1" colspan="1">24</td></tr><tr><td rowspan="1" colspan="1">10</td><td rowspan="1" colspan="1">SHA2-512 []</td><td rowspan="1" colspan="1">"SHA512"</td><td rowspan="1" colspan="1">32</td></tr><tr><td rowspan="1" colspan="1">11</td><td rowspan="1" colspan="1">SHA2-224 []</td><td rowspan="1" colspan="1">"SHA224"</td><td rowspan="1" colspan="1">16</td></tr><tr><td rowspan="1" colspan="1">12</td><td rowspan="1" colspan="1">SHA3-256 []</td><td rowspan="1" colspan="1">"SHA3-256"</td><td rowspan="1" colspan="1">16</td></tr><tr><td rowspan="1" colspan="1">13</td><td rowspan="1" colspan="1">Reserved</td><td rowspan="1" colspan="1"></td><td rowspan="1" colspan="1"></td></tr><tr><td rowspan="1" colspan="1">14</td><td rowspan="1" colspan="1">SHA3-512 []</td><td rowspan="1" colspan="1">"SHA3-512"</td><td rowspan="1" colspan="1">32</td></tr><tr><td rowspan="1" colspan="1">100-110</td><td rowspan="1" colspan="1">Private or Experimental Use</td><td rowspan="1" colspan="1"></td><td rowspan="1" colspan="1"></td></tr></tbody></table>

<table><caption>:</caption><thead><tr><th rowspan="1" colspan="1">Hash Algorithm</th><th rowspan="1" colspan="1">OID</th><th rowspan="1" colspan="1">Full Hash Prefix</th></tr></thead><tbody><tr><td rowspan="1" colspan="1">MD5</td><td rowspan="1" colspan="1">1.2.840.113549.2.5</td><td rowspan="1" colspan="1">0x30, 0x20, 0x30, 0x0C, 0x06, 0x08, 0x2A, 0x86, 0x48, 0x86, 0xF7, 0x0D, 0x02, 0x05, 0x05, 0x00, 0x04, 0x10</td></tr><tr><td rowspan="1" colspan="1">SHA-1</td><td rowspan="1" colspan="1">1.3.14.3.2.26</td><td rowspan="1" colspan="1">0x30, 0x21, 0x30, 0x09, 0x06, 0x05, 0x2B, 0x0E, 0x03, 0x02, 0x1A, 0x05, 0x00, 0x04, 0x14</td></tr><tr><td rowspan="1" colspan="1">RIPEMD-160</td><td rowspan="1" colspan="1">1.3.36.3.2.1</td><td rowspan="1" colspan="1">0x30, 0x21, 0x30, 0x09, 0x06, 0x05, 0x2B, 0x24, 0x03, 0x02, 0x01, 0x05, 0x00, 0x04, 0x14</td></tr><tr><td rowspan="1" colspan="1">SHA2-256</td><td rowspan="1" colspan="1">2.16.840.1.101.3.4.2.1</td><td rowspan="1" colspan="1">0x30, 0x31, 0x30, 0x0D, 0x06, 0x09, 0x60, 0x86, 0x48, 0x01, 0x65, 0x03, 0x04, 0x02, 0x01, 0x05, 0x00, 0x04, 0x20</td></tr><tr><td rowspan="1" colspan="1">SHA2-384</td><td rowspan="1" colspan="1">2.16.840.1.101.3.4.2.2</td><td rowspan="1" colspan="1">0x30, 0x41, 0x30, 0x0D, 0x06, 0x09, 0x60, 0x86, 0x48, 0x01, 0x65, 0x03, 0x04, 0x02, 0x02, 0x05, 0x00, 0x04, 0x30</td></tr><tr><td rowspan="1" colspan="1">SHA2-512</td><td rowspan="1" colspan="1">2.16.840.1.101.3.4.2.3</td><td rowspan="1" colspan="1">0x30, 0x51, 0x30, 0x0D, 0x06, 0x09, 0x60, 0x86, 0x48, 0x01, 0x65, 0x03, 0x04, 0x02, 0x03, 0x05, 0x00, 0x04, 0x40</td></tr><tr><td rowspan="1" colspan="1">SHA2-224</td><td rowspan="1" colspan="1">2.16.840.1.101.3.4.2.4</td><td rowspan="1" colspan="1">0x30, 0x2D, 0x30, 0x0D, 0x06, 0x09, 0x60, 0x86, 0x48, 0x01, 0x65, 0x03, 0x04, 0x02, 0x04, 0x05, 0x00, 0x04, 0x1C</td></tr><tr><td rowspan="1" colspan="1">SHA3-256</td><td rowspan="1" colspan="1">2.16.840.1.101.3.4.2.8</td><td rowspan="1" colspan="1">0x30, 0x31, 0x30, 0x0D, 0x06, 0x09, 0x60, 0x86, 0x48, 0x01, 0x65, 0x03, 0x04, 0x02, 0x08, 0x05, 0x00, 0x04, 0x20</td></tr><tr><td rowspan="1" colspan="1">SHA3-512</td><td rowspan="1" colspan="1">2.16.840.1.101.3.4.2.10</td><td rowspan="1" colspan="1">0x30, 0x51, 0x30, 0x0D, 0x06, 0x09, 0x60, 0x86, 0x48, 0x01, 0x65, 0x03, 0x04, 0x02, 0x0a, 0x05, 0x00, 0x04, 0x40</td></tr></tbody></table>

Implementations MUST implement SHA2-256. Implementations SHOULD implement SHA2-384 and SHA2-512. Implementations MAY implement other algorithms. Implementations SHOULD NOT create messages that require the use of SHA-1, with the exception of computing version 4 key fingerprints for purposes of the MDC in version 1 Symmetrically Encrypted and Integrity Protected Data packets. Implementations MUST NOT generate signatures with MD5, SHA-1, or RIPEMD-160. Implementations MUST NOT use MD5, SHA-1, or RIPEMD-160 as a hash function in an ECDH KDF. Implementations MUST NOT generate packets using MD5, SHA-1, or RIPEMD-160 as a hash function in an S2K KDF. Implementations MUST NOT decrypt a secret using MD5, SHA-1, or RIPEMD-160 as a hash function in an S2K KDF in a version 6 (or later) packet. Implementations MUST NOT validate any recent signature that depends on MD5, SHA-1, or RIPEMD-160. Implementations SHOULD NOT validate any old signature that depends on MD5, SHA-1, or RIPEMD-160 unless the signature's creation date predates known weakness of the algorithm used, and the implementation is confident that the message has been in the secure custody of the user the whole time.[¶](#section-9.5-3)

### 9.6.

<table><caption>:</caption><thead><tr><th rowspan="1" colspan="1">ID</th><th rowspan="1" colspan="1">Name</th><th rowspan="1" colspan="1">Nonce Length (Octets)</th><th rowspan="1" colspan="1">Authentication Tag Length (Octets)</th></tr></thead><tbody><tr><td rowspan="1" colspan="1">0</td><td rowspan="1" colspan="1">Reserved</td><td rowspan="1" colspan="1"></td><td rowspan="1" colspan="1"></td></tr><tr><td rowspan="1" colspan="1">1</td><td rowspan="1" colspan="1">EAX []</td><td rowspan="1" colspan="1">16</td><td rowspan="1" colspan="1">16</td></tr><tr><td rowspan="1" colspan="1">2</td><td rowspan="1" colspan="1">OCB []</td><td rowspan="1" colspan="1">15</td><td rowspan="1" colspan="1">16</td></tr><tr><td rowspan="1" colspan="1">3</td><td rowspan="1" colspan="1">GCM []</td><td rowspan="1" colspan="1">12</td><td rowspan="1" colspan="1">16</td></tr><tr><td rowspan="1" colspan="1">100-110</td><td rowspan="1" colspan="1">Private or Experimental Use</td><td rowspan="1" colspan="1"></td><td rowspan="1" colspan="1"></td></tr></tbody></table>

Implementations MUST implement OCB. Implementations MAY implement EAX, GCM, and other algorithms.[¶](#section-9.6-2)
...
In addition to these rules, a Marker packet () can appear anywhere in the sequence.[¶](#section-10.1.1-3)

Note that a version 6 key uses a self-signed Direct Key signature to store algorithm preferences.[¶](#section-10.1.1-4)

Every subkey for a version 6 primary key MUST be a version 6 subkey. Every subkey MUST have at least one Subkey Binding signature. Every Subkey Binding signature MUST be a self-signature (that is, made by the version 6 primary key). Like all other signatures, every self-signature made by a version 6 key MUST be a version 6 signature.[¶](#section-10.1.1-5)

#### 10.1.2.

When a primary version 6 Public Key is revoked, it is sometimes distributed with only the Revocation Signature:[¶](#section-10.1.2-1)
...
In this case, the Direct Key signature is no longer necessary, since the primary key itself has been marked as unusable.[¶](#section-10.1.2-3)
...
Compressed Data Packet.[¶](#section-10.3-2.4.1)

Literal Message:

Literal Data Packet.[¶](#section-10.3-2.6.1)

ESK:

Public Key Encrypted Session Key Packet | Symmetric Key Encrypted Session Key Packet.[¶](#section-10.3-2.8.1)

ESK Sequence:

ESK | ESK Sequence, ESK.[¶](#section-10.3-2.10.1)

Encrypted Data:

Symmetrically Encrypted Data Packet | Symmetrically Encrypted and Integrity Protected Data Packet.[¶](#section-10.3-2.12.1)

Encrypted Message:

Encrypted Data | ESK Sequence, Encrypted Data.[¶](#section-10.3-2.14.1)
...
Note that some subtle combinations that are formally acceptable by this grammar are nonetheless unacceptable.[¶](#section-10.3.2-1)

##### 10.3.2.1.

As noted above, an Encrypted Message is a sequence of zero or more PKESK packets () and SKESK packets (), followed by an SEIPD () payload. In some historic data, the payload may be a deprecated SED packet () instead of SEIPD, though implementations MUST NOT generate SED packets (see ). The versions of the preceding ESK packets within an Encrypted Message MUST align with the version of the payload SEIPD packet, as described in this section.[¶](#section-10.3.2.1-1)

v3 PKESK and v4 SKESK packets both contain the Symmetric Cipher Algorithm ID and the session key for the subsequent SEIPD packet in their cleartext. Since a v1 SEIPD does not contain a symmetric algorithm ID, all ESK packets preceding a v1 SEIPD payload MUST be either v3 PKESK or v4 SKESK.[¶](#section-10.3.2.1-2)

On the other hand, the cleartext of the v6 ESK packets (either PKESK or SKESK) do not contain a Symmetric Cipher Algorithm ID, so they cannot be used in combination with a v1 SEIPD payload. The payload following any v6 PKESK or v6 SKESK packet MUST be a v2 SEIPD.[¶](#section-10.3.2.1-3)

Additionally, to avoid potentially conflicting cipher algorithm IDs, and for simplicity, implementations MUST NOT precede a v2 SEIPD payload with either v3 PKESK or v4 SKESK packets.[¶](#section-10.3.2.1-4)

The versions of packets found in an Encrypted Message are summarized in the following table. An implementation MUST only generate an Encrypted Message using packet versions that match a row with "Yes" in the "Generate?" column. Other rows are provided for the purpose of historic interoperability. A conforming implementation MUST only generate an Encrypted Message using packets whose versions correspond to a single row.[¶](#section-10.3.2.1-5)

<table><caption>:</caption><thead><tr><th rowspan="1" colspan="1">Version of Encrypted Data Payload</th><th rowspan="1" colspan="1">Version of Preceding Symmetric Key ESK (If Any)</th><th rowspan="1" colspan="1">Version of Preceding Public Key ESK (If Any)</th><th rowspan="1" colspan="1">Generate?</th></tr></thead><tbody><tr><td rowspan="1" colspan="1">SED ()</td><td rowspan="1" colspan="1">-</td><td rowspan="1" colspan="1">v2 PKESK []</td><td rowspan="1" colspan="1">No</td></tr><tr><td rowspan="1" colspan="1">SED ()</td><td rowspan="1" colspan="1">v4 SKESK ()</td><td rowspan="1" colspan="1">v3 PKESK ()</td><td rowspan="1" colspan="1">No</td></tr><tr><td rowspan="1" colspan="1">v1 SEIPD ()</td><td rowspan="1" colspan="1">v4 SKESK ()</td><td rowspan="1" colspan="1">v3 PKESK ()</td><td rowspan="1" colspan="1">Yes</td></tr><tr><td rowspan="1" colspan="1">v2 SEIPD ()</td><td rowspan="1" colspan="1">v6 SKESK ()</td><td rowspan="1" colspan="1">v6 PKESK ()</td><td rowspan="1" colspan="1">Yes</td></tr></tbody></table>

An implementation processing an Encrypted Message MUST discard any preceding ESK packet with a version that does not align with the version of the payload.[¶](#section-10.3.2.1-7)
...
<table><caption>:</caption><thead><tr><th rowspan="1" colspan="1">Signing Key Version</th><th rowspan="1" colspan="1">Signature Packet Version</th><th rowspan="1" colspan="1">OPS Packet Version</th><th rowspan="1" colspan="1">Generate?</th></tr></thead><tbody><tr><td rowspan="1" colspan="1">3 ()</td><td rowspan="1" colspan="1">3 ()</td><td rowspan="1" colspan="1">3 ()</td><td rowspan="1" colspan="1">No</td></tr><tr><td rowspan="1" colspan="1">4 ()</td><td rowspan="1" colspan="1">3 ()</td><td rowspan="1" colspan="1">3 ()</td><td rowspan="1" colspan="1">No</td></tr><tr><td rowspan="1" colspan="1">4 ()</td><td rowspan="1" colspan="1">4 ()</td><td rowspan="1" colspan="1">3 ()</td><td rowspan="1" colspan="1">Yes</td></tr><tr><td rowspan="1" colspan="1">6 ()</td><td rowspan="1" colspan="1">6 ()</td><td rowspan="1" colspan="1">6 ()</td><td rowspan="1" colspan="1">Yes</td></tr></tbody></table>

Note, however, that a version mismatch between these packets does not invalidate the packet sequence as a whole; it merely invalidates the signature, as a signature with an unknown version SHOULD be discarded (see ).[¶](#section-10.3.2.2-4)
...
This section describes algorithms and parameters used with Elliptic Curve Cryptography (ECC) keys. A thorough introduction to ECC can be found in \[\]. Refer to \[\], Appendix B.4, for the methods to generate a uniformly distributed ECC private key.[¶](#section-11-1)

None of the ECC methods described in this document are allowed with deprecated version 3 keys.[¶](#section-11-2)

### 11.1.

This document references three named prime field curves defined in \[\] as "Curve P-256", "Curve P-384", and "Curve P-521" and three named prime field curves defined in \[\] as "brainpoolP256r1", "brainpoolP384r1", and "brainpoolP512r1". All six curves can be used with ECDSA and ECDH public key algorithms. They are referenced using a sequence of octets, referred to as the curve OID. describes in detail how this sequence of octets is formed.[¶](#section-11.1-1)

Separate algorithms are also defined for the use of X25519 and X448 \[\] and Ed25519 and Ed448 \[\]. Additionally, legacy OIDs are defined for "Curve25519Legacy" (for encryption using the ECDH algorithm) and "Ed25519Legacy" (for signing using the EdDSALegacy algorithm).[¶](#section-11.1-2)

### 11.2.

A point on an elliptic curve will always be represented on the wire as an MPI. Each curve uses a specific point format for the data within the MPI itself. Each format uses a designated prefix octet to ensure that the high octet has at least 1 bit set to make the MPI a constant size.[¶](#section-11.2-1)

<table><caption>:</caption><thead><tr><th rowspan="1" colspan="1">Name</th><th rowspan="1" colspan="1">Wire Format</th><th rowspan="1" colspan="1">Reference</th></tr></thead><tbody><tr><td rowspan="1" colspan="1">SEC1</td><td rowspan="1" colspan="1">0x04 || x || y</td><td rowspan="1" colspan="1"></td></tr><tr><td rowspan="1" colspan="1">Prefixed native</td><td rowspan="1" colspan="1">0x40 || native</td><td rowspan="1" colspan="1"></td></tr></tbody></table>

#### 11.2.1.

For a SEC1-encoded (uncompressed) point, the content of the MPI is:[¶](#section-11.2.1-1)
...
where x and y are coordinates of the point P = (x, y), and each is encoded in the big-endian format and zero-padded to the adjusted underlying field size. The adjusted underlying field size is the underlying field size rounded up to the nearest 8-bit boundary, as noted in the "fsize" column in. This encoding is compatible with the definition given in \[\].[¶](#section-11.2.1-3)

#### 11.2.2.

For a custom compressed point, the content of the MPI is:[¶](#section-11.2.2-1)
...
where p is the public key of the point encoded using the rules defined for the specified curve. This format is used for ECDH keys based on curves expressed in Montgomery form and for points when using EdDSA.[¶](#section-11.2.2-3)

#### 11.2.3.

Given the above definitions, the exact size of the MPI payload for an encoded point is 515 bits for both NIST P-256 and brainpoolP256r1, 771 for both NIST P-384 and brainpoolP384r1, 1059 for NIST P-521, 1027 for brainpoolP512r1, and 263 for both Curve25519Legacy and Ed25519Legacy. For example, the length of an EdDSALegacy public key for the curve Ed25519Legacy is 263 bits: 7 bits to represent the 0x40 prefix octet and 32 octets for the native value of the public key.[¶](#section-11.2.3-1)

Even though the zero point (also called the "point at infinity") may occur as a result of arithmetic operations on points of an elliptic curve, it SHALL NOT appear in data structures defined in this document.[¶](#section-11.2.3-2)

Each particular curve uses a designated wire format for the point found in its public key or ECDH data structure. An implementation MUST NOT use a different wire format for a point other than the wire format associated with the curve.[¶](#section-11.2.3-3)
...
Some opaque strings of octets are represented on the wire as an MPI by simply stripping the leading zeros and counting the remaining bits. These strings are of known, fixed length. They are represented in this document as `MPI(N octets of X)`, where `N` is the expected length in octets of the octet string.[¶](#section-11.3.1-1)

For example, a 5-octet opaque string (`MPI(5 octets of X)`) where `X` has the value `00 02 EE 19 00` would be represented on the wire as an MPI like so: `00 1A 02 EE 19 00`.[¶](#section-11.3.1-2)

To encode `X` to the wire format, set the MPI's 2-octet bit counter to the value of the highest set bit (bit 26, or 0x001A), and do not transfer the leading all-zero octet to the wire.[¶](#section-11.3.1-3)

To reverse the process, an implementation can take the following steps, if it knows that X has an expected length of, for example, 5 octets:[¶](#section-11.3.1-4)

- Ensure that the MPI's 2-octet bit count is less than or equal to 40 (5 octets of 8 bits) [¶](#section-11.3.1-5.1.1)
- Allocate 5 octets, setting all to zero initially [¶](#section-11.3.1-5.2.1)
- Copy the MPI data octets (without the two count octets) into the lower octets of the allocated space [¶](#section-11.3.1-5.3.1)

#### 11.3.2.

Another way to ensure that a fixed-length bytes string is encoded simply to the wire while remaining in MPI format is to prefix the byte string with a dedicated non-zero octet. This specification uses 0x40 as the prefix octet. This is represented in this specification as `MPI(prefixed N octets of X)`, where `N` is the known byte string length.[¶](#section-11.3.2-1)

For example, a 5-octet opaque string using `MPI(prefixed 5 octets of X)` where `X` has the value `00 02 EE 19 00` would be written to the wire form as: `00 2F 40 00 02 EE 19 00`.[¶](#section-11.3.2-2)

To encode the string, prefix it with the octet 0x40 (whose 7th bit is set), and then set the MPI's 2-octet bit counter to 47 (0x002F -- 7 bits for the prefix octet and 40 bits for the string).[¶](#section-11.3.2-3)

To decode the string from the wire, an implementation that knows that the variable is formed in this way can:[¶](#section-11.3.2-4)

- ensure that the first three octets of the MPI (the 2-bit count octets plus the prefix octet) are `00 2F 40`, and [¶](#section-11.3.2-5.1.1)
- use the remainder of the MPI directly off the wire.[¶](#section-11.3.2-5.2.1)

Note that this is a similar approach to that used in the EC point encodings found in.[¶](#section-11.3.2-6)

### 11.4.

A key derivation function (KDF) is necessary to implement EC encryption. The Concatenation Key Derivation Function (Approved Alternative 1) \[\] with the KDF hash function that is SHA2-256 \[\] or stronger is REQUIRED.[¶](#section-11.4-1)

For convenience, the synopsis of the encoding method is given below with significant simplifications attributable to the restricted choice of hash functions in this document. However, \[\] is the normative source of the definition.[¶](#section-11.4-2)
...
Note that ZB in the KDF description above is the compact representation of X as defined in [Section 4.2](https://rfc-editor.org/rfc/rfc6090#section-4.2) of \[\].[¶](#section-11.4-4)

### 11.5.

This section describes the One-Pass Diffie-Hellman method, which is a combination of the ECC Diffie-Hellman method that establishes a shared secret and the key derivation method that processes the shared secret into a derived key. Additionally, this section describes a key wrapping method that uses the derived key to protect a session key used to encrypt a message.[¶](#section-11.5-1)

The One-Pass Diffie-Hellman method C(1, 1, ECC CDH) \[\] MUST be implemented with the following restrictions: the ECC Cofactor Diffie-Hellman (CDH) primitive employed by this method is modified to always assume the cofactor is 1, the KDF specified in is used, and the KDF parameters specified below are used.[¶](#section-11.5-2)

The KDF parameters are encoded as a concatenation of the following 5 variable-length and fixed-length fields, which are compatible with the definition of the OtherInfo bit string \[\]:[¶](#section-11.5-3)

- A variable-length field containing a curve OID, which is formatted as follows:[¶](#section-11.5-4.1.1)
	- A 1-octet size of the following field.[¶](#section-11.5-4.1.2.1.1)
		- The octets representing a curve OID, as defined in.[¶](#section-11.5-4.1.2.2.1)
- A 1-octet public key algorithm ID, as defined in.[¶](#section-11.5-4.2.1)
- A variable-length field containing KDF parameters, which are identical to the corresponding field in the ECDH public key and formatted as follows:[¶](#section-11.5-4.3.1)
	- A 1-octet size of the following fields; values 0 and 0xFF are reserved for future extensions.[¶](#section-11.5-4.3.2.1.1)
		- A 1-octet value 0x01, reserved for future extensions.[¶](#section-11.5-4.3.2.2.1)
		- A 1-octet hash function ID used with the KDF.[¶](#section-11.5-4.3.2.3.1)
		- A 1-octet algorithm ID for the symmetric algorithm that is used to wrap the symmetric key for message encryption; see for details.[¶](#section-11.5-4.3.2.4.1)
- 20 octets representing the UTF-8 encoding of the string "Anonymous Sender" padded at the end with spaces (0x20) to 20 octets, which is the octet sequence 41 6E 6F 6E 79 6D 6F 75 73 20 53 65 6E 64 65 72 20 20 20 20.[¶](#section-11.5-4.4.1)
- A variable-length field containing the fingerprint of the recipient encryption subkey identifying the key material that is needed for decryption. For version 4 keys, this field is 20 octets. For version 6 keys, this field is 32 octets.[¶](#section-11.5-4.5.1)

The size in octets of the KDF parameters sequence, as defined above, for encrypting to a version 4 key is 54 for curve NIST P-256; 51 for curves NIST P-384 and NIST P-521; 55 for curves brainpoolP256r1, brainpoolP384r1, and brainpoolP512r1; or 56 for Curve25519Legacy. For encrypting to a version 6 key, the size of the sequence is 66 for curve NIST P-256; 63 for curves NIST P-384 and NIST P-521; or 67 for curves brainpoolP256r1, brainpoolP384r1, and brainpoolP512r1.[¶](#section-11.5-5)

The key wrapping method is described in \[\]. The KDF produces a symmetric key that is used as a KEK as specified in \[\]. Refer to for the details regarding the choice of the KEK algorithm, which SHOULD be one of the three AES algorithms. Key wrapping and unwrapping is performed with the default initial value of \[\].[¶](#section-11.5-6)

To produce the input to the key wrapping method, first concatenate the following values:[¶](#section-11.5-7)

- The 1-octet algorithm identifier, if it was passed (in the case of a v3 PKESK packet).[¶](#section-11.5-8.1.1)
- The session key.[¶](#section-11.5-8.2.1)
- A 2-octet checksum of the session key, equal to the sum of the session key octets, modulo 65536.[¶](#section-11.5-8.3.1)

Then, the above values are padded to an 8-octet granularity using the method described in \[\].[¶](#section-11.5-9)

For example, in a version 3 Public Key Encrypted Session Key packet, an AES-256 session key is encoded as follows, forming a 40-octet sequence:[¶](#section-11.5-10)
...
The octets k0 to k31 above denote the session key, and the octets s0 and s1 denote the checksum of the session key octets. This encoding allows the sender to obfuscate the size of the symmetric encryption key used to encrypt the data. For example, assuming that an AES algorithm is used for the session key, the sender MAY use 21, 13, and 5 octets of padding for AES-128, AES-192, and AES-256, respectively, to provide the same number of octets, 40 total, as an input to the key wrapping method.[¶](#section-11.5-12)

In a version 6 Public Key Encrypted Session Key packet, the symmetric algorithm is not included, as described in. For example, an AES-256 session key would be composed as follows:[¶](#section-11.5-13)
...
The octets k0 to k31 above again denote the session key, and the octets s0 and s1 denote the checksum. In this case, assuming that an AES algorithm is used for the session key, the sender MAY use 22, 14, and 6 octets of padding for AES-128, AES-192, and AES-256, respectively, to provide the same number of octets, 40 total, as an input to the key wrapping method.[¶](#section-11.5-15)

The output of the method consists of two fields. The first field is the MPI containing the ephemeral key used to establish the shared secret. The second field is composed of the following two subfields:[¶](#section-11.5-16)

- One octet encoding the size in octets of the result of the key wrapping method; the value 255 is reserved for future extensions.[¶](#section-11.5-17.1.1)
- Up to 254 octets representing the result of the key wrapping method, applied to the 8-octet padded session key, as described above.[¶](#section-11.5-17.2.1)

Note that for session key sizes 128, 192, and 256 bits, the size of the result of the key wrapping method is, respectively, 32, 40, and 48 octets, unless size obfuscation is used.[¶](#section-11.5-18)

For convenience, the synopsis of the encoding method is given below; however, this section, \[\], and \[\] are the normative sources of the definition.[¶](#section-11.5-19)

- Obtain the authenticated recipient public key R [¶](#section-11.5-20.1.1)
- Generate an ephemeral, single-use key pair {v, V=vG} [¶](#section-11.5-20.2.1)
- Compute the shared point S = vR [¶](#section-11.5-20.3.1)
- m = symm\_alg\_ID || session key || checksum || pkcs5\_padding [¶](#section-11.5-20.4.1)
- curve\_OID\_len = (octet)len(curve\_OID) [¶](#section-11.5-20.5.1)
- Param = curve\_OID\_len || curve\_OID || public\_key\_alg\_ID || 03 || 01 || KDF\_hash\_ID || KEK\_alg\_ID for AESKeyWrap || 41 6E 6F 6E 79 6D 6F 75 73 20 53 65 6E 64 65 72 20 20 20 20 || recipient\_fingerprint [¶](#section-11.5-20.6.1)
- Z\_len = the key size for the KEK\_alg\_ID used with AESKeyWrap [¶](#section-11.5-20.7.1)
- Compute Z = KDF( S, Z\_len, Param ) [¶](#section-11.5-20.8.1)
- Compute C = AESKeyWrap( Z, m ) (per \[\]) [¶](#section-11.5-20.9.1)
- Wipe the memory that contained S, v, and Z to avoid leaking ephemeral secrets [¶](#section-11.5-20.10.1)
- VB = convert point V to the octet string [¶](#section-11.5-20.11.1)
- Output (MPI(VB) || len(C) || C) [¶](#section-11.5-20.12.1)

The decryption is the inverse of the method given. Note that the recipient with key pair (r,R) obtains the shared secret by calculating:[¶](#section-11.5-21)
...
ECDH keys have a hash algorithm parameter for key derivation and a symmetric algorithm for key encapsulation.[¶](#section-11.5.1-1)

For version 6 keys, the following algorithms MUST be used depending on the curve. An implementation MUST NOT generate a version 6 ECDH key over any listed curve that uses different KDF or KEK parameters. An implementation MUST NOT encrypt any message to a version 6 ECDH key over a listed curve that announces a different KDF or KEK parameter.[¶](#section-11.5.1-2)

For version 4 keys, the following algorithms SHOULD be used depending on the curve. An implementation SHOULD only use an AES algorithm as a KEK algorithm.[¶](#section-11.5.1-3)

<table><caption>:</caption><thead><tr><th rowspan="1" colspan="1">Curve</th><th rowspan="1" colspan="1">Hash Algorithm</th><th rowspan="1" colspan="1">Symmetric Algorithm</th></tr></thead><tbody><tr><td rowspan="1" colspan="1">NIST P-256</td><td rowspan="1" colspan="1">SHA2-256</td><td rowspan="1" colspan="1">AES-128</td></tr><tr><td rowspan="1" colspan="1">NIST P-384</td><td rowspan="1" colspan="1">SHA2-384</td><td rowspan="1" colspan="1">AES-192</td></tr><tr><td rowspan="1" colspan="1">NIST P-521</td><td rowspan="1" colspan="1">SHA2-512</td><td rowspan="1" colspan="1">AES-256</td></tr><tr><td rowspan="1" colspan="1">brainpoolP256r1</td><td rowspan="1" colspan="1">SHA2-256</td><td rowspan="1" colspan="1">AES-128</td></tr><tr><td rowspan="1" colspan="1">brainpoolP384r1</td><td rowspan="1" colspan="1">SHA2-384</td><td rowspan="1" colspan="1">AES-192</td></tr><tr><td rowspan="1" colspan="1">brainpoolP512r1</td><td rowspan="1" colspan="1">SHA2-512</td><td rowspan="1" colspan="1">AES-256</td></tr><tr><td rowspan="1" colspan="1">Curve25519Legacy</td><td rowspan="1" colspan="1">SHA2-256</td><td rowspan="1" colspan="1">AES-128</td></tr></tbody></table>
...
Input:[¶](#section-12.1.1-1)

k =

key modulus length in octets.[¶](#section-12.1.1-2.2)

M =

message to be encoded; an octet string of length mLen, where mLen <= k - 11.[¶](#section-12.1.1-2.4)

Output:[¶](#section-12.1.1-3)

EM =

encoded message; an octet string of length k.[¶](#section-12.1.1-4.2)

Error: "message too long".[¶](#section-12.1.1-5)

1. Length checking: If mLen > k - 11, output "message too long" and stop.[¶](#section-12.1.1-6.1.1)
2. Generate an octet string PS of length k - mLen - 3 consisting of pseudorandomly generated non-zero octets. The length of PS will be at least 8 octets.[¶](#section-12.1.1-6.2.1)
3. Concatenate PS, the message M, and other padding to form an encoded message EM of length k octets as [¶](#section-12.1.1-6.3.1)
	```
	EM = 0x00 || 0x02 || PS || 0x00 || M.
	```
	[¶](#section-12.1.1-6.3.2)
4. Output EM.[¶](#section-12.1.1-6.4.1)

#### 12.1.2.

Input:[¶](#section-12.1.2-1)

EM =

encoded message; an octet string.[¶](#section-12.1.2-2.2.1)

Output:[¶](#section-12.1.2-3)

M =

decoded message; an octet string.[¶](#section-12.1.2-4.2.1)

Error: "decryption error".[¶](#section-12.1.2-5)

To decode an EME-PKCS1\_v1\_5 message, separate the encoded message EM into an octet string PS consisting of non-zero octets and a message M as follows [¶](#section-12.1.2-6)
...
If the first octet of EM does not have hexadecimal value 0x00, the second octet of EM does not have hexadecimal value 0x02, there is no octet with hexadecimal value 0x00 to separate PS from M, or the length of PS is less than 8 octets, output "decryption error" and stop. See also regarding differences in reporting between a decryption error and a padding error.[¶](#section-12.1.2-8)

#### 12.1.3.

This encoding method is deterministic and only has an encoding operation.[¶](#section-12.1.3-1)

Input:[¶](#section-12.1.3-2)

Hash =

hash function to be used.[¶](#section-12.1.3-3.2.1)

M =

message to be encoded.[¶](#section-12.1.3-3.4.1)

emLen =

intended length of the encoded message in octets, at least tLen + 11, where tLen is the octet length of the DER encoding T of a certain value computed during the encoding operation.[¶](#section-12.1.3-3.6.1)

Output:[¶](#section-12.1.3-4)

EM =

encoded message; an octet string of length emLen.[¶](#section-12.1.3-5.2.1)

Errors: "message too long"; "intended encoded message length too short".[¶](#section-12.1.3-6)

Steps:[¶](#section-12.1.3-7)
...
Algorithm 0, "uncompressed", may only be used to denote a preference for uncompressed data. An implementation MUST NOT use algorithm 0 as the indicated compression algorithm in a Compressed Data packet (); it can use a Literal Data packet () to encode uncompressed literal data.[¶](#section-12.3.1.1-1)

#### 12.3.2.

Typically, the signer chooses which hash algorithm to use, rather than the verifier, because a signer rarely knows who is going to be verifying the signature. This preference allows a protocol based upon digital signatures ease in negotiation.[¶](#section-12.3.2-1)

Thus, if Alice is authenticating herself to Bob with a signature, it makes sense for her to use a hash algorithm that Bob's implementation uses. This preference allows Bob to state which algorithms Alice may use in his key.[¶](#section-12.3.2-2)

Since SHA2-256 is the hash algorithm that MUST be implemented, if it is not explicitly in the list, it is tacitly at the end. However, it is good form to place it there explicitly.[¶](#section-12.3.2-3)

### 12.4.

The PKCS1-v1\_5 padding scheme, used by the RSA algorithms defined in this document, is no longer recommended, and its use is deprecated by \[\]. Therefore, an implementation SHOULD NOT generate RSA keys.[¶](#section-12.4-1)

There are algorithm types for RSA Sign-Only and RSA Encrypt-Only keys. These types are deprecated in favor of the Key Flags signature subpacket. An implementation MUST NOT create such a key, but it MAY interpret it.[¶](#section-12.4-2)

An implementation MUST NOT generate RSA keys of a size less than 3072 bits. An implementation SHOULD NOT encrypt, sign, or verify using RSA keys of a size less than 3072 bits. An implementation MUST NOT encrypt, sign, or verify using RSA keys of a size less than 2048 bits. An implementation that decrypts a message using an RSA secret key of a size less than 3072 bits SHOULD generate a deprecation warning that the key is too weak for modern use.[¶](#section-12.4-3)

### 12.5.

DSA is no longer recommended. It has also been deprecated in \[\]. Therefore, an implementation MUST NOT generate DSA keys.[¶](#section-12.5-1)

An implementation MUST NOT sign or verify using DSA keys.[¶](#section-12.5-2)

### 12.6.

The PKCS1-v1\_5 padding scheme, used by the Elgamal algorithm defined in this document, is no longer recommended, and its use is deprecated by \[\]. Therefore, an implementation MUST NOT generate Elgamal keys.[¶](#section-12.6-1)

An implementation MUST NOT encrypt using Elgamal keys. An implementation that decrypts a message using an Elgamal secret key SHOULD generate a deprecation warning that the key is too weak for modern use.[¶](#section-12.6-2)
...
The Cipher Feedback (CFB) mode used in this document is defined in Section 6.3 of \[\].[¶](#section-12.9-1)

The CFB segment size `s` is equal to the block size of the cipher (i.e., n-bit CFB mode, where n is the block size used).[¶](#section-12.9-2)

### 12.10.

S2K Specifiers, Signature Subpacket Type IDs, User Attribute Subpacket Type IDs, image format IDs, and the various algorithm IDs described in all reserve the range 100 to 110 for Private and Experimental Use. Packet Type IDs reserve the range 60 to 63 for Private and Experimental Use. These are intentionally managed by the Private Use and Experimental Use policies, as described in \[\].[¶](#section-12.10-1)

However, implementations need to be careful with these and promote them to full IANA-managed parameters when they grow beyond the original, limited system.[¶](#section-12.10-2)
...
If an extension proposal does not update the Features system, it SHOULD include an explanation of why this is unnecessary. If the proposal contains neither an extension to the Features system nor an explanation of why such an extension is unnecessary, the proposal SHOULD be rejected.[¶](#section-12.11-3)
...
In winter 2005, Serge Mister and Robert Zuccherato from Entrust released a paper describing a way that the "quick check" in v1 SEIPD and SED packets can be used as an oracle to decrypt two octets of every cipher block \[\]. This check was intended for early detection of session key decryption errors, particularly to detect a wrong passphrase, since v4 SKESK packets do not include an integrity check.[¶](#section-13.4-1)

There is a danger when using the quick check if timing or error information about the check can be exposed to an attacker, particularly via an automated service that allows rapidly repeated queries.[¶](#section-13.4-2)

Disabling the quick check prevents the attack.[¶](#section-13.4-3)

For very large encrypted data whose session key is protected by a passphrase using a v4 SKESK, the quick check may be convenient to the user by informing them early that they typed the wrong passphrase. But the implementation should use the quick check with care. The recommended approach for secure and early detection of decryption failure is to encrypt data using v2 SEIPD. If the session key is public key encrypted, the quick check is not useful as the public key encryption of the session key should guarantee that it is the right session key.[¶](#section-13.4-4)

The quick check oracle attack is a particular type of attack that exploits ciphertext malleability. For information about other similar attacks, see.[¶](#section-13.4-5)
...
NOTE: the topic of interoperable human-in-the-loop key verification needs more work, which will be done in a separate document.[¶](#section-13.6-7)
...
An implementation that encounters malleable ciphertext MAY choose to release cleartext to the user if it is not encrypted using AEAD, it is known to be dealing with historic archived legacy data, and the user is aware of the risks.[¶](#section-13.7-4)
...
- Any version 1 Symmetrically Encrypted and Integrity Protected Data packet () where the internal Modification Detection Code does not validate.[¶](#section-13.7-7.3.1)
- Any version 2 Symmetrically Encrypted and Integrity Protected Data packet () where the authentication tag of any chunk fails or where there is no final zero-octet chunk.[¶](#section-13.7-7.4.1)
- Any Secret-Key packet with encrypted secret key material () where there is an integrity failure, based on the value of the secret key protection octet:[¶](#section-13.7-7.5.1)
	- Value 253 (AEAD): where the AEAD authentication tag is invalid.[¶](#section-13.7-7.5.2.1.1)
...
- When encrypting to one or more public keys:[¶](#section-13.7-9.2.1)
	- If all recipient keys indicate support for a version 2 Symmetrically Encrypted and Integrity Protected Data packet in their Features signature subpacket (), if all recipient keys are version 6 keys without a Features signature subpacket, or the implementation can otherwise infer that all recipients support v2 SEIPD packets, the implementation SHOULD encrypt using a v2 SEIPD packet.[¶](#section-13.7-9.2.2.1.1)
		- If one of the recipients does not support v2 SEIPD packets, then the message generator MAY use a v1 SEIPD packet instead.[¶](#section-13.7-9.2.2.2.1)
- Passphrase-protected secret key material in a version 6 Secret Key or version 6 Secret Subkey packet SHOULD be protected with AEAD encryption (S2K usage octet 253) unless it will be transferred to an implementation that is known to not support AEAD. An implementation should be aware that, in scenarios where an attacker has write access to encrypted private keys, CFB-encrypted keys (S2K usage octet 254 or 255) are vulnerable to corruption attacks that can cause leakage of secret data when the secret key is used \[\] \[\].[¶](#section-13.7-9.3.1)

Implementers should implement AEAD (v2 SEIPD and S2K usage octet 253) promptly and encourage its spread.[¶](#section-13.7-10)

Users are RECOMMENDED to migrate to AEAD.[¶](#section-13.7-11)

### 13.8.

The salted key derivation of v2 SEIPD packets () allows the recipient of an encrypted message to reply to the sender and all other recipients without needing their public keys but by using the same v6 PKESK packets it received and a different random salt value. This ensures a secure mechanism on the cryptographic level that enables the use of message encryption in cases where a sender does not have a copy of an encryption-capable certificate for one or more participants in the conversation and thus can enhance the overall security of an application. However, care must be taken when using this mechanism not to create security vulnerabilities, such as the following:[¶](#section-13.8-1)

- Replying to only a subset of the original recipients and the original sender by use of the session-key-reuse feature would mean that the remaining recipients (including the sender) of the original message could read the encrypted reply message, too.[¶](#section-13.8-2.1.1)
- Adding a further recipient to the reply that is encrypted using the session-key-reuse feature gives that further recipient also cryptographic access to the original message that is being replied to (and potentially to a longer history of previous messages).[¶](#section-13.8-2.2.1)
- A modification of the list of recipients addressed in the above points also needs to be safeguarded when a message is initially composed as a reply with session-key reuse but then is stored (e.g., as a draft) and later reopened for further editing and to be finally sent.[¶](#section-13.8-2.3.1)
- There is the potential threat that an attacker with network or mailbox access, who is at the same time a recipient of the original message, silently removes themselves from the message before the victim's client receives it. The victim's client that then uses the mechanism for replying with session-key reuse would unknowingly compose an encrypted message that could be read by the attacker. Implementations are encouraged to use the Intended Recipient Fingerprint subpacket () when composing messages and checking the consistency of the set of recipients of a message before replying to it with session-key reuse.[¶](#section-13.8-2.4.1)
...
A keyholder, Alice, may wish to designate a third party to be able to revoke her own key.[¶](#section-13.9-1)

The preferred way for Alice to do this is to produce a specific Revocation Signature (Signature Type ID 0x20, 0x28, or 0x30) and distribute it securely to a preferred revoker who can hold it in escrow. The preferred revoker can then publish the escrowed Revocation Signature at whatever time is deemed appropriate rather than generating the Revocation Signature themselves.[¶](#section-13.9-2)

There are multiple advantages of using an escrowed Revocation Signature over the deprecated Revocation Key subpacket ():[¶](#section-13.9-3)
...
A Revocation Signature may also be split up into shares and distributed among multiple parties, requiring some subset of those parties to collaborate before the escrowed Revocation Signature is recreated.[¶](#section-13.9-5)
...
- In entirely private data, such as asymmetric key generation.[¶](#section-13.10-3.3.1)

With a properly functioning CSPRNG, this range of visibility does not present a security problem, as it is not feasible to determine the CSPRNG state from its output. However, with a broken CSPRNG, it may be possible for an attacker to use visible output to determine the CSPRNG internal state and thereby predict less-visible data like keying material, as documented in \[\].[¶](#section-13.10-4)

An implementation can provide extra security against this form of attack by using separate CSPRNGs to generate random data with different levels of visibility.[¶](#section-13.10-5)
...
Some subpackets are only useful when they are in the hashed section, and an implementation SHOULD ignore them when they are found with unknown provenance in the unhashed section. For example, a Preferred AEAD Ciphersuites subpacket () in a Direct Key self-signature indicates the preferences of the keyholder when encrypting v2 SEIPD data to the key. An implementation that observes such a subpacket found in the unhashed section would open itself to an attack where the recipient's certificate is tampered with to encourage the use of a specific cipher or mode of operation.[¶](#section-13.13-3)
...
This document obsoletes \[\]. IANA has updated all registration information that references \[\] to reference this RFC instead.[¶](#section-15-1)
...
When registering a new symmetric cipher with a block size of 64 or 128 bits and a key size that is a multiple of 64 bits, no new considerations are needed.[¶](#section-15.6.3.2-1)

If the new cipher has a different block size, there needs to be additional documentation describing how to use the cipher in CFB mode.[¶](#section-15.6.3.2-2)

If the new cipher has an unusual key size, then padding needs to be considered for X25519 and X448 key wrapping, which currently needs no padding.[¶](#section-15.6.3.2-3)
...
NIST, "Advanced Encryption Standard (AES)", Updated May 2023, FIPS PUB 197, DOI 10.6028/NIST.FIPS.197-upd1, November 2001, < [https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.197-upd1.pdf](https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.197-upd1.pdf) >.

\[BLOWFISH\]

Schneier, B., "Description of a New Variable-Length Key, 64-Bit Block Cipher (Blowfish)", Fast Software Encryption, Cambridge Security Workshop Proceedings, pp. 191-204, December 1993, < [https://www.schneier.com/academic/archives/1994/09/description\_of\_a\_new.html](https://www.schneier.com/academic/archives/1994/09/description_of_a_new.html) >.

\[BZ2\]

bzip2, "bzip2 and libbzip2", 2010, < [https://sourceware.org/bzip2/](https://sourceware.org/bzip2/) >.

\[EAX\]

Bellare, M., Rogaway, P., and D. Wagner, "A Conventional Authenticated-Encryption Mode", April 2003, < [https://seclab.cs.ucdavis.edu/papers/eax.pdf](https://seclab.cs.ucdavis.edu/papers/eax.pdf) >.

\[ELGAMAL\]

Elgamal, T., "A Public Key Cryptosystem and a Signature Scheme Based on Discrete Logarithms", IEEE Transactions on Information Theory, Vol. 31, Issue 4, pp. 469-472, DOI 10.1109/TIT.1985.1057074, July 1985, < [https://doi.org/10.1109/TIT.1985.1057074](https://doi.org/10.1109/TIT.1985.1057074) >.
...
Lai, X. and J. L. Massey, "A Proposal for a New Block Encryption Standard", Advances in Cryptology - EUROCRYPT '90, Vol. 473, pp. 389-404, DOI 10.1007/3-540-46877-3\_35, January 1991, < [https://link.springer.com/chapter/10.1007/3-540-46877-3\_35](https://link.springer.com/chapter/10.1007/3-540-46877-3_35) >.

\[ISO10646\]

ISO, "Information technology - Universal coded character set (UCS)", ISO/IEC 10646:2020, December 2020, < [https://www.iso.org/standard/76835.html](https://www.iso.org/standard/76835.html) >.

\[JFIF\]

ITU-T, "Information technology - Digital compression and coding of continuous-tone still images: JPEG File Interchange Format (JFIF)", Recommendation ITU-T T.871, May 2011, < [https://www.itu.int/rec/T-REC-T.871-201105-I](https://www.itu.int/rec/T-REC-T.871-201105-I) >.

\[RFC1321\]

Rivest, R., "The MD5 Message-Digest Algorithm", RFC 1321, DOI 10.17487/RFC1321, April 1992, < [https://www.rfc-editor.org/info/rfc1321](https://www.rfc-editor.org/info/rfc1321) >.

\[RFC1950\]

Deutsch, P. and J. Gailly, "ZLIB Compressed Data Format Specification version 3.3", RFC 1950, DOI 10.17487/RFC1950, May 1996, < [https://www.rfc-editor.org/info/rfc1950](https://www.rfc-editor.org/info/rfc1950) >.
...
Adams, C., "The CAST-128 Encryption Algorithm", RFC 2144, DOI 10.17487/RFC2144, May 1997, < [https://www.rfc-editor.org/info/rfc2144](https://www.rfc-editor.org/info/rfc2144) >.

\[RFC2822\]

Resnick, P., Ed., "Internet Message Format", RFC 2822, DOI 10.17487/RFC2822, April 2001, < [https://www.rfc-editor.org/info/rfc2822](https://www.rfc-editor.org/info/rfc2822) >.
...
Schaad, J. and R. Housley, "Advanced Encryption Standard (AES) Key Wrap Algorithm", RFC 3394, DOI 10.17487/RFC3394, September 2002, < [https://www.rfc-editor.org/info/rfc3394](https://www.rfc-editor.org/info/rfc3394) >.
...
Matsui, M., Nakajima, J., and S. Moriai, "A Description of the Camellia Encryption Algorithm", RFC 3713, DOI 10.17487/RFC3713, April 2004, < [https://www.rfc-editor.org/info/rfc3713](https://www.rfc-editor.org/info/rfc3713) >.
...
Eastlake 3rd, D. and T. Hansen, "US Secure Hash Algorithms (SHA and SHA-based HMAC and HKDF)", RFC 6234, DOI 10.17487/RFC6234, May 2011, < [https://www.rfc-editor.org/info/rfc6234](https://www.rfc-editor.org/info/rfc6234) >.
...
NIST, "Recommendation for the Triple Data Encryption Algorithm (TDEA) Block Cipher", NIST Special Publication 800-67 Revision 2, DOI 10.6028/NIST.SP.800-67r2, November 2017, < [https://nvlpubs.nist.gov/nistpubs/SpecialPublications/NIST.SP.800-67r2.pdf](https://nvlpubs.nist.gov/nistpubs/SpecialPublications/NIST.SP.800-67r2.pdf) >.

\[TWOFISH\]

Schneier, B., Kelsey, J., Whiting, D., Wagner, D., Hall, C., and N. Ferguson, "Twofish: A 128-Bit Block Cipher", June 1998, < [https://www.schneier.com/wp-content/uploads/2016/02/paper-twofish-paper.pdf](https://www.schneier.com/wp-content/uploads/2016/02/paper-twofish-paper.pdf) >.
...
Bleichenbacher, D., "Chosen Ciphertext Attacks Against Protocols Based on the RSA Encryption Standard PKCS #1", CRYPTO '98: International Cryptology Conference Proceedings, Vol. 1462, pp. 1-12, August 1998, < [http://archiv.infsec.ethz.ch/education/fs08/secsem/Bleichenbacher98.pdf](http://archiv.infsec.ethz.ch/education/fs08/secsem/Bleichenbacher98.pdf) >.

\[C99\]

ISO, "Information technology - Programming languages: C", ISO/IEC 9899:2018, June 2018, < [https://www.iso.org/standard/74528.html](https://www.iso.org/standard/74528.html) >.

\[CHECKOWAY\]

Checkoway, S., Maskiewicz, J., Garman, C., Fried, J., Cohney, S., Green, M., Heninger, N., Weinmann, RP., Rescorla, E., and H. Shacham, "A Systematic Analysis of the Juniper Dual EC Incident", Proceedings of the 2016 ACM SIGSAC Conference on Computer and Communications Security, DOI 10.1145/2976749.2978395, October 2016, < [https://doi.org/10.1145/2976749.2978395](https://doi.org/10.1145/2976749.2978395) >.
...
RFC Errata, Erratum ID 2199, RFC 4880, < [https://www.rfc-editor.org/errata/eid2199](https://www.rfc-editor.org/errata/eid2199) >.

\[Errata-2200\]

RFC Errata, Erratum ID 2200, RFC 4880, < [https://www.rfc-editor.org/errata/eid2200](https://www.rfc-editor.org/errata/eid2200) >.

\[Errata-2206\]

RFC Errata, Erratum ID 2206, RFC 4880, < [https://www.rfc-editor.org/errata/eid2206](https://www.rfc-editor.org/errata/eid2206) >.

\[Errata-2208\]

RFC Errata, Erratum ID 2208, RFC 4880, < [https://www.rfc-editor.org/errata/eid2208](https://www.rfc-editor.org/errata/eid2208) >.

\[Errata-2214\]

RFC Errata, Erratum ID 2214, RFC 4880, < [https://www.rfc-editor.org/errata/eid2214](https://www.rfc-editor.org/errata/eid2214) >.

\[Errata-2216\]

RFC Errata, Erratum ID 2216, RFC 4880, < [https://www.rfc-editor.org/errata/eid2216](https://www.rfc-editor.org/errata/eid2216) >.

\[Errata-2219\]

RFC Errata, Erratum ID 2219, RFC 4880, < [https://www.rfc-editor.org/errata/eid2219](https://www.rfc-editor.org/errata/eid2219) >.

\[Errata-2222\]

RFC Errata, Erratum ID 2222, RFC 4880, < [https://www.rfc-editor.org/errata/eid2222](https://www.rfc-editor.org/errata/eid2222) >.

\[Errata-2226\]

RFC Errata, Erratum ID 2226, RFC 4880, < [https://www.rfc-editor.org/errata/eid2226](https://www.rfc-editor.org/errata/eid2226) >.

\[Errata-2234\]

RFC Errata, Erratum ID 2234, RFC 4880, < [https://www.rfc-editor.org/errata/eid2234](https://www.rfc-editor.org/errata/eid2234) >.

\[Errata-2235\]

RFC Errata, Erratum ID 2235, RFC 4880, < [https://www.rfc-editor.org/errata/eid2235](https://www.rfc-editor.org/errata/eid2235) >.

\[Errata-2236\]

RFC Errata, Erratum ID 2236, RFC 4880, < [https://www.rfc-editor.org/errata/eid2236](https://www.rfc-editor.org/errata/eid2236) >.

\[Errata-2238\]

RFC Errata, Erratum ID 2238, RFC 4880, < [https://www.rfc-editor.org/errata/eid2238](https://www.rfc-editor.org/errata/eid2238) >.

\[Errata-2240\]

RFC Errata, Erratum ID 2240, RFC 4880, < [https://www.rfc-editor.org/errata/eid2240](https://www.rfc-editor.org/errata/eid2240) >.

\[Errata-2242\]

RFC Errata, Erratum ID 2242, RFC 4880, < [https://www.rfc-editor.org/errata/eid2242](https://www.rfc-editor.org/errata/eid2242) >.

\[Errata-2243\]

RFC Errata, Erratum ID 2243, RFC 4880, < [https://www.rfc-editor.org/errata/eid2243](https://www.rfc-editor.org/errata/eid2243) >.

\[Errata-2270\]

RFC Errata, Erratum ID 2270, RFC 4880, < [https://www.rfc-editor.org/errata/eid2270](https://www.rfc-editor.org/errata/eid2270) >.

\[Errata-2271\]

RFC Errata, Erratum ID 2271, RFC 4880, < [https://www.rfc-editor.org/errata/eid2271](https://www.rfc-editor.org/errata/eid2271) >.

\[Errata-3298\]

RFC Errata, Erratum ID 3298, RFC 4880, < [https://www.rfc-editor.org/errata/eid3298](https://www.rfc-editor.org/errata/eid3298) >.

\[Errata-5491\]

RFC Errata, Erratum ID 5491, RFC 4880, < [https://www.rfc-editor.org/errata/eid5491](https://www.rfc-editor.org/errata/eid5491) >.

\[Errata-7545\]

RFC Errata, Erratum ID 7545, RFC 4880, < [https://www.rfc-editor.org/errata/eid7545](https://www.rfc-editor.org/errata/eid7545) >.

\[Errata-7889\]

RFC Errata, Erratum ID 7889, RFC 4880, < [https://www.rfc-editor.org/errata/eid7889](https://www.rfc-editor.org/errata/eid7889) >.

\[HASTAD\]

Hastad, J., "Solving Simultaneous Modular Equations of Low Degree", DOI 10.1137/0217019, April 1988, < [https://doi.org/10.1137/0217019](https://doi.org/10.1137/0217019) >.

\[JKS02\]

Jallad, K., Katz, J., and B. Schneier, "Implementation of Chosen-Ciphertext Attacks against PGP and GnuPG", DOI 0.1007/3-540-45811-5\_7, September 2002, < [https://www.schneier.com/academic/archives/2002/01/implementation\_of\_ch.html](https://www.schneier.com/academic/archives/2002/01/implementation_of_ch.html) >.

\[KOBLITZ\]

Koblitz, N., "A course in number theory and cryptography", Chapter VI: Elliptic Curves, DOI 10.2307/3618498, 1997, < [https://doi.org/10.2307/3618498](https://doi.org/10.2307/3618498) >.
...
The Open Group, "The Open Group Base Specifications", 'pax - portable archive interchange', Issue 7, 2018 Edition, IEEE Std 1003.1-2017, 2018, < [https://pubs.opengroup.org/onlinepubs/9699919799/utilities/pax.html](https://pubs.opengroup.org/onlinepubs/9699919799/utilities/pax.html) >.

\[PSSLR17\]

Poddebniak, D., Somorovsky, J., Schinzel, S., Lochter, M., and P. Rösler, "Attacking Deterministic Signature Schemes using Fault Attacks", Cryptology ePrint Archive, Paper 2017/1014, October 2017, < [https://eprint.iacr.org/2017/1014](https://eprint.iacr.org/2017/1014) >.

\[REGEX\]

regex, "Henry Spencer's regular expression libraries", < [https://garyhouston.github.io/regex/](https://garyhouston.github.io/regex/) >.

\[RFC1991\]

Atkins, D., Stallings, W., and P. Zimmermann, "PGP Message Exchange Formats", RFC 1991, DOI 10.17487/RFC1991, August 1996, < [https://www.rfc-editor.org/info/rfc1991](https://www.rfc-editor.org/info/rfc1991) >.
...
Lochter, M. and J. Merkle, "Elliptic Curve Cryptography (ECC) Brainpool Standard Curves and Curve Generation", RFC 5639, DOI 10.17487/RFC5639, March 2010, < [https://www.rfc-editor.org/info/rfc5639](https://www.rfc-editor.org/info/rfc5639) >.

\[RFC5869\]

Krawczyk, H. and P. Eronen, "HMAC-based Extract-and-Expand Key Derivation Function (HKDF)", RFC 5869, DOI 10.17487/RFC5869, May 2010, < [https://www.rfc-editor.org/info/rfc5869](https://www.rfc-editor.org/info/rfc5869) >.

\[RFC6090\]

McGrew, D., Igoe, K., and M. Salter, "Fundamental Elliptic Curve Cryptography Algorithms", RFC 6090, DOI 10.17487/RFC6090, February 2011, < [https://www.rfc-editor.org/info/rfc6090](https://www.rfc-editor.org/info/rfc6090) >.
...
Standards for Efficient Cryptography Group, "SEC 1: Elliptic Curve Cryptography",, May 2009, < [https://www.secg.org/sec1-v2.pdf](https://www.secg.org/sec1-v2.pdf) >.

\[SHA1CD\]

"sha1collisiondetection", commit b4a7b0b, December 2020, < [https://github.com/cr-marcstevens/sha1collisiondetection](https://github.com/cr-marcstevens/sha1collisiondetection) >.

\[SHAMBLES\]

Leurent, G. and T. Peyrin, "Sha-1 is a shambles: first chosen-prefix collision on sha-1 and application to the PGP web of trust", August 2020, < [https://dl.acm.org/doi/abs/10.5555/3489212.3489316/](https://dl.acm.org/doi/abs/10.5555/3489212.3489316/) >.
...
Stevens, M., "Counter-cryptanalysis", Cryptology ePrint Archive, Paper 2013/358, June 2013, < [https://eprint.iacr.org/2013/358](https://eprint.iacr.org/2013/358) >.

\[UNIFIED-DIFF\]

Free Software Foundation, "Comparing and Merging Files", 'Detailed Description of Unified Format', Section 2.2.2.2, January 2021, < [https://www.gnu.org/software/diffutils/manual/html\_node/Detailed-Unified.html](https://www.gnu.org/software/diffutils/manual/html_node/Detailed-Unified.html) >.

\[USENIX-STUDY\]

Dechand, S., Schürmann, D., Busse, K., Acar, Y., Fahl, S., and M. Smith, "An Empirical Study of Textual Key-Fingerprint Representations", ISBN 978-1-931971-32-4, August 2016, < [https://www.usenix.org/system/files/conference/usenixsecurity16/sec16\_paper\_dechand.pdf](https://www.usenix.org/system/files/conference/usenixsecurity16/sec16_paper_dechand.pdf) >.

## Appendix A.

To help with the implementation of this specification, a set of non-normative examples follow.[¶](#appendix-A-1)

### A.1.

The secret key used for this example is:[¶](#appendix-A.1-1)
...
The algorithm-specific input parameters without the MPI length headers are:[¶](#appendix-A.1-5)
...
The entire Public Key packet is thus:[¶](#appendix-A.1-7)
...
The same packet represented in ASCII-armored form is:[¶](#appendix-A.1-9)
...
Using the SHA2-256 hash algorithm yields the digest:[¶](#appendix-A.2-3)
...
which is fed into the EdDSA signature function and yields the following signature:[¶](#appendix-A.2-5)
...
The entire Signature packet is thus:[¶](#appendix-A.2-7)
...
The same packet represented in ASCII-armored form is:[¶](#appendix-A.2-9)
...
Here is a Transferable Public Key consisting of:[¶](#appendix-A.3-1)

- A version 6 Ed25519 Public Key packet [¶](#appendix-A.3-2.1.1)
- A version 6 Direct Key self-signature [¶](#appendix-A.3-2.2.1)
- A version 6 X25519 Public Subkey packet [¶](#appendix-A.3-2.3.1)
- A version 6 Subkey Binding signature [¶](#appendix-A.3-2.4.1)

The primary key has the following fingerprint:[¶](#appendix-A.3-3)

`CB186C4F0609A697E4D52DFA6C722B0C1F1E27C18A56708F6525EC27BAD9ACC9` [¶](#appendix-A.3-4)

The subkey has the following fingerprint:[¶](#appendix-A.3-5)

`12C83F1E706F6308FE151A417743A1F033790E93E9978488D1DB378DA9930885` [¶](#appendix-A.3-6)
...
The corresponding Transferable Secret Key can be found in.[¶](#appendix-A.3-8)

#### A.3.1.

The Direct Key self-signature in the certificate in is made over the following sequence of data:[¶](#appendix-A.3.1-1)
...
The same data, broken out by octet and semantics, is:[¶](#appendix-A.3.1-3)
...
The Subkey Binding signature in is made over the following sequence of data:[¶](#appendix-A.3.1-5)
...
The same data, broken out by octet and semantics, is:[¶](#appendix-A.3.1-7)
...
Here is a Transferable Secret Key consisting of:[¶](#appendix-A.4-1)

- A version 6 Ed25519 Secret Key packet [¶](#appendix-A.4-2.1.1)
- A version 6 Direct Key self-signature [¶](#appendix-A.4-2.2.1)
- A version 6 X25519 Secret Subkey packet [¶](#appendix-A.4-2.3.1)
- A version 6 Subkey Binding signature [¶](#appendix-A.4-2.4.1)
...
The corresponding Transferable Public Key can be found in.[¶](#appendix-A.4-4)
...
The S2K-derived material for the primary key is:[¶](#appendix-A.5.1-1)
...
After HKDF, the symmetric key used for AEAD encryption of the primary key is:[¶](#appendix-A.5.1-3)
...
The additional data for AEAD for the primary key is:[¶](#appendix-A.5.1-5)
...
The S2K-derived key material for the subkey is:[¶](#appendix-A.5.2-1)
...
After HKDF, the symmetric key used for AEAD encryption of the subkey is:[¶](#appendix-A.5.2-3)
...
The additional data for AEAD for the subkey is:[¶](#appendix-A.5.2-5)
...
Here is a signed message that uses the Cleartext Signature Framework (). It can be verified with the certificate from.[¶](#appendix-A.6-1)

Note that this message makes use of dash-escaping () due to its contents.[¶](#appendix-A.6-2)
...
The Signature packet here is:[¶](#appendix-A.6-4)
...
The signature is made over the following data:[¶](#appendix-A.6-6)
...
The same data, broken out by octet and semantics, is:[¶](#appendix-A.6-8)
...
The calculated SHA2-512 hash digest over this data is:[¶](#appendix-A.6-10)
...
This is the same message and signature as in but as an inline-signed message. The hashed data is exactly the same, and all intermediate values and annotated hex dumps are also applicable.[¶](#appendix-A.7-1)
...
This example encrypts the cleartext string `Hello, world!` for the sample cert (see ), using AES-128 with AEAD-OCB encryption.[¶](#appendix-A.8-1)

#### A.8.1.

This packet contains the following series of octets:[¶](#appendix-A.8.1-1)
...
The same data, broken out by octet and semantics, is:[¶](#appendix-A.8.1-3)
...
Ephemeral key:[¶](#appendix-A.8.2-1)
...
This ephemeral key is derived from the following ephemeral secret key material, which is never placed on the wire:[¶](#appendix-A.8.2-3)
...
Public key from the target certificate (see ):[¶](#appendix-A.8.2-5)
...
The corresponding long-lived X25519 private key material (see ):[¶](#appendix-A.8.2-7)
...
Decrypted session key:[¶](#appendix-A.8.2-13)
...
This packet contains the following series of octets:[¶](#appendix-A.8.3-1)
...
The same data, broken out by octet and semantics, is:[¶](#appendix-A.8.3-3)
...
Starting AEAD-OCB decryption of data, using the session key.[¶](#appendix-A.8.4-1)

HKDF info:[¶](#appendix-A.8.4-2)
...
Initialization vector:[¶](#appendix-A.8.4-8)
...
Chunk #0:[¶](#appendix-A.8.4-10)

Nonce:[¶](#appendix-A.8.4-11)
...
Additional authenticated data:[¶](#appendix-A.8.4-13)
...
Encrypted data chunk:[¶](#appendix-A.8.4-15)
...
Decrypted chunk #0.[¶](#appendix-A.8.4-17)

Literal Data packet with the string contents `Hello, world!`:[¶](#appendix-A.8.4-18)
...
Padding packet:[¶](#appendix-A.8.4-20)
...
Authenticating final tag:[¶](#appendix-A.8.4-22)

Final nonce:[¶](#appendix-A.8.4-23)
...
Final additional authenticated data:[¶](#appendix-A.8.4-25)
...
This example encrypts the cleartext string `Hello, world!` with the passphrase `password`, using AES-128 with AEAD-EAX encryption.[¶](#appendix-A.9-1)

#### A.9.1.

This packet contains the following series of octets:[¶](#appendix-A.9.1-1)
...
The same data, broken out by octet and semantics, is:[¶](#appendix-A.9.1-3)
...
The derived key is:[¶](#appendix-A.9.2-1)
...
Authenticated Data:[¶](#appendix-A.9.2-7)
...
Decrypted session key:[¶](#appendix-A.9.2-11)
...
This packet contains the following series of octets:[¶](#appendix-A.9.3-1)
...
The same data, broken out by octet and semantics, is:[¶](#appendix-A.9.3-3)
...
Starting AEAD-EAX decryption of data, using the session key.[¶](#appendix-A.9.4-1)

HKDF info:[¶](#appendix-A.9.4-2)
...
Initialization vector:[¶](#appendix-A.9.4-8)
...
Chunk #0:[¶](#appendix-A.9.4-10)

Nonce:[¶](#appendix-A.9.4-11)
...
Additional authenticated data:[¶](#appendix-A.9.4-13)
...
Decrypted chunk #0.[¶](#appendix-A.9.4-15)

Literal Data packet with the string contents `Hello, world!`:[¶](#appendix-A.9.4-16)
...
Padding packet:[¶](#appendix-A.9.4-18)
...
Authenticating final tag:[¶](#appendix-A.9.4-20)

Final nonce:[¶](#appendix-A.9.4-21)
...
Final additional authenticated data:[¶](#appendix-A.9.4-23)
...
This example encrypts the cleartext string `Hello, world!` with the passphrase `password`, using AES-128 with AEAD-OCB encryption.[¶](#appendix-A.10-1)

#### A.10.1.

This packet contains the following series of octets:[¶](#appendix-A.10.1-1)
...
The same data, broken out by octet and semantics, is:[¶](#appendix-A.10.1-3)
...
The derived key is:[¶](#appendix-A.10.2-1)
...
Authenticated Data:[¶](#appendix-A.10.2-7)
...
Decrypted session key:[¶](#appendix-A.10.2-11)
...
This packet contains the following series of octets:[¶](#appendix-A.10.3-1)
...
The same data, broken out by octet and semantics, is:[¶](#appendix-A.10.3-3)
...
Starting AEAD-OCB decryption of data, using the session key.[¶](#appendix-A.10.4-1)

HKDF info:[¶](#appendix-A.10.4-2)
...
Initialization vector:[¶](#appendix-A.10.4-8)
...
Chunk #0:[¶](#appendix-A.10.4-10)

Nonce:[¶](#appendix-A.10.4-11)
...
Additional authenticated data:[¶](#appendix-A.10.4-13)
...
Decrypted chunk #0.[¶](#appendix-A.10.4-15)

Literal Data packet with the string contents `Hello, world!`:[¶](#appendix-A.10.4-16)
...
Padding packet:[¶](#appendix-A.10.4-18)
...
Authenticating final tag:[¶](#appendix-A.10.4-20)

Final nonce:[¶](#appendix-A.10.4-21)
...
Final additional authenticated data:[¶](#appendix-A.10.4-23)
...
This example encrypts the cleartext string `Hello, world!` with the passphrase `password`, using AES-128 with AEAD-GCM encryption.[¶](#appendix-A.11-1)

#### A.11.1.

This packet contains the following series of octets:[¶](#appendix-A.11.1-1)
...
The same data, broken out by octet and semantics, is:[¶](#appendix-A.11.1-3)
...
The derived key is:[¶](#appendix-A.11.2-1)
...
Authenticated Data:[¶](#appendix-A.11.2-7)
...
Decrypted session key:[¶](#appendix-A.11.2-11)
...
This packet contains the following series of octets, is:[¶](#appendix-A.11.3-1)
...
The same data, broken out by octet and semantics, is:[¶](#appendix-A.11.3-3)
...
Starting AEAD-GCM decryption of data, using the session key.[¶](#appendix-A.11.4-1)

HKDF info:[¶](#appendix-A.11.4-2)
...
Initialization vector:[¶](#appendix-A.11.4-8)
...
Chunk #0:[¶](#appendix-A.11.4-10)

Nonce:[¶](#appendix-A.11.4-11)
...
Additional authenticated data:[¶](#appendix-A.11.4-13)
...
Decrypted chunk #0.[¶](#appendix-A.11.4-15)

Literal Data packet with the string contents `Hello, world!`:[¶](#appendix-A.11.4-16)
...
Padding packet:[¶](#appendix-A.11.4-18)
...
Authenticating final tag:[¶](#appendix-A.11.4-20)

Final nonce:[¶](#appendix-A.11.4-21)
...
Final additional authenticated data:[¶](#appendix-A.11.4-23)
...
This subsection offers a concise, non-normative summary of the substantial additions to and departures from \[\] and \[\]. It is intended to help implementers who are augmenting an existing implementation from those specifications to comply with this specification. Cryptographic algorithms marked with "MTI" are mandatory to implement.[¶](#appendix-B-1)

- Public Key Signing Algorithms:[¶](#appendix-B-2.1.1)
	- Ed25519 (Sections and ) -- MTI [¶](#appendix-B-2.1.2.1.1)
		- Ed448 (Sections and ) [¶](#appendix-B-2.1.2.2.1)
		- EdDSALegacy with Ed25519Legacy (Sections and ) [¶](#appendix-B-2.1.2.3.1)
		- ECDSA with Brainpool curves () [¶](#appendix-B-2.1.2.4.1)
- Public Key Encryption Algorithms:[¶](#appendix-B-2.2.1)
	- X25519 (Sections and ) -- MTI [¶](#appendix-B-2.2.2.1.1)
		- X448 (Sections and ) [¶](#appendix-B-2.2.2.2.1)
		- ECDH with Curve25519Legacy () [¶](#appendix-B-2.2.2.3.1)
		- ECDH with Brainpool curves () [¶](#appendix-B-2.2.2.4.1)
- AEAD Encryption:[¶](#appendix-B-2.3.1)
	- V2 SEIPD () [¶](#appendix-B-2.3.2.1.1)
		- AEAD modes:[¶](#appendix-B-2.3.2.2.1)
		- OCB mode () -- MTI [¶](#appendix-B-2.3.2.2.2.1.1)
				- EAX mode () [¶](#appendix-B-2.3.2.2.2.2.1)
				- GCM mode () [¶](#appendix-B-2.3.2.2.2.3.1)
		- V6 PKESK () [¶](#appendix-B-2.3.2.3.1)
		- V6 SKESK () [¶](#appendix-B-2.3.2.4.1)
		- Features signature subpacket: add flag for v2 SEIPD () [¶](#appendix-B-2.3.2.5.1)
		- Signature Subpacket: Preferred AEAD Ciphersuites () [¶](#appendix-B-2.3.2.6.1)
		- Secret key encryption: AEAD "S2K usage octet" (Sections and ) [¶](#appendix-B-2.3.2.7.1)
- Version 6 Keys and Signatures:[¶](#appendix-B-2.4.1)
	- Version 6 Public Keys () [¶](#appendix-B-2.4.2.1.1)
		- Version 6 Fingerprint and Key ID () [¶](#appendix-B-2.4.2.2.1)
		- Version 6 Secret Keys () [¶](#appendix-B-2.4.2.3.1)
		- Version 6 Signatures () [¶](#appendix-B-2.4.2.4.1)
		- Version 6 One-Pass Signatures () [¶](#appendix-B-2.4.2.5.1)
- Certificate (Transferable Public Key) Structure:[¶](#appendix-B-2.5.1)
...
- Message Structure: Packet Criticality () [¶](#appendix-B-2.10.1)
- Deprecations:[¶](#appendix-B-2.11.1)
	- Public Key Algorithms:[¶](#appendix-B-2.11.2.1.1)
		- Avoid RSA weak keys () [¶](#appendix-B-2.11.2.1.2.1.1)
				- Avoid DSA () [¶](#appendix-B-2.11.2.1.2.2.1)
				- Avoid ElGamal (Sections and ) [¶](#appendix-B-2.11.2.1.2.3.1)
				- For Version 6 Keys: Avoid EdDSA25519Legacy and Curve25519Legacy () [¶](#appendix-B-2.11.2.1.2.4.1)
		- Digest Algorithms:[¶](#appendix-B-2.11.2.2.1)
		- Avoid MD5, SHA1, and RIPEMD160 () [¶](#appendix-B-2.11.2.2.2.1.1)
		- Symmetric Key Algorithms:[¶](#appendix-B-2.11.2.3.1)
		- Avoid IDEA, TripleDES, and CAST5 () [¶](#appendix-B-2.11.2.3.2.1.1)
		- S2K Specifier:[¶](#appendix-B-2.11.2.4.1)
		- Avoid Simple S2K () [¶](#appendix-B-2.11.2.4.2.1.1)
		- Secret Key Protections (a.k.a. S2K Usage):[¶](#appendix-B-2.11.2.5.1)
		- Avoid MalleableCFB () [¶](#appendix-B-2.11.2.5.2.1.1)
		- Packet Types:[¶](#appendix-B-2.11.2.6.1)
		- Avoid Symmetrically Encrypted Data (Sections and ) [¶](#appendix-B-2.11.2.6.2.1.1)
		- Literal Data Packet Metadata:[¶](#appendix-B-2.11.2.7.1)
		- Avoid Filename and Date fields () [¶](#appendix-B-2.11.2.7.2.1.1)
				- Avoid Special `_CONSOLE` "filename" () [¶](#appendix-B-2.11.2.7.2.2.1)
		- Packet Versions:[¶](#appendix-B-2.11.2.8.1)
		- Avoid Version 3 Public Keys () [¶](#appendix-B-2.11.2.8.2.1.1)
				- Avoid Version 3 Signatures () [¶](#appendix-B-2.11.2.8.2.2.1)
		- Signature Types:[¶](#appendix-B-2.11.2.9.1)
		- Avoid Reserved Signature Type ID 0xFF (Sections and ) [¶](#appendix-B-2.11.2.9.2.1.1)
		- Signature Subpackets:[¶](#appendix-B-2.11.2.10.1)
		- For Version 6 Signatures: Avoid Issuer Key ID () [¶](#appendix-B-2.11.2.10.2.1.1)
				- Avoid Revocation Key () [¶](#appendix-B-2.11.2.10.2.2.1)
		- ASCII Armor:[¶](#appendix-B-2.11.2.11.1)
		- Ignore; do not emit CRC () [¶](#appendix-B-2.11.2.11.2.1.1)
				- Do not emit "Version" Armor Header () [¶](#appendix-B-2.11.2.11.2.2.1)
		- Cleartext Signature Framework:[¶](#appendix-B-2.11.2.12.1)
		- Ignore; avoid emitting unnecessary Hash: headers () [¶](#appendix-B-2.11.2.12.2.1.1)
				- Reject Cleartext Signature Framework signatures with invalid Hash: headers () or any other Armor Header () [¶](#appendix-B-2.11.2.12.2.2.1)
...
- "Packet Tag" was used to refer to the Packet Type ID () or sometimes to the encoded Packet Type ID ().[¶](#appendix-B.1-3.7.1)

## Appendix C.

The following verified errata have been incorporated or are otherwise resolved by this document:[¶](#appendix-C-1)

- \[\] - S2K hash/cipher octet correction [¶](#appendix-C-2.1.1)
- \[\] - No implicit use of IDEA correction [¶](#appendix-C-2.2.1)
- \[\] - PKESK acronym expansion [¶](#appendix-C-2.3.1)
- \[\] - Signature key owner clarification [¶](#appendix-C-2.4.1)
- \[\] - Signature hashing clarification [¶](#appendix-C-2.5.1)
- \[\] - Self-signature applies to user ID correction [¶](#appendix-C-2.6.1)
- \[\] - Session key encryption storage clarification [¶](#appendix-C-2.7.1)
- \[\] - Simple hash MUST/MAY clarification [¶](#appendix-C-2.8.1)
- \[\] - Native line endings SHOULD clarification [¶](#appendix-C-2.9.1)
- \[\] - Radix-64/base64 clarification [¶](#appendix-C-2.10.1)
- \[\] - ASCII/UTF-8 collation sequence clarification [¶](#appendix-C-2.11.1)
- \[\] - Packet Composition is a sequence clarification [¶](#appendix-C-2.12.1)
- \[\] - Subkey packets come after all User ID packets clarification [¶](#appendix-C-2.13.1)
- \[\] - Subkey removal clarification [¶](#appendix-C-2.14.1)
- \[\] - mL/emLen variable correction [¶](#appendix-C-2.15.1)
- \[\] - CFB mode initialization vector (IV) clarification [¶](#appendix-C-2.16.1)
- \[\] - SHA-224 octet sequence correction [¶](#appendix-C-2.17.1)
- \[\] - Radix-64 correction [¶](#appendix-C-2.18.1)
- \[\] - Key Revocation signatures correction [¶](#appendix-C-2.19.1)
- \[\] - C code fix for CRC24\_POLY define [¶](#appendix-C-2.20.1)
- \[\] - Armor Header colon hex fix [¶](#appendix-C-2.21.1)
- \[\] - Signature/certification correction [¶](#appendix-C-2.22.1)
...
This document also draws on much previous work from a number of other authors including,,,,,,,,,,,,,, and.[¶](#appendix-D-3)
