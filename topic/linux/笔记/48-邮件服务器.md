...
`DKIM`（DomainKeys Identified Mail）在发出的邮件头部添加[[数字签名]]，收件方通过 DNS 查询公钥验证邮件未被篡改：

**生成 DKIM 密钥对（以 OpenDKIM 为例）**
...
**/etc/postfix/main.cf（添加 LMTP 投递）**
...
**/etc/dovecot/conf.d/10-master.conf（启用 LMTP socket）**
...
**/etc/dovecot/conf.d/10-master.conf（暴露 SASL 接口）**
