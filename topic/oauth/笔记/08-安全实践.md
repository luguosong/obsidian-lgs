...
常规[[授权流程]]中，授权请求参数通过浏览器 URL 传递，存在以下问题：
...
> 注意：DPoP Proof 只签名了 HTTP 方法和 URL，`不签名`请求体和其他请求头。这是刻意为之的设计——避免了 HTTP [[消息规范]]化的复杂难题。消息完整性依赖 TLS 层保障。
...
DPoP 还可以将授权码绑定到客户端的 DPoP 密钥，实现`端到端`的[[授权流程]]绑定——从授权请求到 Token 请求再到资源访问，全程使用同一密钥对。
