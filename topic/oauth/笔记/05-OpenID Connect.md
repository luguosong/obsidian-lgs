...
还记得[[核心概念]]里讲的 JWKS 端点吗？发现文档就是它的升级版——不是一个端点的地址，而是`所有端点的地址集合`，外加服务器的能力声明。客户端访问一个固定的 URL 就能拿到全部配置，不用逐个硬编码。
...
2. `展示授权页面`：授权服务器展示登录/同意页面，[[用户认证]]过程完全在授权服务器侧进行。
...
OIDC 提供了标准的[[会话管理]]机制，允许客户端（RP）实时感知用户在 OP（OpenID Provider）端的登录状态。
