...
>
> OAuth 2.1 没有引入新的[[授权流程]]或令牌类型。它的价值在于`将分散的安全要求集中为一个文档`，让开发者不再需要同时参考多个 RFC 来实现安全的 OAuth 系统。OAuth 2.0 生态中的扩展规范（如 [[Token Exchange]] RFC [[8693]]、[[Device Auth]]orization Grant RFC [[8628]] 等）不受影响，继续独立适用。
