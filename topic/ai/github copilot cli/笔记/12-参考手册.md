...
| `copilot --remote` | 启用[[远程访问]]（GitHub.com / Mobile） |
...
| `SERVER-NAME` | [[MCP 服务器]]工具调用 | `MyMCP(create_issue)`, `MyMCP` |
...
| `COPILOT_CUSTOM_INSTRUCTIONS_DIRS` | [[自定义指令]]附加目录（逗号分隔） |
...
| `copilot-instructions.md` | 全局[[自定义指令]]（应用于所有会话） |
| `instructions/` | 额外指令文件目录（`*.instructions.md`） |
| `mcp-config.json` | 用户级 [[MCP 服务器]]定义 |
| `lsp-config.json` | 用户级 [[LSP 服务器]]定义 |
...
| `mcp-servers` | object | 否 | [[MCP 服务器]]配置 |
