...
| `上下文维护` | 开发者大脑手动索引跨文件关系 | AI 通过上下文窗口和[[代理模式]]自动扫描关联文件 |
...
[[Copilot]] CLI 提供了多层次的[[扩展机制]]，从轻量级的 [[Skills]] 到完整的插件系统，覆盖不同场景的扩展需求。
...
> 适合刚接触 [[Copilot]] CLI 的开发者，掌握安装配置和基本[[交互模式]]。

> **进阶路径**：workflows → agents → skills → hooks → mcp
> 在熟悉基础后，学习工作流编排和各类[[扩展机制]]，提升自动化能力。

> **高级路径**：byok → automation → remote → sdk
> 面向需要深度定制和集成的场景，包括自带模型、[[远程访问]]和 SDK 开发。
