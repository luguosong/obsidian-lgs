...
- **Mid-input slash 命令**：slash 命令可出现在输入中间，单条消息可调用多个 skill
- **`userPromptSubmitted` 钩子直返**：钩子可直接处理请求并返回响应，绕过 LLM 调用
- **`--prerelease` 更新参数**：`copilot update` 和 `/update` 支持获取最新预发布版本

### 🔧 改进

- **多账户操作加速**：`/user list` 和 `/user switch` 响应更快
- **工具权限保持**：autopilot 模式下授予的工具权限在 `/clear` 后保留
- **Effort level 应用**：通过 `/model` 切换模型时正确应用 effort level

### 🐛 修复

- **`/add-dir` 路径补全**：不再闪烁或被 `@` 和 `#` 选择器拦截
- **Shell 命令**：`!` 前缀的 shell 命令在所有 shell 配置下正常工作
- **Shell 别名支持**：`!` 命令现在支持 shell 别名和 rc 文件设置
- **配额显示**：Free 用户正确显示剩余用量而非始终显示 100% 已用
- **Ctrl+C 权限提示**：权限提示待处理时按 Ctrl+C 不再导致 CLI 挂起
- **无效 URL 处理**：settings.json 中的无效 URL 不再导致 CLI 启动崩溃，改为跳过并警告
- **Slash 命令选择器**：无匹配结果时仍显示项目信息
- **Timeline 显示**：rubber-duck 子 Agent 显示已解析的模型名称
...
- **用户名 Toggle**：`/statusline` 选择器添加用户名开关，在页脚显示活跃账户

### 🔧 改进

- **Auto mode 模型路由**：使用服务端模型路由改进实时模型选择
- **MCP 子进程终止**：会话结束时通过 npx 或 uvx 启动的 MCP 子进程现在完全终止
- **更新进度显示**：运行更新命令时显示下载进度

### 🐛 修复

- **恢复提示**：多个会话活跃时显示正确的会话名称

### 🔒 安全

- **恶意仓库防护**：修复项目内嵌套恶意 bare 仓库导致的 RCE 漏洞 (GHSA-9ccr-r5hg-74gf)
...
- **`-C <directory>` 标志**：启动前切换工作目录，类似 `git -C`
- **Rubber-duck Agent**：为 GPT 会话添加 rubber-duck Agent（实验性模式下可用）

### 🔧 改进

- **MCP 错误诊断**：[[MCP 服务器]]失败警告包含 stderr 输出，并建议直接可运行的 `/mcp show` 命令
- **远程会话导出**：支持非 GitHub 仓库和无仓库目录
- **退出消息**：未重命名的会话显示 session ID 而非自动生成名称

### 🐛 修复

- **Enter 键卡死**：取消请求后 Enter 键不再永久卡住
- **Windows 更新**：包解压时瞬态 EPERM 不再导致 ENOENT 失败
- **会话恢复**：选择 "Go back" 后不再显示虚假 "session in use" 警告
- **空会话退出**：无用户消息且无保存会话时不再显示退出摘要
...
- **CLI 启动加速**：认证在后台解析的同时立即渲染 UI
- **Shell 自动补全**：bash、zsh、fish 补全在首次运行时自动安装，`copilot update` 后自动更新
- **Tab 补全改进**：接受参数的 slash 命令 Tab 补全时自动添加尾部空格
- **实验性 MCP Tasks**：带有 `taskSupport: "required"` 的 MCP 工具作为非阻塞后台 Agent 运行，通过 `list_agents` 和 `read_agent` 跟踪
- **`-p` 模式扩展加载**：用户扩展默认加载，项目扩展需 `GITHUB_COPILOT_PROMPT_MODE_EXTENSIONS=true`
- **`--attachment` 标志**：非[[交互模式]] (-p/--prompt) 下附加文件到初始提示
- **`@` 补全改进**：支持 `./` 路径，不再对目录添加尾部空格，优先显示项目文件

### 🔧 改进

- **远程会话错误提示**：显示已登录账户和针对性修复步骤
- **Markdown 渲染**：ask user prompt 问题中正确渲染 Markdown 格式
- **Slash 命令搜索**：搜索命令描述并高亮匹配字符
- **Memory 确认提示**：显示存储范围（仓库或用户）
- **流式渲染优化**：慢速主机上保持流畅的文本和闪烁动画
- **Windows 稳定性**：通过解决 Node 24.x 中的 V8 崩溃提升稳定性
- **文件编辑可靠性**：更好地从模糊或错位的编辑块中恢复

### 🐛 修复

- **Windows 包解压**：杀毒软件导致 EPERM 错误时不再崩溃
- **响应 XML 标签**：Assistant 响应不再包含多余的 system notification XML 标签
- **Large output 引导**：正确引用配置的 grep 工具名称
- **插件市场 SSH URL**：使用 git SSH URL 添加插件市场现在正常
- **会话文件 Unicode**：包含 Unicode 行分隔符的会话文件正确加载
- **Reasoning effort 提示**：正确显示 "Esc to cancel" 间距
- **SQL todo 时间线**：更准确地显示 INSERT OR IGNORE/REPLACE 和 blocked 状态更新
...
- **MCP `client_credentials` OAuth 授权**：[[MCP 服务器]]支持 `client_credentials` OAuth 授权类型，可实现无需浏览器的完全无头认证
...
- 修复 `preToolUse.matcher` 被忽略的问题。升级后，带 `matcher` 的 Hook 仅对完全匹配[[正则表达式]]的工具名称运行
- [[自定义指令]]文件在 `.gitignored` 目录（如 `.github/instructions/`）中现在正确加载
...
- [[LSP 服务器]]配置支持可配置的 spawn、initialization 和 warmup 超时
- 上下文窗口指示器在状态栏中默认隐藏
- MCP OAuth 迁移到共享运行时流程，移除 [[MCP 服务器]]时清除关联 OAuth 状态
- 自动纠正自定义工具调用在 [[Agent 循环]]中的错误
...
- `/clear` 或 `/new` 后，[[自定义指令]]和 skill 会从磁盘重新加载
...
- 去重内容相同的[[自定义指令]]文件（如 `copilot-instructions.md` 和 `CLAUDE.md`），减少每轮对话的 token 浪费
...
- 修复 `sessionStart` 和 `sessionEnd` Hook 在[[交互模式]]下每个提示触发一次，改为每个会话只触发一次
- 修复通过 BYOM/BYOK 配置使用 Anthropic 模型时，权限检查和其他 Hook 无法正常工作的问题
- 修复插件 Agent 未遵循 frontmatter 中指定的模型配置

### 🗑️ 移除

- 移除 `.vscode/mcp.json` 和 `.devcontainer/devcontainer.json` 作为 [[MCP 服务器]]配置源，CLI 现在只读取 `.mcp.json`。检测到 `.vscode/mcp.json` 但没有 `.mcp.json` 时显示迁移提示
...
- **`copilot mcp` 命令**：新增 [[MCP 服务器]]管理命令
...
- [[MCP 服务器]]在工作目录变更后能正确使用有效认证重新连接

### 🐛 修复

- [[MCP 服务器]]在登录、用户切换和 `/mcp reload` 后能正确加载
...
- **MCP 配置 RPC**：新增 `mcp.config.list`、`mcp.config.add`、`mcp.config.update`、`mcp.config.remove` 服务端 RPC，用于管理持久化 [[MCP 服务器]]配置
- [[Copilot]] 吉祥物现在在[[交互模式]]下会有微妙的眨眼动画 👀
...
- 被 allowlist 策略阻止的 [[MCP 服务器]]从 `/mcp show` 中隐藏
...
- **MCP Sampling 支持**：[[MCP 服务器]]可请求 LLM 推理（sampling），需用户通过审查提示批准
...
- 会话结束时正确清理 shell 子进程
- [[MCP 服务器]]使用 Microsoft Entra ID 认证时不再每次登录都显示同意界面
...
- `.mcp.json` 定义的 [[MCP 服务器]]在工作目录为 git 根目录时正确启动
- Windows 上当非系统 `clip.exe` 在 PATH 中优先时剪贴板复制正常工作
- `/diff` 在存在行内高亮时正确渲染所有行
- Workspace [[MCP 服务器]]正确加载并对 Agent 可见
...
- [[MCP 服务器]]被策略阻止时显示警告
- 组织级第三方 [[MCP 服务器]]策略对所有用户强制执行
- 多个扩展的 Hook 现在合并而非覆盖
- `sessionStart` Hook 的 `additionalContext` 注入到对话中
- GitHub [[MCP 服务器]]连接远程主机时遵守用户配置
- 启动消息显示已加载的 Hook 数量
...
- 通过 `--plugin-dir` 加载的插件在 `/plugin list` 的"外部[[插件"]]分区中显示
...
- Workspace [[MCP 服务器]]在确认文件夹信任后才加载
...
- **MCP Allowlist 验证**：[[MCP 服务器]]可通过 `MCP_ALLOWLIST` 功能标志对配置的注册表进行验证
...
- [[自定义指令]]文件路径通过 `COPILOT_CUSTOM_INSTRUCTIONS_DIRS` 正确加载
...
- **DevContainer MCP 配置**：从 `.devcontainer/devcontainer.json` 读取 [[MCP 服务器]]配置
...
- CLI 崩溃时终端正确重置
- 具有非标准 `outputSchema` 的 [[MCP 服务器]]可正常访问
...
- **MCP 带外交互**：[[MCP 服务器]]可请求用户访问 URL 进行带外交互（OAuth、API Key 等）
...
- 插件贡献的 [[LSP 服务器]]在 `/lsp show` 中加载和显示
...
- GitHub [[MCP 服务器]]在同时存在 Azure DevOps 和 GitHub remote 时保持启用
