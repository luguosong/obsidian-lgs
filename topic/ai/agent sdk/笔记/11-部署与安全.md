...
| 网络 | 出站 HTTPS 到 `api.anthropic.com`，可选访问 [[MCP 服务器]]或外部工具 |
...
**空闲容器应该保持还是关闭？**
取决于你的用户[[交互模式]]。不同沙箱提供商支持不同的空闲超时配置，根据用户响应频率调整。容器最低运行成本约每小时 5 美分。

**如何与容器内的 Agent 通信？**
在容器中暴露端口，应用为外部客户端提供 HTTP/Web[[Socket]] 端点，SDK 在容器内部运行。
