...
匹配器用[[正则表达式]]过滤 Hook 的触发条件。对于工具类 Hook，匹配器按工具名称过滤：
...
当你需要把 [[Skills]]、Agents、Hooks 和 [[MCP 服务器]]打包成一个可复用、可分发的单元时，Plugins 就是答案。它让团队共享扩展变得简单——一个 Plugin 目录包含了所有相关配置。
