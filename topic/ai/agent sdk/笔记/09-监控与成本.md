...
Traces（追踪）是三种信号中最详细的。启用 `CLAUDE_CODE_ENHANCED_TELEMETRY_BETA=1` 后，[[Agent 循环]]的每一步都变成一个 span：

| Span 名称 | 包含内容 |
|----------|---------|
| `claude_code.interaction` | [[Agent 循环]]的一次完整轮次（从接收 prompt 到产生响应） |
...
**Python：自定义 service name 和 resource attributes**
...
**TypeScript：自定义 service name 和 resource attributes**
...
结果消息（`ResultMessage`）标志着 [[Agent 循环]]的结束，包含 `total_cost_usd` 字段——该调用中所有步骤的累积估算成本：
