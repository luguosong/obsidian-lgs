...
- **权限模式**：应用当前权限模式。`bypassPermissions` 批准到达此步骤的所有请求；`acceptEdits` 自动批准[[文件操作]]；其他模式继续向下
...
自动批准[[文件操作]]，让 Claude 可以不经确认地编辑代码。其他工具（如非文件系统操作的 Bash 命令）仍需正常权限。
