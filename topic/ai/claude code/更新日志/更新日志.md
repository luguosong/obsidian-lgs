...
- **MCP 服务器在 `/clear` 后消失**：修复 VS Code 扩展、JetBrains 插件和 Agent SDK 中，`/clear` 后 `.mcp.json` 配置的 MCP 服务器、插件和 claude.ai 连接器静默消失的问题
- **OAuth 登录循环**：修复并发凭证写入可能覆盖刚轮换的 OAuth token 并强制重新登录的罕见问题
- **MCP OAuth 刷新 token 丢失**：修复多个服务器并发刷新时 MCP OAuth refresh token 丢失的问题，拥有多个远程 MCP 服务器的用户不再需要每天重新认证
...
- **MCP 工具结果不可见**：修复 MCP 服务器返回 content blocks 时工具结果不可见的问题
...
- **stdio MCP 服务器内存泄漏**：修复 stdio MCP 服务器向 stdout 写入非协议数据时导致无限内存增长（10GB+ RSS）的问题
- **MCP 工具列表失败静默处理**：修复连接但 `tools/list` 失败的 MCP 服务器静默显示 0 个工具的问题，现在会重试一次并在 `/mcp` 中显示"connected · tools fetch failed"
...
- **WebSocket 警告日志级别**：修复 `--debug` 模式下语音模式中无害的 WebSocket 警告被记录为错误的问题
