...
`/resume` 用于恢复之前的会话。选择器默认显示当前目录的 session，按 `Ctrl+A` 切换为显示所有项目（v2.1.108 改进）。此外，`--resume` / `--continue` 现在还会恢复未过期的[[计划任务]]（v2.1.110 新增），让你继续上次中断的定时任务。
...
1. **零副作用探索**：Plan Mode 下所有[[文件操作]]都是虚拟的——不会真正写入磁盘，也不会触发构建、测试等工具链。即使探索了 50 个文件，退出 Plan Mode 后这些中间过程不会留在上下文里（只保留最终方案）
