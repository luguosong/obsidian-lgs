...
> 本节从**平台视角**概述 Chrome 扩展的定位和核心能力。详细的安装步骤、可用工具列表、工作流示例和[[故障排除]]，参见 [第三方集成 — Chrome 扩展](../integrations/index.md) 章节。
...
| 会话模式 | **交互式会话**（使用 `-p` 标志的非[[交互模式]]不可用） |
| 内置 [[MCP 服务器]]名称 | `computer-use`（在 `/mcp` 菜单中启用） |
...
详细的 Remote Control CLI 标志、连接配置和[[故障排除]]，参见[[第三方集成]]笔记中的 [Channel 与其他集成方式对比](../integrations/index.md) 章节。
...
当你需要在 CI 或脚本中获得**一致、可复现**的结果时，`--bare` 是首选。它跳过 Hooks、[[Skills]]、Plugins、[[MCP 服务器]]、自动记忆和 `CLAUDE.md` 的自动发现——只有你显式传入的标志才会生效。队友的 `~/.claude` 中的 Hook 或项目的 `.mcp.json` 中的 [[MCP 服务器]]都不会运行。
