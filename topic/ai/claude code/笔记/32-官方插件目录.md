...
[[Claude Code]] 内置的[[官方市场]] `claude-plugins-official`（[GitHub 仓库](https://github.com/anthropics/claude-plugins-official) | [在线目录](https://claude.com/plugins)）提供了 **33 个内部插件** 和 **15 个外部集成**（截至 2025 年 5 月，数据可能随版本更新变动，建议通过 `/plugin search` 获取最新列表）。本页按功能分类列出所有插件，方便快速查找和安装。
...
| `feature-dev` | Commands、Agents | 结构化功能[[开发工作流]]，7 个阶段 + 专用代理，从需求到实现全覆盖 |
| `pr-review-toolkit` | Agents | PR 审查工具集，6 个代理覆盖评论、测试、错误、类型和代码质量 |
| `hookify` | Commands | 分析你的[[行为模式]]或显式指令，自动生成 Hooks 来阻止不期望的操作（如防止 Claude 自动提交代码） |
...
帮助你构建 [[Claude Code]] 生态的插件、[[Skills]]、[[MCP 服务器]]等。
...
| `mcp-server-dev` | [[Skills]]、MCP | [[MCP 服务器]]设计与构建，支持交互式 UI 和 bundling |
