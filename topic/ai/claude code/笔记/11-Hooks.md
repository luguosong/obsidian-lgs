...
| `mcp_tool` | 调用 MCP 工具 | 需要直接调用已连接的 [[MCP 服务器]]工具时（v2.1.118 新增） |
...
| `Elicitation` | [[MCP 服务器]]请求用户输入时 | ✅ 可自动响应 |
...
不是每个事件都需要响应所有情况。`matcher` 字段用[[正则表达式]]精确过滤触发条件：
...
⚠️ 注意：`matcher` 的匹配规则是[[正则表达式]]，大小写敏感。`Edit` 不会匹配 `edit`。
...
| `Elicitation` / `ElicitationResult` | [[MCP 服务器]]名称 | 你配置的 [[MCP 服务器]]名称 |
...
| `ask` | 正常弹出权限确认对话框 |
| `defer` | 仅在 `-p` 非[[交互模式]]下有效，暂停等待外部处理（[[Agent SDK]] 可收集输入后恢复） |
...
💡 这个 Hook 的[[工作原理]]：`jq -r '.tool_input.file_path'` 从 stdin 的 JSON 中提取被编辑文件的路径，然后传给 Prettier 格式化。因为使用 `PostToolUse`，格式化在文件写入之后执行，所以不会被阻止。
...
**.claude/hooks/block-protected-files.sh**
