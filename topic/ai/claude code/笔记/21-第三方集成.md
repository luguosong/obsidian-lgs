...
> Bedrock 的「推理配置文件 ID」用于将 API 请求固定到特定模型版本，避免 Bedrock 平台自动更新模型带来的行为变化。你可以在 AWS Bedrock 控制台的 **Inference [[Profile]]s** 页面找到它。
...
推理配置文件 ID 可以在 AWS 控制台的 Bedrock → Inference [[Profile]]s 页面找到。
...
| `.claude/.mcp.json` | 项目级 | 项目级 [[MCP 服务器]]配置 |
