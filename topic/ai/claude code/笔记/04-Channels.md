...
- 🎯 Channel 是什么，它和普通 [[MCP 服务器]]有什么本质区别
...
你可能已经在用 [[MCP 服务器]]给 Claude 接入了数据库、Slack 等外部服务。那 Channel 和普通 MCP 有什么不同？
...
> [!info] 终端中的显示
>
> 当 Claude 通过 Channel 回复时，终端会显示工具调用和确认信息（如「已发送」），但不会显示回复的[[文本内容]]。实际的回复出现在对应的平台（Telegram、Discord 等）上。
...
> [!tip] 市场找不到插件？
>
> 运行 `/plugin marketplace update claude-plugins-official` 刷新市场，或 `/plugin marketplace add anthropics/claude-plugins-official` 添加[[官方市场]]后重试。
